(function () {
  ...
  const PAY_LABELS = { cash: 'Наличные', kaspi: 'Kaspi QR', transfer: 'Банк', mixed: 'Смешанная', debt: 'В долг' };
  ...
  function setProducts(arr) {
    if (window.DataService) window.DataService.set('products', arr);
  }
  ...
  function getDeferred() { return window.ApDb ? window.ApDb.getDeferred() : []; }
  function setDeferred(arr) { if (window.ApDb) window.ApDb.setDeferred(arr); }
  function getDebts() { return window.ApDb ? window.ApDb.getDebts() : []; }
  function setDebts(arr) { if (window.ApDb) window.ApDb.setDebts(arr); }
  function getDebtors() { return window.ApDb ? window.ApDb.getDebtors() : []; }
  function setDebtors(arr) { if (window.ApDb) window.ApDb.setDebtors(arr); }
  ...
  function getDocTemplates() { try { return JSON.parse(localStorage.getItem('ap_doc_templates') || '[]'); } catch (e) { return []; } }
  function setDocTemplates(arr) { try { localStorage.setItem('ap_doc_templates', JSON.stringify(arr)); } catch (e) {} }

  function _closeParentModals() {
    var parents = ['modal-view-document', 'modal-create-invoice', 'modal-create-z2', 'modal-create-sf'];
  ...
  window.openDocTemplateManager = function () {
    _closeParentModals();
    openModal('modal-doc-templates');
    renderDocTemplates();
  };
  window.renderDocTemplates = function () {
    var list = document.getElementById('doc-templates-list');
    if (!list) return;
    var templates = getDocTemplates();
    if (!templates.length) { list.innerHTML = '<div class="empty">Нет сохранённых шаблонов</div>'; return; }
    list.innerHTML = templates.map(function (t, i) {
      return '<div class="doc-template-card" style="background:var(--bg3);border:1px solid var(--border);border-radius:8px;padding:12px;margin-bottom:8px;cursor:pointer" onclick="loadDocTemplate(' + i + ')">' +
        '<div style="display:flex;justify-content:space-between;align-items:center">' +
        '<div><strong>' + esc(t.name || 'Шаблон ' + (i + 1)) + '</strong><br><span style="font-size:12px;color:var(--muted)">' + esc(t.storeName || '') + (t.bin ? ' · ' + esc(t.bin) : '') + '</span></div>' +
        '<div style="display:flex;gap:4px">' +
        '<button class="btn btn-sm btn-secondary" onclick="event.stopPropagation();editDocTemplate(' + i + ')">✏️</button>' +
        '<button class="btn btn-sm btn-danger" onclick="event.stopPropagation();deleteDocTemplate(' + i + ')">✕</button>' +
        '</div></div></div>';
    }).join('');
  };
  window.editDocTemplate = function (idx) {
    var templates = getDocTemplates();
    var t = templates[idx] || {};
    ...
    document.getElementById('tpl-save-btn').onclick = function () { saveDocTemplate(idx); };
    document.getElementById('tpl-save-as-new-btn').style.display = '';
    openModal('modal-doc-template-edit');
  };
  window.deleteDocTemplate = function (idx) {
  ...
  window.addDocTemplate = function () {
    window._editingDocTemplateIdx = -1;
    var f = function (id) { return document.getElementById(id); };
    f('tpl-name').value = ''; f('tpl-storeName').value = ''; f('tpl-bin').value = ''; f('tpl-iik').value = '';
    f('tpl-bik').value = ''; f('tpl-bankName').value = ''; f('tpl-kbe').value = ''; f('tpl-paymentCode').value = '';
    f('tpl-beneficiary').value = ''; f('tpl-address').value = ''; f('tpl-phone').value = '';
    f('tpl-directorName').value = ''; f('tpl-directorPosition').value = ''; f('tpl-extra').value = '';
    f('tpl-modal-title').textContent = 'Новый шаблон';
    document.getElementById('tpl-save-btn').onclick = function () { saveDocTemplate(-1); };
    document.getElementById('tpl-save-as-new-btn').style.display = 'none';
    openModal('modal-doc-template-edit');
  };
  window.saveDocTemplate = function (idx) {
    var f = function (id) { return document.getElementById(id); };
    var t = {
    ...
    var templates = getDocTemplates();
    if (idx >= 0 && idx < templates.length) { templates[idx] = t; } else { templates.push(t); }
    setDocTemplates(templates);
    closeModal('modal-doc-template-edit');
    renderDocTemplates();
    toast('Шаблон сохранён', 'ok');
  };
  window.loadDocTemplate = function (idx) {
    // Always store in localStorage first (for both edit and create flows)
    window.loadDocTemplateNewDoc(idx);
    // Then populate edit form fields if they exist
    var templates = getDocTemplates();
    var t = templates[idx];
    if (!t) return;
    var si = { storeName: t.storeName, bin: t.bin, iik: t.iik, bik: t.bik, bankName: t.bankName, kbe: t.kbe, paymentCode: t.paymentCode, beneficiary: t.beneficiary, address: t.address, phone: t.phone, directorName: t.directorName, directorPosition: t.directorPosition, extra: t.extra };
    var fields = [
      ['edit-storeName', 'storeName'], ['edit-storeBIN', 'bin'], ['edit-iik', 'iik'], ['edit-bik', 'bik'],
      ['edit-bankName', 'bankName'], ['edit-kbe', 'kbe'], ['edit-paymentCode', 'paymentCode'],
      ['edit-beneficiary', 'beneficiary'], ['edit-storeAddress', 'address'], ['edit-storePhone', 'phone'],
      ['edit-directorName', 'directorName'], ['edit-directorPosition', 'directorPosition'], ['edit-storeExtra', 'extra']
    ];
    fields.forEach(function (pair) {
      var el = document.getElementById(pair[0]);
      if (el) el.value = si[pair[1]] || '';
    });
    closeModal('modal-doc-templates');
    _reopenParentModal();
    toast('Шаблон загружен: ' + t.name, 'ok');
  };
  window.loadDocTemplateNewDoc = function (idx) {
    var templates = getDocTemplates();
    var t = templates[idx];
    if (!t) return;
    // Store selected template info in localStorage for use when creating new docs
    try { localStorage.setItem('ap_selected_template', JSON.stringify(t)); } catch (e) {}
  };
  window.selectDocTemplateForDoc = function () {
    _closeParentModals();
    openModal('modal-doc-templates');
    renderDocTemplates();
  };
  ...
  // Generate search variations for typo/layout handling
  function generateSearchVariations(term) {
    ...
    // English -> Russian keyboard layout mapping
    var enToRu = {
    ...
    // Try Russian layout if input looks like English, and vice versa
    var hasEnglish = /[a-zA-Z]/.test(t);
    var hasRussian = /[а-яА-ЯёЁ]/.test(t);

    if (hasEnglish && !hasRussian) {
      // Try to transliterate as if typed in Russian layout with English keyboard
      var translit = '';
      for (var i = 0; i < t.length; i++) {
        translit += enToRu[t[i]] || t[i];
      }
      if (translit !== t) results.push(translit);
    }

    if (hasRussian && !hasEnglish) {
      // Try to transliterate as if typed in English layout with Russian keyboard
      var translit2 = '';
      for (var j = 0; j < t.length; j++) {
        translit2 += ruToEn[t[j]] || t[j];
      }
      if (translit2 !== t) results.push(translit2);
    }

    // Also add the original term with some common substitutions
    // 'c' -> 'к', 'e' -> 'и', 'o' -> 'о', 'a' -> 'а' (for mixed layout)
    var mixed = '';
    var mixedMap = { 'c':'к', 'e':'е', 'o':'о', 'a':'а', 'k':'к', 'm':'м', 't':'т' };
    for (var k = 0; k < t.length; k++) {
      mixed += mixedMap[t[k]] || t[k];
    }
    if (mixed !== t) results.push(mixed);

    return results;
  }

  window.smartMatchProducts = function(term) {
    var t = term.trim();
    if (!t) return [];

    // Generate search variations for typo/transliteration handling
    var variations = generateSearchVariations(t);

    var products = getProducts().filter(function(p) { return p.quantity > 0; });
    var scored = [];
    ...
    products.forEach(function(p) {
      ...
      variations.forEach(function(v) {
        ...
        if (searchText === vl) { bestScore = Math.max(bestScore, 1000); matchType = 'exact'; return; }

        // Exact barcode match
        if (barcode === vl) { bestScore = Math.max(bestScore, 950); matchType = 'barcode'; return; }

        // Exact code match
        if (code === vl) { bestScore = Math.max(bestScore, 900); matchType = 'code'; return; }

        // Starts with
        if (name.indexOf(vl) === 0) { bestScore = Math.max(bestScore, 500 + (1000 - vl.length)); matchType = 'prefix'; return; }
        if (code.indexOf(vl) === 0) { bestScore = Math.max(bestScore, 400); matchType = 'code_prefix'; return; }

        // Contains (substring)
        if (name.indexOf(vl) >= 0) { bestScore = Math.max(bestScore, 200 + (1000 - vl.length) + (100 - name.indexOf(vl))); matchType = 'substring'; }
        if (code.indexOf(vl) >= 0) { bestScore = Math.max(bestScore, 150); matchType = 'code_substr'; }
        if (sku.indexOf(vl) >= 0) { bestScore = Math.max(bestScore, 140); matchType = 'sku'; }

        // Word-by-word matching (e.g. "кола" matches "Coca-Cola")
        var nameWords = name.split(/[\s\-_.,\/]+/);
        nameWords.forEach(function(w) {
          if (w.indexOf(vl) === 0) bestScore = Math.max(bestScore, 180 + (1000 - vl.length));
          if (w.length > 2 && vl.length > 2) {
            // Fuzzy matching for typos
            var dist = levenshtein(w, vl);
            if (dist <= 1) bestScore = Math.max(bestScore, 160);
            else if (dist <= 2) bestScore = Math.max(bestScore, 80);
          }
        });
      });

      if (bestScore > 0) {
        scored.push({ product: p, score: bestScore, matchType: matchType });
      }
    });
  ...
  // Hotkey to restore sidebar: Ctrl+Shift+S
  document.addEventListener('keydown', function (e) {
    if (e.ctrlKey && e.shiftKey && (e.key === 'S' || e.key === 's')) {
      var restoreBtn = document.getElementById('sidebar-restore-btn');
      if (restoreBtn && restoreBtn.style.display === 'flex') { window.restoreSidebar(); e.preventDefault(); return; }
    }
  });
  document.addEventListener('keydown', function (e) {
  ...
  var fmt = window.fmt;
  window.fmtShort = function (n) { return (Number(n) || 0).toLocaleString('ru-RU'); };
  var fmtDate = window.fmtDate;
  var fmtDateTime = window.fmtDateTime;
  var escapeHtml = window.escapeHtml;
  var esc = window.esc;
  var todayStr = window.todayStr;

  // ─── ЦЕНТРАЛИЗОВАННАЯ СИСТЕМА EXCEL ───
  // Конфигурация колонок для всех разделов (единый источник истины)
  var EXCEL_SECTIONS = {
    products: {
      label: 'Товары',
      headers: ['Штрихкод', 'Артикул', 'Наименование', 'Категория', 'Единица измерения', 'Закупочная цена', 'Цена продажи', 'Остаток на складе', 'Минимальный остаток', 'Поставщик', 'Описание', 'Статус'],
      fields: ['barcode', 'code', 'name', '_categoryName', 'unit', 'purchasePrice', 'price', 'quantity', 'minStock', 'supplier', 'description', '_status'],
      widths: [18, 14, 42, 20, 10, 14, 14, 12, 12, 22, 30, 12],
      numFmt: ['s', 's', 's', 's', 's', '#,##0', '#,##0', '#,##0', '#,##0', 's', 's', 's'],
      required: ['barcode', 'code', 'name'],
      dataKey: 'productData'
    },
    sales: {
      label: 'Продажи',
      headers: ['Номер документа', 'Дата', 'Клиент', 'Товар', 'Количество', 'Цена', 'Сумма', 'Способ оплаты', 'Кассир', 'Статус'],
      fields: ['_docNum', 'date', '_customerName', 'productName', 'quantity', 'unitPrice', 'total', '_paymentLabel', 'userName', '_saleStatus'],
      widths: [16, 18, 24, 42, 10, 14, 16, 18, 20, 14],
      numFmt: ['s', 'dd.mm.yyyy hh:mm', 's', 's', '#,##0', '#,##0', '#,##0', 's', 's', 's'],
      required: ['productName', 'quantity', 'total'],
      dataKey: 'saleData'
    },
    documents: {
      label: 'Документы продаж',
      headers: ['Номер', 'Тип', 'Дата', 'Клиент', 'Телефон', 'Сумма', 'Статус', 'Создатель'],
      fields: ['docNumber', '_docType', 'documentDate', 'customerName', 'customerPhone', 'total', '_docStatus', '_createdByName'],
      widths: [14, 16, 18, 30, 18, 16, 14, 22],
      numFmt: ['s', 's', 'dd.mm.yyyy', 's', 's', '#,##0', 's', 's'],
      required: ['customerName', 'total'],
      dataKey: 'documentData'
    },
    expenses: {
      label: 'Расходы',
      headers: ['Категория', 'Сумма', 'Дата', 'Описание', 'Кем создан', 'Статус'],
      fields: ['category', 'amount', 'date', 'description', 'userName', '_expenseStatus'],
      widths: [22, 16, 18, 40, 22, 14],
      numFmt: ['s', '#,##0', 'dd.mm.yyyy', 's', 's', 's'],
      required: ['category', 'amount'],
      dataKey: 'expenseData'
    },
    customers: {
      label: 'Клиенты',
      headers: ['Имя', 'Телефон', 'Уровень', 'Сумма покупок', 'Бонусы', 'Последняя покупка', 'Дата регистрации'],
      fields: ['name', 'phone', '_tierName', 'spent', 'bonusBalance', '_lastPurchase', 'createdAt'],
      widths: [24, 18, 14, 18, 14, 20, 18],
      numFmt: ['s', 's', 's', '#,##0', '#,##0', 'dd.mm.yyyy', 'dd.mm.yyyy'],
      required: ['name', 'phone'],
      dataKey: 'customerData'
    },
    debts: {
      label: 'Долги',
      headers: ['Должник', 'Телефон', 'Рейтинг', 'Товар', 'Количество', 'Сумма', 'Кассир', 'Дата возврата', 'Статус', 'Примечание', 'Дата создания'],
      fields: ['debtorName', '_debtorPhone', '_debtorRating', 'productName', 'quantity', 'amount', 'cashierName', 'dueDate', 'status', 'note', 'date'],
      widths: [22, 18, 10, 34, 10, 16, 18, 16, 12, 30, 18],
      numFmt: ['s', 's', 's', 's', '#,##0', '#,##0', 's', 'dd.mm.yyyy', 's', 's', 'dd.mm.yyyy'],
      required: ['debtorName', 'amount'],
      dataKey: 'debtData'
    },
    deferred: {
      label: 'Отложенные',
      headers: ['Клиент', 'Телефон', 'Товар', 'Количество', 'Цена', 'Сумма', 'Кассир', 'Дата', 'Статус', 'Примечание'],
      fields: ['customerName', 'customerPhone', 'productName', 'quantity', 'unitPrice', 'total', 'cashierName', 'date', 'status', 'note'],
      widths: [24, 18, 40, 10, 14, 16, 20, 18, 14, 30],
      numFmt: ['s', 's', 's', '#,##0', '#,##0', '#,##0', 's', 'dd.mm.yyyy', 's', 's'],
      required: ['customerName', 'productName', 'total'],
      dataKey: 'deferredData'
    },
    shifts: {
      label: 'Смены',
      headers: ['Кассир', 'Открыта', 'Закрыта', 'Продаж', 'Выручка', 'Себестоимость', 'Прибыль'],
      fields: ['cashier', 'openedAt', 'closedAt', 'count', 'revenue', 'cogs', 'profit'],
      widths: [22, 18, 18, 10, 16, 16, 16],
      numFmt: ['s', 'dd.mm.yyyy hh:mm', 'dd.mm.yyyy hh:mm', '#,##0', '#,##0', '#,##0', '#,##0'],
      required: ['cashier', 'revenue'],
      dataKey: 'shiftData'
    },
    cashiers: {
      label: 'Кассиры',
      headers: ['Кассир', 'Смен', 'Продаж', 'Выручка', 'Себестоимость', 'Прибыль', 'Доля от выручки'],
      fields: ['name', 'shifts', 'sales', 'revenue', 'cogs', '_profit', '_share'],
      widths: [22, 10, 10, 16, 16, 16, 16],
      numFmt: ['s', '#,##0', '#,##0', '#,##0', '#,##0', '#,##0', '#,##0'],
      required: ['name', 'revenue'],
      dataKey: 'cashierData'
    }
  };

  // Helper: получить значение поля из объекта (в т.ч. вложенные пути)
  function getFieldValue(obj, field) {
    if (!obj || field == null) return '';
    if (field === '_categoryName') {
      var cat = getCategories().find(function (c) { return c.id === obj.category; });
      return cat ? cat.name : '';
    }
    ...
    if (field === '_customerName') {
      ...
      if (obj.customerId) { var c = getCustomers().find(function (x) { return x.id === obj.customerId; }); return c ? c.name : ''; }
      return '';
    }
    ...
    if (field === '_tierName') { var t = getCustomerTier(Number(obj.spent) || 0); return t ? t.name : ''; }
    if (field === '_lastPurchase') {
      if (!obj.id) return '';
      var lastSale = getSales().filter(isSaleActive).filter(function (s) { return s.customerId === obj.id; }).sort(function (a, b) { return (b.date || '').localeCompare(a.date || ''); });
      return lastSale.length ? lastSale[0].date : '';
    }
    if (field === '_debtorPhone' || field === '_debtorRating') {
      var debtors = window.getDebtors ? getDebtors() : [];
      var debtor = debtors.find(function (d) { return d.id === obj.debtorId; });
  ...
  async function createExcelWorkbook(headers, rows, widths, sheetName, numFmts) {
    if (typeof ExcelJS !== 'undefined') {
      ...
      headerRow.height = 28;
      headerRow.eachCell(function (cell) { styleExcelCell(cell, headerStyle); });
      ...
      rows.forEach(function (rowData) {
        ...
        row.eachCell(function (cell, colIdx) {
          ...
          styleExcelCell(cell, {
            ...
            alignment: { vertical: 'middle', wrapText: true, horizontal: isNum ? 'right' : 'left' },
            fill: alt ? { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF9FAFB' } } : { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFFFFF' } },
          ...
          if (fmt && fmt.indexOf('dd.') !== 0 && fmt !== 's') { cell.numFmt = fmt; }
        });
        alt = !alt;
      });
    ...
    // Fallback: XLSX (без стилей)
    if (typeof XLSX !== 'undefined') {
      var data = [headers].concat(rows);
      var ws2 = XLSX.utils.aoa_to_sheet(data);
      ws2['!cols'] = widths.map(function (w) { return { wch: w }; });
      var wb2 = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb2, ws2, sheetName || 'Данные');
      var out = XLSX.write(wb2, { bookType: 'xlsx', type: 'buffer' });
      return out;
    }
  ...
  async function exportSectionToExcel(section, data, filename) {
    ...
    if (!cfg) { toast('Неизвестный раздел: ' + section, 'err'); return; }
    if (!data || !data.length) { toast('Нет данных для экспорта', 'err'); return; }
  ...
  window.exportSectionToExcel = exportSectionToExcel;

  // ─── СКАЧАТЬ ШАБЛОН (пустой) ───
  async function downloadExcelTemplate(section) {
    var cfg = EXCEL_SECTIONS[section];
    if (!cfg) { toast('Неизвестный раздел', 'err'); return; }
    var emptyRow = cfg.fields.map(function () { return ''; });
  ...
  var _importState = { section: null, rawData: null, rows: null };
  ...
  reader.onload = function (e) {
    try {
      ...
      if (name.endsWith('.csv')) {
        // Для CSV используем raw текст и передаём опции CSV
        var text = new TextDecoder('utf-8').decode(e.target.result);
        var wb = XLSX.read(text, { type: 'string', raw: true });
      } else {
        var wb = XLSX.read(e.target.result, opts);
      }
      var ws = wb.Sheets[wb.SheetNames[0]];
      if (!ws) { toast('Файл не содержит листов', 'err'); return; }
      var raw = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' });
      if (raw.length < 2) { toast('Файл не содержит данных', 'err'); return; }
...
window.readExcelFile = readExcelFile;
  ...
  function showImportPreview(section, rawData, fileName) {
    ...
    var dataRows = rawData.slice(1).filter(function (r) { return r.some(function (c) { return c !== ''; }); });

    if (dataRows.length === 0) { toast('Нет данных для импорта', 'err'); return; }

    // Авто-сопоставление всех колонок из cfg.fields
    var colMap = {};
    var missing = [];
    cfg.fields.forEach(function (field) {
      ...
      headers.forEach(function (h, i) {
        if (String(h).toLowerCase().trim() === expected.toLowerCase().trim()) hdrIdx = i;
      });
      if (hdrIdx < 0) {
        headers.forEach(function (h, i) {
          if (hdrIdx >= 0) return;
          if (String(h).toLowerCase().indexOf(expected.toLowerCase().slice(0, 4)) >= 0) hdrIdx = i;
        });
      }
      if (hdrIdx >= 0) colMap[field] = hdrIdx;
    });
    // Проверяем только обязательные поля
    cfg.required.forEach(function (field) {
      if (colMap[field] === undefined) missing.push(cfg.headers[cfg.fields.indexOf(field)] || field);
    });
    if (missing.length) {
    ...
    _importState = { section: section, rawData: rawData, colMap: colMap, headers: headers, rows: dataRows, fileName: fileName };

    // Build preview table
    var previewHtml = '<div style="max-height:400px;overflow-y:auto">';
    previewHtml += '<table><thead><tr>';
    cfg.headers.forEach(function (h) { previewHtml += '<th>' + escapeHtml(h) + '</th>'; });
    ...
    if (dataRows.length > maxPreview) previewHtml += '<tr><td colspan="' + cfg.headers.length + '" style="text-align:center;color:var(--muted)">... и ещё ' + (dataRows.length - maxPreview) + ' строк</td></tr>';
    ...
    document.getElementById('import-preview-confirm').onclick = function () { confirmImport(); };
    openModal('modal-import-preview');
  }

  function confirmImport() {
    var state = _importState;
    if (!state || !state.section) { toast('Нет данных для импорта', 'err'); return; }
    ...
    dataRows.forEach(function (row) {
      try {
        var item = {};
        // Читаем все найденные колонки (не только обязательные)
        Object.keys(colMap).forEach(function (field) {
          var colIdx = colMap[field];
          if (colIdx >= 0) item[field] = (row[colIdx] !== undefined && row[colIdx] !== null ? String(row[colIdx]).trim() : '');
        });
        if (state.section === 'products') {
          if (!item.barcode && !item.code && !item.name) { errors++; return; }
          var existing = getProducts();
          // Ищем дубликат: только если barcode или code не пустые
          var dup = null;
          if (item.barcode) dup = existing.find(function (p) { return p.barcode === item.barcode; });
          if (!dup && item.code) dup = existing.find(function (p) { return p.code === item.code; });
          if (dup) {
            // Update existing
            if (item.name) dup.name = item.name;
            if (item.category) dup.category = item.category;
            if (item.unit) dup.unit = item.unit;
            if (item.quantity !== '') dup.quantity = Number(item.quantity) || 0;
            if (item.price !== '') dup.price = Number(item.price) || 0;
            if (item.purchasePrice !== '') dup.purchasePrice = Number(item.purchasePrice) || 0;
            if (item.minStock !== '') dup.minStock = Number(item.minStock) || 0;
            if (item.supplier) dup.supplier = item.supplier;
            if (item.description) dup.description = item.description;
            if (item.barcode) dup.barcode = item.barcode;
            if (item.code) dup.code = item.code;
            setProducts(existing);
          } else {
            existing.push({
              id: uid(), barcode: item.barcode || '', code: item.code || '',
              name: item.name || '', category: item.category || '',
              unit: item.unit || 'шт', quantity: Number(item.quantity) || 0,
          ...
          if (!item.phone) { errors++; return; }
          var customers = getCustomers();
          var dupC = customers.find(function (c) { return (c.phone || '').replace(/\D/g, '') === item.phone.replace(/\D/g, ''); });
          if (!dupC) {
            customers.push({ id: uid(), name: item.name || '', phone: item.phone, spent: 0, bonusBalance: 0 });
            setCustomers(customers);
          }
          success++;
        } else if (state.section === 'expenses') {
          if (!item.category || !item.amount) { errors++; return; }
          var expenses = getExpenses();
          expenses.push({ id: uid(), category: item.category, amount: Number(item.amount), date: new Date().toISOString(), description: item.description || '', userName: currentUser ? currentUser.name : '', status: 'active' });
    ...
    refreshAll();
    _importState = { section: null, rawData: null, rows: null };
  }

  // ─── СТАРЫЕ ФУНКЦИИ (совместимость) ───
  window.exportToExcel = function (headers, rows, filename) {
    if (typeof XLSX === 'undefined') { toast('Excel библиотека не загружена', 'err'); return; }
    var data = [headers].concat(rows);
  ...
  window.exportAoAToExcel = function (rows, filename, sheetName, widths) {
    if (typeof XLSX === 'undefined') { toast('Excel библиотека не загружена', 'err'); return; }
  ...
  window.closeModal = function (id) {
    ...
    // When closing template list modal, reopen the parent document modal if any
  ...
  window.toggleSidebar = function () {
    ...
    if (btn) btn.innerHTML = collapsed ? '<i data-lucide="panel-left"></i>' : '<i data-lucide="panel-left-close"></i>';
    if (typeof lucide !== 'undefined') lucide.createIcons();
    if (window.ApDb) window.ApDb.set('sidebarCollapsed', collapsed);
  };

  window.toggleDashLowStock = function () {
    ...
    if (window.ApDb) window.ApDb.set('dashLowStockHidden', !hidden);
  };
  ...
  function refreshAll() {
    ...
    else if (document.getElementById('page-myshift') && document.getElementById('page-myshift').classList.contains('active')) renderMyShiftPage();
  }
  ...
  window.saveCustomer = function () {
    ...
    if (editId && !checkPermission('editCustomer')) { toast('Нет прав на редактирование клиентов', 'err'); return; }
    if (!editId && !checkPermission('addCustomer')) { toast('Нет прав на добавление клиентов', 'err'); return; }
  ...
  window.deleteCustomer = function(id) {
    if (!checkPermission('deleteCustomer')) { toast('Нет прав на удаление клиентов', 'err'); return; }
    var c = getCustomers().find(function(x) { return x.id === id; });
    confirmAction('Удалить клиента', 'Клиент «' + (c ? c.name : '') + '» будет удалён безвозвратно.', function() {
  ...
  window.showApp = function showApp() {
    ...
    try {
      ...
      currentUser.pin = getUserPin(currentUser.id) || null;
      ...
      // Auto-refresh visible data every 15 seconds
      if (!window._autoRefreshInterval) {
        window._autoRefreshInterval = setInterval(function () {
          ...
          if (activePage) {
            ...
            else if (id === 'page-products') renderProducts();
            else if (id === 'page-statistics') renderStatistics();
            else if (id === 'page-audits') { renderWriteOffsTable(); renderAuditsArchive(); }
          }
        }, 15000);
      }
  ...
  function hasGroupPermission(pageName) {
    ...
    if (!group) return true; // pages without a group mapping are always visible
    var keys = PERMISSION_GROUPS[group];
    if (!keys) return true;
    var perms = getUserPermissions(currentUser.id);
    return keys.some(function(k) { return perms[k] === true; });
  }
  function applyRoleUI() {
    ...
    document.querySelectorAll('[data-perm]').forEach(function (el) {
      ...
      if (!checkPermission(perm)) el.classList.add('hidden-role');
      else el.classList.remove('hidden-role');
    });
    // Group-level visibility: hide entire sidebar buttons if no permission in group
    document.querySelectorAll('.nav-btn[data-page]').forEach(function(el) {
      var page = el.getAttribute('data-page');
      if (!hasGroupPermission(page)) el.classList.add('hidden-role');
      else el.classList.remove('hidden-role');
    });
  }
  ...
  window.goPage = function (name) {
    ...
    if (name === 'sales') { fillSaleProducts(); clearSaleSelection(); renderSalesToday(); renderPosSideHistory(); updateSaleShiftBanner(); focusSaleSearch(); renderPosCatBrowser(); posRefreshFavorites(); updatePosClock(); if (typeof posCashierName === 'function') posCashierName(); }
    if (name === 'expenses') renderExpenses();
    if (name === 'statistics') renderStatistics();
    if (name === 'promotions') renderPromotionsPage();
    ...
    if (name === 'debts') { migrateDebtData(); renderDebts(); }
    if (name === 'deferred') { migrateDeferredData(); renderDeferred(); }
  ...
  function getStats() {
    ...
    sales.forEach(function (s) {
      ...
      if (s.payment === 'debt') { /* не в кассу */ }
      else if (s.payment === 'cash') cash += s.total;
  ...
  function getPeriodDateRange(period) {
    ...
    else if (period === 'week') { var d = now.getDay(); var diff = d === 0 ? 6 : d - 1; start = new Date(now.getFullYear(), now.getMonth(), now.getDate() - diff); }
  ...
  function renderDashboard() {
    ...
    const expenses = filterByPeriod(getExpenses(), 'date', period).filter(function(e){ return e.status !== 'cancelled'; });
    ...
    var productSalesMap = {}; // {productId or name: {qty, total, name, category}}
    var categorySalesMap = {}; // {categoryName: {qty, total}}
    sales.forEach(function (s) {
      if (Number(s.total) > 0) totalRevenue += Number(s.total);
      if (s.payment === 'cash') totalCash += Number(s.total) || 0;
      else if (s.payment === 'kaspi') totalKaspi += Number(s.total) || 0;
      else if (s.payment === 'transfer') totalTransfer += Number(s.total) || 0;
      totalCOGS += (Number(s.purchasePrice) || 0) * (Number(s.quantity) || 0);
      salesQty += Number(s.quantity) || 0;

      var pName = s.productName || '—';
      if (!productSalesMap[pName]) productSalesMap[pName] = { qty: 0, total: 0, name: pName, category: '' };
      productSalesMap[pName].qty += Number(s.quantity) || 0;
      productSalesMap[pName].total += Number(s.total) || 0;
      if (s.productId) {
        var prod = products.find(function(p){ return p.id === s.productId; });
        if (prod) {
          productSalesMap[pName].category = prod.category || '';
          var catName = '';
          var catObj = getCategories().find(function(c){ return c.id === prod.category; });
          catName = catObj ? catObj.name : 'Без категории';
          if (!categorySalesMap[catName]) categorySalesMap[catName] = { qty: 0, total: 0 };
    ...
    var activeDebts = allDebts.filter(function(d) { return d.status === 'open' && d.amount > 0; });
    var totalDebtAmount = activeDebts.reduce(function(sum, d) { return sum + (Number(d.amount) || 0); }, 0);
    ...
    var docTypeLabels = { invoice: 'Счета', z2: 'Накладные З-2', invoice_sf: 'Счёт-фактуры', deferred: 'Отложенные' };

    // Docs total sum (all docs with positive total)
    var docsTotalSum = docs.reduce(function(sum, d) { return sum + (Number(d.total) || 0); }, 0);

    // ─── DASHBOARD CARDS ───
    var salesCount = sales.length;
    document.getElementById('dash-cards').innerHTML =
      card('📦 Товаров', products.length, '') +
      card('🛒 Продаж', salesCount + ' (' + fmt(totalRevenue) + ' ₸)', 'ok') +
      card('💵 Наличные', fmt(totalCash), 'cash') +
      card('📱 Kaspi QR', fmt(totalKaspi), 'kaspi') +
      card('🏦 Банк', fmt(totalTransfer), 'bank') +
      card('💰 Выручка', fmt(totalRevenue), 'ok') +
      card('📉 Расходы', fmt(totalExpenses), 'warn') +
      card('📋 Долги', fmt(totalDebtAmount), 'err');

    // ─── TOP PRODUCTS ───
    var topProducts = Object.keys(productSalesMap).map(function(k) { return productSalesMap[k]; });
    ...
    if (topProducts.length) {
      ...
      topEl.innerHTML = topProducts.map(function(p) {
        ...
        return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;font-size:13px">' +
          '<div style="flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + esc(p.name) + '</div>' +
          '<div style="width:100px;background:var(--bg3);border-radius:4px;height:16px;position:relative;overflow:hidden">' +
          '<div style="position:absolute;top:0;left:0;height:100%;width:' + pct + '%;background:var(--accent);border-radius:4px;opacity:0.6"></div>' +
          '</div>' +
          '<div style="min-width:40px;text-align:right;font-weight:600">' + p.qty + '</div>' +
          '</div>';
      }).join('');
    } else {
    ...
    var catEntries = Object.keys(categorySalesMap).map(function(k) { return { name: k, qty: categorySalesMap[k].qty, total: categorySalesMap[k].total }; });
    ...
    reportEl.innerHTML = '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px">' +
      finCard('🛒 Сумма продаж', fmt(totalRevenue) + ' ₸', 'ok') +
      finCard('📄 Сумма по документам', fmt(docsTotalSum) + ' ₸', '') +
      finCard('📉 Сумма расходов', fmt(totalExpenses) + ' ₸', 'warn') +
      finCard('📋 Активные долги', fmt(totalDebtAmount) + ' ₸', 'err') +
      finCard('💎 Чистый баланс', fmt(Math.abs(netBalance)) + ' ₸', netBalance >= 0 ? 'ok' : 'err') +
      '</div>' +
      '<div style="margin-top:12px;font-size:13px;color:var(--text-muted)">'
      + Object.keys(docTypeLabels).map(function(t) {
        var cnt = docCounts[t] || 0;
        return docCounts[t] ? '<span style="margin-right:16px">' + docTypeLabels[t] + ': <strong>' + cnt + '</strong></span>' : '';
      }).join('') + (docs.length ? '<span>Всего документов: <strong>' + docs.length + '</strong></span>' : '')
      + '</div>';
    ...
    // Today's sales (moved from sales page)
    renderSalesToday();
  }
  ...
  function syncProductFilterUI() {
    ...
    if (supSel) {
      ...
      var suppliers = prods.filter(function(p){ return p.supplier; }).map(function(p){ return p.supplier; });
      suppliers = suppliers.filter(function(v,i,a){ return a.indexOf(v)===i; }).sort(function(a,b){ return a.localeCompare(b,'ru'); });
      supSel.innerHTML = '<option value="">Все поставщики</option>' +
        suppliers.map(function(s){ return '<option value="'+esc(s)+'">'+esc(s)+'</option>'; }).join('');
      if (curSup && suppliers.indexOf(curSup)>=0) supSel.value = curSup;
      else supSel.value = '';
    }
  ...
  window.renderProducts = function () {
    ...
    // Summary cards: total cost price & total selling price of all products
    ...
    if (summaryEl) {
      summaryEl.innerHTML =
        '<div class="card"><div class="card-label">Общий остаток (шт)</div><div class="card-value">' + totalQty + '</div></div>' +
        '<div class="card warn"><div class="card-label">Общая себестоимость</div><div class="card-value">' + fmt(totalCost) + ' ₸</div></div>' +
        '<div class="card ok"><div class="card-label">Общая цена продажи</div><div class="card-value">' + fmt(totalRetail) + ' ₸</div></div>' +
        (totalCost > 0 ? '<div class="card"><div class="card-label">Наценка (суммарная)</div><div class="card-value">' + fmt(totalRetail - totalCost) + ' ₸</div></div>' : '');
    }

    if (search) {
      var smartResults = smartMatchProducts(search);
      var smartIds = {};
      smartResults.forEach(function(p) { smartIds[p.id] = true; });
      list = list.filter(function (p) { return smartIds[p.id]; });
    }
    ...
    // New filters: SKU, supplier, date range
    var skuSearch = (document.getElementById('filter-sku').value || '').toLowerCase().trim();
    if (skuSearch) list = list.filter(function (p) { return (p.sku || '').toLowerCase().includes(skuSearch); });
    var supFilter = (document.getElementById('filter-supplier').value || '').trim();
    if (supFilter) list = list.filter(function (p) { return (p.supplier || '') === supFilter; });
    ...
    const admin = isAdmin();
    const cols = ['<input type="checkbox" id="bulk-select-all" onchange="toggleBulkSelectAll(this)" title="Выбрать всё">', '★', 'Категория', 'Код', 'Артикул (SKU)', 'Штрих-код', 'Название', 'Поставщик'];
    ...
    const rows = list.map(function (p) {
      ...
      if (p.compatibility) nameCell += '<br><span style="font-size:11px;color:var(--primary);background:var(--primary-light);padding:2px 6px;border-radius:4px;">🚗 ' + p.compatibility + '</span>';
      
      var starBtn = p.favorite
        ? '<span class="star-fav" onclick="toggleFavoriteFromTable(\'' + p.id + '\', event)" title="Убрать из избранного">★</span>'
        : '<span class="star-no-fav" onclick="toggleFavoriteFromTable(\'' + p.id + '\', event)" title="Добавить в избранное">☆</span>';
      const row = [
      ...
      var actions = '<div class="actions">';
      if (canEdit) actions += '<button class="icon-btn" onclick="editProduct(\'' + p.id + '\')">✏️</button>';
      if (canDelete) actions += '<button class="icon-btn del" onclick="deleteProduct(\'' + p.id + '\')">🗑</button>';
      actions += '</div>';
      row.push(actions);
      return row;
    });
    ...
    document.querySelectorAll('.bulk-item').forEach(function(el) {
      if (_bulkSelected.has(el.dataset.id)) el.checked = true;
    });
    updateBulkBar();
  };
  ...
  window.exportProductsToCSV = function () {
    ...
    link.download = 'SANAQ_Товары_' + todayStr() + '.csv'; link.click();
    URL.revokeObjectURL(link.href);
    toast('CSV файл скачан', 'ok');
  };

  // ─── MARKUP SYNC ───
  window.syncFromPurchase = function() {
    var purchase = parseFloat(document.getElementById('product-purchase').value) || 0;
  ...
  window.syncFromMarkup = function() {
    var purchase = parseFloat(document.getElementById('product-purchase').value) || 0;
    var markup = parseFloat(document.getElementById('product-markup').value) || 0;
    if (purchase > 0 && markup >= 0) {
      var price = purchase * (1 + markup / 100);
  ...
  window.updateMarkup = function() {
    var purchase = parseFloat(document.getElementById('product-purchase').value) || 0;
    var price = parseFloat(document.getElementById('product-price').value) || 0;
    var markup = purchase > 0 ? ((price - purchase) / purchase) * 100 : 0;
    document.getElementById('product-markup').value = markup.toFixed(2);
  };
  ...
  // ─── CATEGORIES ───
  window.renderCategoriesPage = function() {
    renderCategoriesList('categories-page-list');
    var pageWrap = document.getElementById('new-category-wrapper-page');
    if (pageWrap) pageWrap.style.display = checkPermission('addCategory') ? 'flex' : 'none';
    var pageHint = document.getElementById('page-cat-drag-hint');
    if (pageHint) pageHint.style.display = checkPermission('editCategory') ? 'block' : 'none';
  };

  window.pageAddCategory = function() {
    if (!checkPermission('addCategory')) { toast('Нет прав на добавление разделов', 'err'); return; }
    ...
    list.push({ id: uid(), name: name, color: '#2563EB', icon: '📦', order: list.length });
    setCategories(list);
    nameInput.value = '';
    renderCategoriesList('categories-page-list');
  ...
  window.openCategoriesModal = function() {
    ...
    if (wrap) wrap.style.display = checkPermission('addCategory') ? 'flex' : 'none';
    if (hint) hint.style.display = checkPermission('editCategory') ? 'block' : 'none';
  ...
  function renderCategoriesList(containerId) {
    ...
    if (!list.length) {
      ...
      list.forEach(function(c, i) {
        ...
        var icon = c.icon || '📦';
        html += '<div class="cat-card" draggable="true" data-index="' + i + '" data-id="' + c.id + '" ondragstart="onCatDragStart(event)" ondragover="onCatDragOver(event)" ondrop="onCatDrop(event,\'' + (containerId || '') + '\')" ondragend="onCatDragEnd(event)">' +
          '<div class="cat-card-inner">' +
          '<span class="cat-drag-handle" title="Перетащить">⠿</span>' +
          '<span class="cat-color-dot" style="background:' + color + '"></span>' +
          '<span class="cat-icon-display">' + icon + '</span>' +
          '<span style="flex:1;font-weight:600;color:var(--text)">' + esc(c.name) + '</span>' +
          (canEditCat ? '<button class="icon-btn" onclick="editCategory(\'' + c.id + '\',\'' + (containerId || '') + '\')" style="font-size:13px" title="Редактировать">✏️</button>' : '') +
          (canDeleteCat ? '<button class="icon-btn del" onclick="deleteCategory(\'' + c.id + '\',\'' + (containerId || '') + '\')" style="font-size:14px" title="Удалить">✕</button>' : '') +
          '</div></div>';
      });
      html += '</div>';
    }
  ...
  window.deleteCategory = function(id, containerId) {
    if (!checkPermission('deleteCategory')) { toast('Нет прав на удаление разделов', 'err'); return; }
  ...
  window.editCategory = function(id, containerId) {
    var list = getCategories();
    ...
    inner.innerHTML =
      '<div class="cat-edit-inline">' +
        '<input type="text" id="cat-edit-name-' + id + '" value="' + esc(cat.name) + '" placeholder="Название">' +
        '<input type="color" id="cat-edit-color-' + id + '" value="' + color + '">' +
        '<span class="cat-icon-display" id="cat-edit-icon-display-' + id + '">' + icon + '</span>' +
        ...
        '</div>' +
        '<button class="btn btn-primary btn-sm" onclick="saveCategory(\'' + id + '\',\'' + cid + '\')">Сохранить</button>' +
        '<button class="btn btn-secondary btn-sm" onclick="renderCategoriesList(\'' + cid + '\')">Отмена</button>' +
      '</div>';
  };

  window.selectCatIcon = function(id, ic) {
  ...
  window.saveCategory = function(id, containerId) {
    if (!checkPermission('editCategory')) { toast('Нет прав на редактирование разделов', 'err'); return; }
    ...
    if (!nameEl || !colorEl || !displayEl) return;
  ...
  window.onCatDragStart = function(e) {
  ...
  window.onCatDrop = function(e, containerId) {
    e.preventDefault();
    var target = e.target.closest('.cat-card');
    if (!target || !_catDragSrc || target === _catDragSrc) return;
    var list = getCategories();
  ...
  window.saveProduct = function () {
    ...
    if (editId && !checkPermission('editProducts')) { toast('Нет прав на редактирование товаров', 'err'); return; }
    if (!editId && !checkPermission('addProducts')) { toast('Нет прав на добавление товаров', 'err'); return; }
    ...
    if (sku) {
      const dupSku = list.find(function (p) { return p.sku === sku && p.id !== editId; });
      if (dupSku) { toast('Артикул (SKU) «' + sku + '» уже используется', 'err'); return; }
    }

    var data = { code: code, barcode: barcode || '', name: name, category: category, sku: sku, supplier: supplier, quantity: qty, purchasePrice: purchasePrice, markup: markup, price: price, minStock: minStock, info: info, compatibility: compatibility };
    ...
    if (editId) {
      ...
      list.push(Object.assign({ id: uid(), favorite: false }, data));
    ...
    renderDashboard();
    refreshAll();
    // After save, prime the next refresh so old Supabase data doesn't linger in cache
    _schedulePostSaveSync();
  };
  var _postSaveSyncTimer = null;
  function _schedulePostSaveSync() {
    if (_postSaveSyncTimer) clearTimeout(_postSaveSyncTimer);
  ...
  window.deleteProduct = function (id) {
    if (!checkPermission('deleteProducts')) { toast('Нет прав на удаление товаров', 'err'); return; }
    const p = getProducts().find(function (x) { return x.id === id; });
  ...
  window.setPayment = function (btn) {
    document.querySelectorAll('.pay-btn, .pos-pay-btn, .pos-pay-strip button').forEach(function (b) { b.classList.remove('active'); });
  ...
  // ─── Universal Product ───
  window.addUniversalProduct = function () {
    var modal = document.getElementById('modal-universal-product');
    if (!modal) { toast('Модальное окно не найдено', 'err'); return; }
  ...
  window.confirmUniversalProduct = function () {
    ...
    if (!name) { toast('Введите название товара', 'err'); return; }
    if (price <= 0) { toast('Введите цену товара', 'err'); return; }
    if (qty < 1) qty = 1;
    saleCart.push({
      id: 'uni_' + uid(),
      code: '', barcode: '',
      name: name, price: price, maxQty: 999999,
      qty: qty, isUniversal: true
    });
  ...
  // ─── New POS Action Buttons ───
  var _selectedCartItemId = null;
  window.selectCartItem = function (id) {
    _selectedCartItemId = id;
    document.querySelectorAll('#sale-cart-body tr').forEach(function (r) { r.classList.remove('selected'); });
    var row = document.querySelector('#sale-cart-body tr[data-id="' + id + '"]');
    if (row) row.classList.add('selected');
  };
  window.getSelectedCartItem = function () {
    if (!_selectedCartItemId) return null;
    return saleCart.find(function (c) { return c.id === _selectedCartItemId; }) || null;
  };
  window.adjustSelectedQty = function (delta) {
    var item = getSelectedCartItem();
    if (!item) { toast('Выберите товар в таблице', 'warn'); return; }
    updateCartQty(item.id, delta);
  };
  window.promptCartQty = function () {
    var item = getSelectedCartItem();
    if (!item) { toast('Выберите товар в таблице', 'warn'); return; }
  ...
  window.removeSelectedItem = function () {
    var item = getSelectedCartItem();
    if (!item) { toast('Выберите товар в таблице', 'warn'); return; }
    if (!confirm('Удалить "' + item.name + '" из чека?')) return;
    removeFromCart(item.id);
  };
  window.checkSelectedPrice = function () {
    var item = getSelectedCartItem();
    if (!item) { toast('Выберите товар в таблице', 'warn'); return; }
    toast('💰 ' + item.name + ': ' + fmt(item.price) + ' ₸ × ' + item.qty + ' = ' + fmt(item.price * item.qty) + ' ₸', 'ok');
  };
  ...
  // ─── POS Clock & Cashier ───
  window.updatePosClock = function () {
  ...
  setInterval(function () { var p = document.getElementById('page-sales'); if (p && !p.classList.contains('hidden')) updatePosClock(); }, 10000);
  window.posCashierName = function () {
    var el = document.getElementById('pos-cashier-display');
    if (el && currentUser) el.textContent = currentUser.name || currentUser.username || 'Кассир';
  };

  // ─── POS View Tabs ───
  window.showPosView = function (view) {
    document.querySelectorAll('.pos-top-tab').forEach(function (b) { b.classList.toggle('active', b.dataset.posView === view); });
    if (view === 'sales') { /* default view */ }
    if (view === 'return') openReturnSelector();
  };

  // ─── POS Category Browser ───
  var _posBrowserState = { cat: '' };

  window.renderPosCatBrowser = function () {
  ...
  window.filterCategory = function (catId) {
    document.querySelectorAll('.pos-cat-pill').forEach(function (b) { b.classList.remove('active'); });
    if (catId) {
  ...
  window.posSelectCat = function (catId) {
    _posBrowserState.cat = catId;
    filterCategory(catId);
  };

  window.renderPosBrowserProducts = function (products, catName) {
    // No-op in new layout; kept for compatibility
  };

  window.toggleFavoriteFromTable = function (productId, e) {
  ...
  window.toggleFavPos = function (productId, e) {
    if (e) e.stopPropagation();
    var products = getProducts();
    var p = products.find(function (x) { return x.id === productId; });
    if (!p) return;
  ...
  window.posRefreshFavorites = function () {
    ...
    if (!favs.length) {
      ...
      grid.innerHTML = favs.map(function(p) {
        return '<div class="pos-quick-item" onclick="addToCart(\'' + p.id + '\')">' +
          esc(p.name) + '<div class="qp-price">' + fmt(p.price) + '</div></div>';
      }).join('');
    }
  };
  ...
  window.openPaymentModal = function () {
    ...
    if (total <= 0) { toast('Сумма чека 0', 'err'); return; }
    var payBtns = document.querySelectorAll('#modal-pos-payment .pos-pay-strip button');
    if (payBtns.length) { setPayment(payBtns[0]); }
    openModal('modal-pos-payment');
    setTimeout(function () { var cg = document.getElementById('cash-given'); if (cg) cg.focus(); }, 200);
  };
  var _posCatModalState = { mode: 'categories', catId: '', catName: '' };

  window.renderPosCatList = function () {
    try {
      ...
      list.innerHTML = html;
    } catch (e) { toast('Ошибка при возврате к категориям', 'err'); }
  };

  window.renderPosCatProducts = function (catId) {
    try {
      var list = document.getElementById('pos-cat-modal-list');
      if (!list) return;
      ...
      var products = (getProducts() || []).filter(function(p) { return p.category === catId && p.quantity > 0; });
      if (!products.length) {
        list.style.cssText = '';
      ...
      list.style.cssText = '';
      list.innerHTML = '<div style="margin-bottom:16px;display:flex;align-items:center;gap:12px;padding:4px"><button class="pos-cat-pill" onclick="window.renderPosCatList()">← Назад</button> <span style="font-size:18px;font-weight:700;color:#374151">' + esc(_posCatModalState.catName) + '</span></div>' +
        '<div class="pos-modal-grid">' +
        products.sort(function (a, b) { return (b.quantity || 0) - (a.quantity || 0); }).map(function (p) {
          return '<div class="pos-quick-item" onclick="closeModal(\'modal-pos-categories\');addToCart(\'' + p.id + '\')">' +
            '<div class="qp-price">' + fmt(p.price) + '</div>' +
            '<div style="font-size:14px;color:#6b7280;margin-top:4px">' + esc(p.name) + '</div></div>';
        }).join('') +
        '</div>';
    } catch (e) { toast('Ошибка загрузки товаров', 'err'); }
  };

  window.openPosCategories = function () {
    try {
      window.renderPosCatList();
      openModal('modal-pos-categories');
    } catch (e) { toast('Ошибка загрузки категорий', 'err'); }
  };
  window.filterCategoryFromModal = function (catId) {
    if (catId) {
      _posBrowserState.cat = catId;
  ...
  window.openPosFavorites = function () {
    var grid = document.getElementById('pos-fav-modal-grid');
    if (!grid) return;
    var all = getProducts();
    var favs = all.filter(function (p) { return p.favorite && p.quantity > 0; });
    if (!favs.length) {
      ...
      grid.innerHTML = favs.sort(function (a, b) { return (b.quantity || 0) - (a.quantity || 0); }).map(function (p) {
        return '<div class="pos-quick-item" onclick="closeModal(\'modal-pos-favorites\');addToCart(\'' + p.id + '\')">' +
          '<div class="qp-price">' + fmt(p.price) + '</div>' +
          '<div style="font-size:14px;color:#6b7280;margin-top:4px">' + esc(p.name) + '</div></div>';
      }).join('');
    }
  ...
  window.renderPosSideHistory = function () {
    ...
    if (!shift) { el.innerHTML = '<div class="pos-empty-msg">Смена не открыта</div>'; return; }
    var allSales = getSales().filter(function (s) { return isSaleActive(s); });
    var shiftSales = allSales.filter(function (s) { return s.shiftId === shift.id; });
    var receipts = groupSalesIntoReceipts(shiftSales);
    if (!receipts.length) { el.innerHTML = '<div class="pos-empty-msg">Нет продаж в этой смене</div>'; return; }
    var html = '';
    receipts.forEach(function (r) {
      var payLabel = PAY_LABELS[r.payment] || r.payment || '';
      var payBadge = '';
      if (payLabel === 'Наличные') payBadge = '<span class="side-pay-badge" style="background:#ecfdf5;color:#059669">нал</span>';
      else if (payLabel === 'Kaspi QR') payBadge = '<span class="side-pay-badge" style="background:#f0fdf4;color:#16a34a">kaspi</span>';
      else if (payLabel === 'Банк') payBadge = '<span class="side-pay-badge" style="background:#eff6ff;color:#2563eb">банк</span>';
      else if (payLabel === 'Смешанный') payBadge = '<span class="side-pay-badge" style="background:#fefce8;color:#ca8a04">смеш</span>';
      else if (payLabel === 'В долг') payBadge = '<span class="side-pay-badge" style="background:#fef2f2;color:#dc2626">долг</span>';
      else if (payLabel) payBadge = '<span class="side-pay-badge" style="background:#f3f4f6;color:#6b7280">' + esc(payLabel) + '</span>';
      var timeStr = r.date ? fmtDate(r.date) : '—';
      var itemsCount = r.items.reduce(function (s, it) { return s + (Number(it.quantity) || 0); }, 0);
      html += '<div class="pos-side-sale" onclick="openReceipt(\'' + r.id + '\')">' +
        '<span class="pos-side-sale-num">№' + r.id.slice(-6) + '</span>' +
        '<div class="pos-side-sale-info"><span class="side-sale-time">' + esc(timeStr) + '</span><span>x' + itemsCount + '</span></div>' +
        payBadge +
        '<span class="pos-side-sale-amount">' + fmt(r.total) + '</span>' +
      '</div>';
    });
    el.innerHTML = html;
  };

  // Global search in POS — searches across all products
  window.onSaleSearch = function () {
    ...
    if (!box) return;
    if (!term.trim()) { box.classList.add('hidden'); box.innerHTML = ''; return; }
    var matches = smartMatchProducts(term);
    var catFilter = _posBrowserState.cat || '';
    if (catFilter) matches = matches.filter(function (p) { return p.category === catFilter; });
    if (!matches.length) {
  ...
  window.clearSaleSelection = function(noConfirm) {
    ...
    var mt = document.getElementById('mixed-transfer'); if (mt) mt.value = '0';
    ...
    var dn = document.getElementById('debt-pay-name');  if (dn) dn.value = '';
    var dp = document.getElementById('debt-pay-phone'); if (dp) dp.value = '';
    var dd = document.getElementById('debt-pay-due');   if (dd) dd.value = '';
    var dno = document.getElementById('debt-pay-note'); if (dno) dno.value = '';
    togglePaymentSection('cash-change-wrap', currentPayment === 'cash');
    togglePaymentSection('mixed-payment-wrap', currentPayment === 'mixed');
    togglePaymentSection('debt-payment-wrap', currentPayment === 'debt');
    renderSaleCart();
    if (saleCart.length === 0) toast('Корзина очищена');
  }

  // ─── Admin PIN Verification ───
  var _adminPinCallback = null;
  window.requireAdminPin = function(operationName, callback) {
    var admins = getUsers().filter(function(u) { return u.role === 'admin'; });
    if (!admins.length) { toast('Нет администратора в системе', 'err'); return; }
    ...
    _adminPinCallback = callback;
    openModal('modal-admin-pin');
    setTimeout(function() { document.getElementById('admin-pin-input').focus(); }, 200);
  };
  window.verifyAdminPin = function() {
    var pin = document.getElementById('admin-pin-input').value.trim();
    if (!pin) { document.getElementById('admin-pin-error').style.display = 'block'; return; }
    var admins = getUsers().filter(function(u) { return u.role === 'admin'; });
    var ok = false;
    admins.forEach(function(a) {
      if (getUserPin(a.id) === pin) ok = true;
    });
    if (ok) {
  ...
  // Close modal-admin-pin on overlay click
  document.addEventListener('click', function(e) {
    var m = document.getElementById('modal-admin-pin');
    if (m && m.classList.contains('show') && e.target === m) closeModal('modal-admin-pin');
  });

  // ─── Manual Discount (Item) ───
  var _discountItemId = null;
  window.openItemDiscount = function(cartItemId) {
    _discountItemId = cartItemId;
    var item = saleCart.find(function(c) { return c.id === cartItemId; });
    if (!item) { toast('Товар не найден', 'err'); return; }
  ...
  window.toggleItemDiscountValue = function() {
    var type = document.getElementById('item-discount-type').value;
    var input = document.getElementById('item-discount-value');
    if (type === 'percent') { input.max = 100; input.placeholder = 'Например: 10'; }
    else { input.max = 999999; input.placeholder = 'Например: 500'; }
  };
  window.applyItemDiscount = function() {
    ...
    if (!value || value <= 0) { toast('Введите значение скидки', 'err'); return; }
    var item = saleCart.find(function(c) { return c.id === _discountItemId; });
    if (!item) { toast('Товар не найден', 'err'); return; }
    var maxAllowed = isAdmin() ? 25 : getUserMaxDiscount(currentUser.id);
    var effectiveDisc = type === 'percent' ? value : (value / item.price * 100);
    if (effectiveDisc > 25) { toast('Максимальная скидка в системе — 25%', 'err'); return; }
  ...
  // ─── Manual Discount (Cart) ───
  window.openCartDiscount = function() {
    if (!saleCart.length) { toast('Корзина пуста', 'err'); return; }
    ...
    function doOpen() {
      ...
      setTimeout(function() { document.getElementById('cart-discount-value').focus(); }, 200);
    }
  ...
  window.applyCartDiscount = function() {
    ...
    if (!value || value <= 0) { toast('Введите значение скидки', 'err'); return; }
    var maxAllowed = isAdmin() ? 25 : getUserMaxDiscount(currentUser.id);
    if (type === 'percent' && value > 25) { toast('Максимальная скидка в системе — 25%', 'err'); return; }
  ...
  function applyCartDiscountFinal(type, value, reason) {
    _cartDiscountInfo = { type: type, value: value, reason: reason };
    // Remove per-item discounts (now applied at cart level)
  ...
  window.removeCartDiscount = function() {
    if (!_cartDiscountInfo) return;
    _cartDiscountInfo = null;
    renderSaleCart();
  ...
  window.startCameraScanner = function (targetInputId) {
    ...
    if ('BarcodeDetector' in window) {
      // Real-time scanning via native BarcodeDetector
      scannerActive = true;
      ...
      navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment', width: { ideal: 1280 }, height: { ideal: 720 } } })
        .then(function (stream) {
          if (!scannerActive) { stopTracks(stream); return; }
          ...
          video.play().then(function () {
            var detector;
            try { detector = new BarcodeDetector({ formats: ['ean_13', 'ean_8', 'code_128', 'code_39', 'code_93', 'upc_a', 'upc_e', 'codabar', 'itf'] }); }
            catch (e) { try { detector = new BarcodeDetector(); } catch (e2) { stopCameraScanner(); toast('Сканер недоступен', 'err'); return; } }
            ...
            setTimeout(function() {
              ...
              if (btn && scannerStream) {
                ...
                if (track && track.getCapabilities) {
                  ...
                  if (caps && caps.torch) btn.style.display = 'inline-block';
                }
              }
            }, 500);
            var lastScan = 0;
            _scanLoopFn = function() {
              if (!scannerActive) return;
              var now = Date.now();
              if (now - lastScan < 150) { scannerAnimFrame = requestAnimationFrame(_scanLoopFn); return; }
              lastScan = now;
              detector.detect(video).then(function (barcodes) {
                if (!scannerActive) return;
                if (barcodes.length > 0) {
                  onBarcodeDetected(barcodes[0].rawValue);
                  // If scanner is still active after onBarcodeDetected, continue scanning
                  if (scannerActive) { scannerAnimFrame = requestAnimationFrame(_scanLoopFn); }
                  return;
                }
                scannerAnimFrame = requestAnimationFrame(_scanLoopFn);
              }).catch(function () {
                if (scannerActive) scannerAnimFrame = requestAnimationFrame(_scanLoopFn);
              });
            };
            scannerAnimFrame = requestAnimationFrame(_scanLoopFn);
          }).catch(function () { stopCameraScanner(); toast('Ошибка запуска видео', 'err'); });
        })
        .catch(function (err) {
          ...
          readerEl.innerHTML =
            ...
            '<div style="font-size:12px;margin-top:8px;color:#999">Разрешите доступ к камере в настройках браузера, или сделайте фото штрих-кода:</div>' +
            '<button class="btn btn-primary" style="margin-top:16px" onclick="scanFromPhoto()">📸 Сделать фото</button></div>';
        });
    } else {
      // No native API — use photo capture
      scanFromPhoto();
    }
  };
  ...
  window.scanFromPhoto = function () {
    ...
    input.onchange = function () {
      ...
      img.onload = function () {
        if ('BarcodeDetector' in window) {
          try {
            var detector = new BarcodeDetector({ formats: ['ean_13', 'ean_8', 'code_128', 'code_39', 'code_93', 'upc_a', 'upc_e'] });
            ...
            }).catch(function () { toast('Ошибка распознавания фото', 'err'); });
          } catch (e) {
            try {
            ...
            } catch (e2) { toast('Сканер недоступен', 'err'); }
          }
        } else {
          toast('Сканер не поддерживается на этом устройстве', 'err');
        }
      };
      img.onerror = function () { toast('Ошибка загрузки фото', 'err'); };
  ...
  function onBarcodeDetected(code) {
    ...
    var now = Date.now();
    // Debounce: prevent duplicate scans within 800ms
    if (now - _lastScanTime < 800) { return; }
    _lastScanTime = now;
    ...
    if (scannerTarget === 'sale-search') {
      ...
      if (exact) {
        ...
        if (!_scannerAutoClose) {
          // Keep scanning - just continue the animation frame
          return;
        }
      } else {
        ...
        if (matches.length === 1) {
          ...
          // Keep scanning
          if (scannerAnimFrame) { scannerAnimFrame = requestAnimationFrame(_scanLoopFn); }
          return;
        } else {
    ...
    // If auto-close setting is on, close scanner
    if (_scannerAutoClose) {
      scannerActive = false;
      cleanupScanner();
      try { closeModal('modal-scanner'); } catch(e) {}
    } else {
      // Continue scanning: restart the detection loop
      if (scannerAnimFrame) { scannerAnimFrame = requestAnimationFrame(_scanLoopFn); }
    }
  }
  ...
  window.changeCartQty = function(productId, delta) {
    var idx = saleCart.findIndex(function(c) { return c.id === productId; });
    if (idx === -1) return;
  ...
  window.renderSaleCart = function() {
    ...
    if (!saleCart.length) {
      ...
      if (totalEl) { totalEl.value = ''; totalEl.dataset.value = 0; }
      if (totalDisplay) totalDisplay.textContent = '0 ₸';
      if (changeDisplay) changeDisplay.textContent = '0 ₸';
      if (countEl) countEl.textContent = '0 товаров';
      if (btnComplete) btnComplete.disabled = true;
      if (btnDefer) btnDefer.disabled = true;
    ...
    if (btnDefer) btnDefer.disabled = false;
    ...
    tbody.innerHTML = saleCart.map(function(c, i) {
      ...
      var priceCell = promoDisc > 0
        ? '<span style="text-decoration:line-through;color:var(--text-muted)">' + fmt(c.price) + '</span> <span style="color:var(--err);font-weight:600">-' + fmt(promoDisc) + '</span>'
        : fmt(c.price);
      var discBtn = c._promoName
        ? '<span title="' + esc(c._promoName) + '" style="color:var(--ok);font-weight:600">🏷</span>'
        : '<span onclick="event.stopPropagation();openItemDiscount(\'' + c.id + '\')" style="cursor:pointer;color:var(--text-muted);font-size:12px;font-weight:500">🏷</span>';
      return '<tr onclick="selectCartItem(\'' + c.id + '\')" data-id="' + c.id + '">' +
        '<td>' + (i + 1) + '</td>' +
        ...
        '</div></td>' +
        '<td>' + discBtn + '</td>' +
        '<td>' + fmt((c.price - promoDisc) * c.qty) + '</td>' +
        '</tr>';
    }).join('');
    if (countEl) countEl.textContent = saleCart.length + ' товаров';

    let subTotal = saleCart.reduce(function(sum, c) { return sum + (c.price - (c._discount || 0)) * c.qty; }, 0);
    if (subTotalEl) subTotalEl.textContent = fmt(subTotal);

    // Calculate cart-level discount (auto-recalculated from percentage)
    var cartDiscAmt = 0;
    if (_cartDiscountInfo) {
    ...
    if (currentCustomer) {
      ...
      if (ci) {
        ...
        if (ciText) ciText.innerHTML = 'Клиент: <strong>' + (currentCustomer.name || '—') + '</strong> · Уровень: <strong>' + tier.name + '</strong> (-' + (tier.discount * 100) + '%) · Бонусы: <strong>' + fmt(currentCustomer.bonusBalance) + '</strong>';
      }
      var bw = document.getElementById('sale-bonus-wrap');
      ...
      if (bm) bm.textContent = '(макс ' + fmt(maxBonus) + ')';
    } else {
      var ci = document.getElementById('sale-customer-info');
      if (ci) ci.style.display = 'none';
      var bw = document.getElementById('sale-bonus-wrap');
    ...
    if (cartDiscAmt > 0 && cartDiscRow && cartDiscEl) {
      ...
      if (_cartDiscountInfo.reason) cartDiscEl.innerHTML += ' — ' + esc(_cartDiscountInfo.reason);
    } else if (cartDiscRow) {
    ...
    if (bsEl) {
      ...
      if (bonusSpend > maxBonus) { bonusSpend = maxBonus; bsEl.value = maxBonus; }
      if (bonusSpend < 0) { bonusSpend = 0; bsEl.value = 0; }
    }
    ...
    if (totalDisplay) totalDisplay.textContent = fmt(finalTotal) + ' ₸';
  ...
  window.calcChange = function () {
    ...
    if (given <= 0) {
      if (changeEl) { changeEl.value = ''; changeEl.style.color = '#059669'; }
      if (changeDisplay) changeDisplay.textContent = '0 ₸';
      if (btnComplete && saleCart.length > 0) btnComplete.disabled = false;
    } else if (change < 0) {
      if (changeEl) { changeEl.value = 'Не хватает ' + Math.abs(change).toLocaleString('ru-RU') + ' ₸'; changeEl.style.color = '#dc2626'; }
      if (changeDisplay) { changeDisplay.textContent = '-' + Math.abs(change).toLocaleString('ru-RU') + ' ₸'; changeDisplay.className = 'pos-bottom-total-value pos-red'; }
      if (btnComplete) btnComplete.disabled = true;
    } else {
      if (changeEl) { changeEl.value = change.toLocaleString('ru-RU') + ' ₸'; changeEl.style.color = '#059669'; }
      if (changeDisplay) { changeDisplay.textContent = change.toLocaleString('ru-RU') + ' ₸'; changeDisplay.className = 'pos-bottom-total-value pos-green'; }
      if (btnComplete && saleCart.length > 0) btnComplete.disabled = false;
    }
  };

  window.calcMixedRemainder = function (changed) {
    var totalEl  = document.getElementById('sale-total');
    var total    = parseFloat((totalEl.dataset.value || totalEl.value) || 0);
    ...
    if (Math.abs(rem) < 0.01) {
      ...
      if (changeDisplay) { changeDisplay.textContent = '0 ₸'; changeDisplay.className = 'pos-bottom-total-value pos-green'; }
      if (btnComplete && saleCart.length > 0) btnComplete.disabled = false;
    } else if (rem > 0) {
      ...
      if (changeDisplay) { changeDisplay.textContent = Math.abs(rem).toLocaleString('ru-RU') + ' ₸'; changeDisplay.className = 'pos-bottom-total-value pos-green'; }
      if (btnComplete) btnComplete.disabled = true;
    }
  };
  ...
  window.deferSale = function () {
    ...
    saleCart.forEach(function (c) {
      ...
      totalQty += c.qty;
      items.push({
        productId: c.id, productCode: c.code || '', productName: c.name,
        quantity: c.qty, unitPrice: c.price, total: itemTotal
      });
    });
    var defId = uid();
    deferred.push({
      ...
      total: total, quantity: totalQty,
      cashierName: currentUser.name, status: 'pending', note: '',
      date: new Date().toISOString(), completedAt: null
    });
    setDeferred(deferred);
    // Also create a document entry so it appears in Документы section
    var docs = getDocuments();
    docs.unshift({
      id: defId, type: 'deferred', docType: 'deferred',
      items: items.map(function (it) { return { productCode: it.productCode, productName: it.productName, quantity: it.quantity, unitPrice: it.unitPrice, total: it.total }; }),
      clientName: name, customerName: name,
      total: total,
      status: 'pending', date: new Date().toISOString(), documentDate: new Date().toISOString(),
      meta: {}
    });
    setDocuments(docs);
    setProducts(products);
  ...
  window.restoreDeferredSale = function (id) {
    if (!id) return;
    var deferred = getDeferred();
    var rec = deferred.find(function (d) { return d.id === id; });
    ...
    rec.items.forEach(function (item) {
      ...
      var existing = saleCart.find(function (c) { return c.id === item.productId && c.receiptId === 'deferred'; });
      if (existing) existing.qty += item.quantity;
      else saleCart.push({ id: item.productId, code: item.productCode, name: item.productName, qty: item.quantity, price: item.unitPrice, receiptId: 'deferred' });
    });
    setProducts(products);
    deferred = deferred.map(function (d) {
      if (d.id !== id) return d;
      return Object.assign({}, d, { status: 'in_cart', completedAt: new Date().toISOString() });
    });
    setDeferred(deferred);
    toast('Товары восстановлены в корзину', 'ok');
    // переключаемся на продажи без очистки корзины
    document.querySelectorAll('.nav-btn').forEach(function (b) { b.classList.toggle('active', b.dataset.page === 'sales'); });
    document.querySelectorAll('.page').forEach(function (p) { p.classList.toggle('active', p.id === 'page-sales'); });
    fillSaleProducts();
    updateSaleTotal();
    renderSalesToday();
    renderPosSideHistory();
  ...
  window.deleteDeferred = function (id) {
    if (!id) return;
    confirmAction('Удалить', 'Удалить отложенную продажу из списка?', function () {
      ...
      if (rec.status === 'in_cart') {
        rec.items.forEach(function (item) {
          saleCart = saleCart.filter(function (c) { return c.id !== item.productId || c.receiptId !== 'deferred'; });
        });
        updateSaleTotal();
      }
      if (window.ApDb && typeof window.ApDb.deleteDeferred === 'function') {
        ...
        setDeferred(deferred.filter(function (d) { return d.id !== id; }));
      }
      // Also remove/update the associated document
      var docs = getDocuments();
      var docIdx = docs.findIndex(function (d) { return d.id === id; });
      if (docIdx >= 0) {
        if (rec.status === 'completed') {
          // Keep document but mark cancelled
  ...
  window.payDeferred = function (id) {
    if (!id) return;
    var deferred = getDeferred();
    var rec = deferred.find(function (d) { return d.id === id; });
    if (!rec) { toast('Запись не найдена', 'err'); return; }
    if (rec.status === 'completed') { toast('Уже оплачено', 'err'); return; }
    showPaymentMethodModal(function (paymentMethod) {
      var shift = getOpenShiftForCashier(currentUser.id) || getOpenShiftForCashier(currentUser.username);
      if (!shift) { toast('Смена не открыта', 'err'); goPage('myshift'); return; }
      var sales = getSales();
      var products = getProducts();
      var receiptId = uid();
      ...
      rec.items.forEach(function (item) {
        var p = products.find(function (x) { return x.id === item.productId; });
        sales.push({
          id: uid(), receiptId: receiptId, shiftId: shift.id,
          productId: item.productId, productCode: item.productCode || '',
          productName: item.productName, quantity: item.quantity,
          unitPrice: item.unitPrice, purchasePrice: p ? (Number(p.purchasePrice) || 0) : 0,
          total: item.total, payment: paymentMethod,
          ...
          customerId: null, userName: currentUser.name, username: currentUser.username,
          date: dateStr, status: 'completed',
          debtorName: rec.customerName || '', debtPhone: rec.customerPhone || '', debtReturnDate: ''
        });
      });
      setSales(sales);
      // Update deferred item status
      deferred = deferred.map(function (d) {
        if (d.id !== id) return d;
        return Object.assign({}, d, { status: 'completed', paymentMethod: paymentMethod, completedAt: dateStr });
      });
      setDeferred(deferred);
      // Update document status
      var docs = getDocuments().map(function (d) {
        if (d.id !== id) return d;
        return Object.assign({}, d, { status: 'paid', paymentMethod: paymentMethod });
      });
      setDocuments(docs);
  ...
  window.cancelDeferredDoc = function (id) {
    if (!id) return;
    var deferred = getDeferred();
    var rec = deferred.find(function (d) { return d.id === id; });
    if (!rec) { toast('Запись не найдена', 'err'); return; }
    if (rec.status === 'completed') { toast('Нельзя отменить оплаченный документ', 'err'); return; }
    var msg = rec.status === 'in_cart'
      ? 'Товары уже в корзине. Перенести обратно в отложенные? Склад не изменится.'
      : 'Перенести товары обратно в отложенные? Склад не изменится.';
    confirmAction('Отменить документ', msg, function () {
      // Remove from cart if status is in_cart
      if (rec.status === 'in_cart') {
        rec.items.forEach(function (item) {
          saleCart = saleCart.filter(function (c) { return c.id !== item.productId || c.receiptId !== 'deferred'; });
        });
      ...
      deferred = deferred.map(function (d) {
        if (d.id !== id) return d;
        return Object.assign({}, d, { status: 'pending', paymentMethod: null });
      });
      setDeferred(deferred);
      // Update document status
      var docs = getDocuments().map(function (d) {
        if (d.id !== id) return d;
        return Object.assign({}, d, { status: 'cancelled', paymentMethod: null });
      });
      setDocuments(docs);
  ...
  window.renderDeferred = function () {
    ...
    // Show payment method column only for non-cancelled items
    var cols = ['Клиент', 'Телефон', 'Товары', 'Сумма', 'Кассир', 'Дата', 'Статус', 'Оплата', ''];
    var rows = filtered.map(function (d) {
      var statusBadge = d.status === 'pending' ? '<span class="badge badge-warn">В ожидании</span>'
        : d.status === 'awaiting_payment' ? '<span class="badge badge-info">Ожидает оплаты</span>'
        : d.status === 'completed' ? '<span class="badge badge-ok">Оплачен</span>'
        : d.status === 'in_cart' ? '<span class="badge badge-info">В корзине</span>'
        : '<span class="badge badge-danger">Отменён</span>';
      var paymentLabel = d.paymentMethod
        ? (d.paymentMethod === 'cash' ? '💵 Наличные' : d.paymentMethod === 'kaspi' ? '📱 Kaspi QR' : d.paymentMethod === 'transfer' ? '🏦 Банк' : d.paymentMethod)
        : '—';
      var itemsHtml = (d.items || []).map(function (it) {
        return '<div style="font-size:12px;padding:2px 0">' + esc(it.productName) + ' × ' + it.quantity + ' = ' + fmt(it.total) + '</div>';
      }).join('');
    ...
    document.getElementById('deferred-table').innerHTML = rows.length
      ? tableHTML(cols, rows)
      : '<div class="empty">Нет отложенных товаров</div>';
  };

  window.openDeferredModal = function () {
    if (!isAdmin()) { toast('Только администратор', 'err'); return; }
  ...
  window.searchDeferredProduct = function () {
    ...
    if (!products.length) { results.innerHTML = '<div style="padding:8px;color:var(--text-muted)">Ничего не найдено</div>'; return; }
    results.innerHTML = products.map(function (p) {
      return '<div class="sale-result-item" onclick="selectDeferredProduct(\'' + p.id + '\')">' +
        '<strong>' + esc(p.name) + '</strong>' +
        (p.code ? ' <span class="code-tag">' + esc(p.code) + '</span>' : '') +
        ' <span style="color:var(--text-muted)">Остаток: ' + p.quantity + '</span></div>';
    }).join('');
  };

  window.selectDeferredProduct = function (id) {
  ...
  window.calcDeferredTotal = function () {
  ...
  window.saveDeferred = function () {
    ...
    if (qty <= 0) { toast('Укажите количество', 'err'); return; }
    var p = getProducts().find(function (x) { return x.id === productId; });
    if (!p) { toast('Товар не найден', 'err'); return; }
    if (qty > p.quantity) { toast('Недостаточно товара (остаток: ' + p.quantity + ')', 'err'); return; }
    var deferred = getDeferred();
    var defId = uid();
    p.quantity -= qty;
    ...
    deferred.push({
      id: defId,
      items: [{ productId: p.id, productCode: p.code || '', productName: p.name, quantity: qty, unitPrice: price, total: itemTotal }],
      customerName: customerName, customerPhone: '',
      total: itemTotal, quantity: qty,
      cashierName: currentUser.name, status: 'pending', note: note,
      date: new Date().toISOString(), completedAt: null
    });
    setDeferred(deferred);
    // Also create a document entry so it appears in Документы section
    ...
    docs.unshift({
      id: defId, type: 'deferred', docType: 'deferred',
      items: [{ productCode: p.code || '', productName: p.name, quantity: qty, unitPrice: price, total: itemTotal }],
      customerName: customerName, clientName: customerName,
      total: itemTotal,
      status: 'pending', date: new Date().toISOString(), documentDate: new Date().toISOString(),
      meta: {}
    });
  ...
  window.exportDeferredExcel = function () {
    exportSectionToExcel('deferred', getDeferred(), 'SANAQ_Отложенные_' + todayStr() + '.xlsx');
  };

  window.completeSale = function () {
    if (!checkPermission('createSale')) { toast('Нет прав на создание продажи', 'err'); return; }
    if (!saleCart.length) { toast('Корзина пуста', 'err'); return; }
    ...
    // Check stock (skip universal products)
    for (let i = 0; i < saleCart.length; i++) {
      const c = saleCart[i];
      if (c.isUniversal) continue;
    ...
    if (currentPayment === 'debt') {
      ...
      if (!debtName) { toast('Введите имя должника', 'err'); return; }
      if (!debtPhone) { toast('Введите телефон должника', 'err'); return; }
      if (!debtDue) { toast('Выберите дату возврата долга', 'err'); return; }
    }
    ...
    // Если оплата долгом — создаём запись в долгах
    if (currentPayment === 'debt') {
      var debtors = getDebtors();
      var debtor = debtors.find(function (d) { return d.name.toLowerCase() === debtName.toLowerCase() && d.phone === debtPhone; });
      if (!debtor) {
        debtor = { id: uid(), name: debtName, phone: debtPhone, rating: 'good' };
        debtors.push(debtor);
        setDebtors(debtors);
      }
      var debts = getDebts();
      debts.push({
        ...
        debtorId: debtor.id,
        debtorName: debtName,
        productCode: saleCart.map(function(c){return c.code||''}).join(', '),
        productName: saleCart.map(function(c){return c.name}).join(', '),
        quantity: saleCart.reduce(function(s,c){return s+c.qty}, 0),
        amount: finalTotal,
    ...
    renderPosSideHistory();
    renderDashboard();
    if (currentPayment === 'debt') renderDebts();
  };
    ...
    document.getElementById('sales-today-list').innerHTML = receipts.length
      ? tableHTML(cols, receipts.map(function (r) {
          return [
            ...
            fmtDate(r.date),
            '<button class="btn btn-sm btn-secondary" onclick="openReceipt(\'' + r.id + '\')">Открыть</button> ' +
            '<button class="btn btn-sm btn-primary" onclick="printInvoice(\'' + r.id + '\')">Накладная</button> ' +
            '<button class="btn btn-sm btn-primary" onclick="printSalePKO(\'' + r.id + '\')">ПКО</button>'
          ];
        }))
      : '<div class="empty">Продаж сегодня нет</div>';
  }
  
  // ─── НАКЛАДНАЯ (из истории продаж) ───
  function buildInvoiceHTML(receiptId) {
    var allSales = getSales().filter(isSaleActive);
    var sales = allSales.filter(function (s) { return s.receiptId === receiptId; });
    if (!sales.length) sales = allSales.filter(function (s) { return s.id === receiptId; });
    if (!sales.length) sales = allSales.filter(function (s) { return String(s.receiptId) === String(receiptId) || String(s.id) === String(receiptId); });
    ...
    var totalQty = sales.reduce(function (s, x) { return s + (Number(x.quantity) || 0); }, 0);

    function esc(v) { return String(v || '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }

    var rows = sales.map(function (s, i) {
      return '<tr>' +
        '<td style="padding:6px 4px;border:1px solid #000;text-align:center;font-size:11px">' + (i + 1) + '</td>' +
        '<td style="padding:6px 4px;border:1px solid #000;text-align:center;font-size:11px">' + esc(s.productCode || '—') + '</td>' +
        '<td style="padding:6px 4px;border:1px solid #000;font-size:11px">' + esc(s.productName) + '</td>' +
        '<td style="padding:6px 4px;border:1px solid #000;text-align:center;font-size:11px">шт</td>' +
        '<td style="padding:6px 4px;border:1px solid #000;text-align:center;font-size:11px">' + s.quantity + '</td>' +
        '<td style="padding:6px 4px;border:1px solid #000;text-align:center;font-size:11px">' + s.quantity + '</td>' +
        '<td style="padding:6px 4px;border:1px solid #000;text-align:right;font-size:11px">' + fmt(s.unitPrice) + '</td>' +
        '<td style="padding:6px 4px;border:1px solid #000;text-align:right;font-size:11px;font-weight:600">' + fmt(s.total) + '</td>' +
        '</tr>';
    }).join('');
    ...
    html += '<thead><tr>' +
      '<th rowspan="2" style="padding:5px;border:1px solid #000;text-align:center;font-size:10px;width:28px">№ п/п</th>' +
      '<th rowspan="2" style="padding:5px;border:1px solid #000;text-align:center;font-size:10px;width:70px">Номенкл. номер</th>' +
      '<th rowspan="2" style="padding:5px;border:1px solid #000;text-align:center;font-size:10px">Наименование, характеристика</th>' +
      '<th rowspan="2" style="padding:5px;border:1px solid #000;text-align:center;font-size:10px;width:38px">Ед.</th>' +
      '<th colspan="2" style="padding:5px;border:1px solid #000;text-align:center;font-size:10px;width:70px">Количество</th>' +
      '<th rowspan="2" style="padding:5px;border:1px solid #000;text-align:center;font-size:10px;width:70px">Цена, KZT</th>' +
      '<th rowspan="2" style="padding:5px;border:1px solid #000;text-align:center;font-size:10px;width:80px">Сумма с НДС, KZT</th>' +
      '</tr><tr>' +
      '<th style="padding:5px;border:1px solid #000;text-align:center;font-size:9px">подлежит</th>' +
      '<th style="padding:5px;border:1px solid #000;text-align:center;font-size:9px">отпущено</th>' +
  ...
  window.printInvoice = function (receiptId) {
    try {
      if (!receiptId) { toast('Не указан ID накладной', 'err'); return; }
      var html = buildInvoiceHTML(receiptId);
      if (!html && typeof receiptId !== 'string') html = buildInvoiceHTML(String(receiptId));
      if (!html) { toast('Накладная не найдена', 'err'); return; }
  ...
  function showInvoiceOverlay(html) {
    ...
    overlay.innerHTML = '<style>' + printCSS + '</style><div id="invoice-print-area" style="background:#fff;max-width:800px;width:100%;max-height:95vh;overflow-y:auto;border-radius:8px;box-shadow:0 8px 40px rgba(0,0,0,0.3);padding:30px;position:relative">' +
      '<div class="no-print" style="position:sticky;top:0;float:right;display:flex;gap:8px;align-items:center;z-index:1;margin-bottom:10px">' +
      '<button onclick="window.print()" style="padding:8px 20px;font-size:14px;background:#1e40af;color:#fff;border:none;border-radius:6px;cursor:pointer">🖨 Печать</button>' +
      '<button onclick="downloadInvoiceExcel(window._currentPrintReceiptId)" style="padding:8px 20px;font-size:14px;background:#16a34a;color:#fff;border:none;border-radius:6px;cursor:pointer">📥 Excel</button>' +
      '<button onclick="document.getElementById(\'invoice-overlay\').remove()" style="padding:8px 20px;font-size:14px;background:#888;color:#fff;border:none;border-radius:6px;cursor:pointer">✕ Закрыть</button>' +
      '</div>' + html + '</div>';
    document.body.appendChild(overlay);
  }

  window.printSalePKO = function (receiptId) {
    try {
      if (!receiptId) { toast('Не указан ID чека', 'err'); return; }
      var allSales = getSales().filter(isSaleActive);
      var sales = allSales.filter(function (s) { return s.receiptId === receiptId; });
      if (!sales.length) sales = allSales.filter(function (s) { return s.id === receiptId; });
      if (!sales.length) sales = allSales.filter(function (s) { return String(s.receiptId) === String(receiptId) || String(s.id) === String(receiptId); });
      if (!sales.length) { toast('Продажа не найдена', 'err'); return; }
      var first = sales[0];
      var store = window.ApAuth && window.ApAuth.getCurrentStore();
      var storeName = store ? store.storeName : 'SANAQ';
  ...
  function buildSalePKOHTML(sale, receiptId, storeName, total) {
    ...
    h += '<div style="display:flex;justify-content:space-between;margin-bottom:16px">' +
      '<span><strong>№:</strong> ' + receiptId.slice(-6) + '</span>' +
      '<span><strong>Дата:</strong> ' + fmtDate(date) + '</span>' +
      '</div>';
    ...
    h += '</div>';
    h += '<div style="font-size:12px;margin-top:16px">' +
      '<div style="display:flex;justify-content:space-between;max-width:500px">' +
      '<span>Главный бухгалтер: _______________</span>' +
      '<span>Кассир: _______________</span>' +
      '</div></div>';
    h += '<div style="font-size:11px;color:#555;margin-top:24px;text-align:center">' +
      '<em>Организация (ИП): ' + escapeHtml(storeName) + '</em></div>';
    h += '</div>';
    return h;
  }
  
  window.downloadInvoiceExcel = async function (receiptId) {
    if (!receiptId) { toast('ID накладной не указан', 'err'); return; }
    var allSales = getSales().filter(isSaleActive);
    var sales = allSales.filter(function (s) { return s.receiptId === receiptId; });
    if (!sales.length) sales = allSales.filter(function (s) { return s.id === receiptId; });
    if (!sales.length) sales = allSales.filter(function (s) { return String(s.receiptId) === String(receiptId) || String(s.id) === String(receiptId); });
    if (!sales.length) { toast('Нет данных для экспорта', 'err'); return; }
    var first = sales[0];
    var store = window.ApAuth && window.ApAuth.getCurrentStore();
    var storeName = store ? store.storeName : 'SANAQ';
    ...
    if (first.payment === 'mixed') {
      var pts = [];
      if (Number(first.cashAmount) > 0) pts.push('нал:' + fmtShort(first.cashAmount));
      if (Number(first.kaspiAmount) > 0) pts.push('kaspiqr:' + fmtShort(first.kaspiAmount));
      if (Number(first.transferAmount) > 0) pts.push('банк:' + fmtShort(first.transferAmount));
      if (pts.length) pay = pts.join(' ');
    }
    var headers = ['№', 'Код товара', 'Наименование', 'Кол-во', 'Цена', 'Сумма'];
    var widths = [6, 18, 42, 12, 14, 16];
    var rows = sales.map(function (s, i) {
      return [i + 1, s.productCode || '', s.productName || '', Number(s.quantity) || 0, Number(s.unitPrice) || 0, Number(s.total) || 0];
    });
    var hdrRow = [
      storeName + ' — Накладная № ' + receiptId.slice(-6),
      '', '', '', '', ''
    ];
    var infoRow1 = ['Дата: ' + fmtDate(first.date), '', 'Кассир: ' + (first.userName || '—'), '', '', ''];
    var infoRow2 = [bin ? 'БИН/ИИН: ' + bin : '', '', 'Оплата: ' + pay, '', '', ''];
    var totalRow = ['', '', '', '', 'Итого:', total];
    ...
    if (typeof ExcelJS !== 'undefined') {
      ...
      styleExcelCell(t.getCell(1), { font: { name: 'Arial', size: 14, bold: true, color: { argb: 'FF1F2937' } }, alignment: { horizontal: 'center', vertical: 'middle' } });
      ...
      i1.height = 20;
      i1.eachCell(function (c) { styleExcelCell(c, { font: { name: 'Arial', size: 10 }, alignment: { vertical: 'middle' } }); });

      var i2 = ws.addRow(infoRow2);
      ws.mergeCells(3, 1, 3, 2);
      ws.mergeCells(3, 3, 3, 4);
      i2.height = 20;
      i2.eachCell(function (c) { styleExcelCell(c, { font: { name: 'Arial', size: 10 }, alignment: { vertical: 'middle' } }); });
      ...
      th.eachCell(function (c, ci) {
        styleExcelCell(c, {
          font: { name: 'Arial', size: 10, bold: true, color: { argb: 'FFFFFFFF' } },
          fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1F2937' } },
          alignment: { horizontal: ci === 1 || ci >= 4 ? 'center' : 'left', vertical: 'middle', wrapText: true },
          border: { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } }
        });
      });
      ...
      rows.forEach(function (rd) {
        ...
        rw.eachCell(function (c, ci) {
          styleExcelCell(c, {
            font: { name: 'Arial', size: 10 },
            alignment: { vertical: 'middle', horizontal: ci >= 4 ? 'right' : (ci === 1 ? 'center' : 'left'), wrapText: true },
            fill: alt ? { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF9FAFB' } } : { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFFFFF' } },
            border: { top: { style: 'thin', color: { argb: 'FFE5E7EB' } }, left: { style: 'thin', color: { argb: 'FFE5E7EB' } }, bottom: { style: 'thin', color: { argb: 'FFE5E7EB' } }, right: { style: 'thin', color: { argb: 'FFE5E7EB' } } }
          });
          if (ci >= 5) c.numFmt = '#,##0';
      ...
      totRow.eachCell(function (c, ci) {
        styleExcelCell(c, {
          ...
          alignment: { vertical: 'middle', horizontal: ci < 5 ? 'right' : 'right' },
          border: { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } }
        });
        if (ci === 6) c.numFmt = '#,##0';
      });
  ...
  // ─── ВОЗВРАТ ТОВАРА ───
  window.openReturnSelector = function () {
    if (!isAdmin()) { toast('Только для администратора', 'err'); return; }
    var all = getSales().filter(isSaleActive);
    var receipts = groupSalesIntoReceipts(all).slice(0, 50);
    if (!receipts.length) { toast('Нет доступных продаж для возврата', 'err'); return; }
    ...
    body.innerHTML = receipts.map(function (r) {
      var itemsSummary = r.items.map(function (it) {
        return esc(it.productName) + ' × ' + it.quantity;
      }).join(', ');
      return '<div class="return-receipt-row" onclick="selectReturnReceipt(\'' + r.id + '\')">' +
        '<div style="font-weight:600">Чек № ' + esc(r.id.slice(-6)) + '</div>' +
        '<div style="font-size:12px;color:var(--text-muted)">' + fmtDate(r.date) + ' | ' + esc(r.userName || '—') + ' | ' + fmt(r.total) + '</div>' +
        '<div style="font-size:11px;color:var(--text-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + itemsSummary.slice(0, 120) + '</div>' +
        '</div>';
    }).join('');
    openModal('modal-return-selector');
  };

  window.selectReturnReceipt = function (receiptId) {
    closeModal('modal-return-selector');
    openReturnModalFromReceipt(receiptId);
  };

  window.openReturnModalFromReceipt = function (receiptId) {
    if (!isAdmin()) { toast('Только для администратора', 'err'); return; }
    receiptId = receiptId || window._currentReceiptId;
    if (!receiptId) { toast('Чек не найден', 'err'); return; }
    var all = getSales().filter(isSaleActive);
    ...
    r.items.forEach(function (it) {
      ...
      tr.innerHTML =
        ...
        '<td style="text-align:center"><input type="number" class="form-input" style="width:70px;padding:4px;text-align:center" min="0" max="' + qty + '" value="' + qty + '" data-price="' + price + '" data-max="' + refundMax + '" oninput="calcReturnTotal()"></td>' +
        '<td style="text-align:right;font-weight:600;color:var(--err)">' + fmt(refundMax) + '</td>';
      body.appendChild(tr);
    });
  ...
  window.submitReturn = function () {
    ...
    var receiptSales = allSales.filter(function (s) { return s.receiptId === receiptId && isSaleActive(s); });
    ...
    inputs.forEach(function (inp) {
      ...
      if (!sale) { toast('Ошибка: продажа не найдена для строки ' + (idx + 1), 'err'); return; }
      if (sale.status === 'returned') { toast('Товар уже возвращён: ' + sale.productName, 'err'); return; }
      var price = Number(inp.dataset.price) || 0;
      var refundAmount = returnQty * price;
      totalRefund += refundAmount;
      items.push({ sale: sale, returnQty: returnQty, refundAmount: refundAmount });
    });

    if (!hasAny) { toast('Укажите количество к возврату', 'err'); return; }
    if (totalRefund <= 0) { toast('Сумма возврата должна быть больше 0', 'err'); return; }

    confirmAction(
      'Подтверждение возврата',
      'Возврат товаров на сумму ' + fmt(totalRefund) + '. Товар вернётся на склад.',
      function () {
        var returnedItems = [];
        ...
        items.forEach(function (it) {
          ...
          if (returnQty >= origQty) {
            var idx = allSales.findIndex(function (s) { return s.id === sale.id; });
            if (idx >= 0) {
            ...
            var sidx = allSales.findIndex(function (s) { return s.id === sale.id; });
            if (sidx >= 0) {
          ...
          returnedItems.push({
            saleId: sale.id, productId: sale.productId, productName: sale.productName,
            quantity: returnQty, refundAmount: it.refundAmount
          });
        });

        setSales(allSales);

        // Создаём запись расхода (возврат) для учёта
        var expenses = getExpenses();
        expenses.push({
          id: uid(), category: 'Возврат', amount: totalRefund,
          note: 'Возврат по чеку № ' + receiptId.slice(-6),
          userName: currentUser.name, date: new Date().toISOString(), status: 'active'
        });
        setExpenses(expenses);
  ...
  function getShiftReturnCount(shiftId) {
    var returns = getSales().filter(function(s) { return s.status === 'returned' && s.shiftId === shiftId; });
    return returns.length;
  }
  function canMakeReturn() {
    var shift = getOpenShiftForCashier(currentUser.id) || getOpenShiftForCashier(currentUser.username);
    if (!shift) { toast('Смена не открыта', 'err'); return false; }
  ...
  window.saveExpense = function () {
    if (!checkPermission('viewStatistics')) { toast('Нет доступа к расходам', 'err'); return; }
  ...
  window.cancelExpenseConfirm = function (id) {
    if (!checkPermission('viewStatistics')) { toast('Нет доступа', 'err'); return; }
  ...
  window.switchStatsTab = function (tab) {
    ...
    if (tab === 'shifts') renderShiftStats();
    if (tab === 'cashiers') renderCashierStats();
  };

  // Cashier share (stored in localStorage)
  ...
  window.saveCashierShare = function () {
  ...
  function renderStatistics() {
    ...
    else if (period === 'week') { var d = new Date(now); d.setDate(d.getDate() - 7); periodStart = d; }
    else if (period === 'month') { periodStart = new Date(now.getFullYear(), now.getMonth(), 1); }
    const sales = getSales().filter(function (s) { return isSaleActive(s) && new Date(s.date) >= periodStart; });
    const expenses = getExpenses().filter(function (e) { return isExpenseActive(e) && new Date(e.date) >= periodStart; });
    ...
    const expTotal = expenses.reduce(function (s, e) { return s + e.amount; }, 0);
    ...
    document.getElementById('stats-cards').innerHTML =
      card('Кол-во продаж', salesCount, '') +
      card('Выручка', fmt(revenue), 'ok') +
      card('Прибыль', fmt(profit), profit >= 0 ? 'ok' : 'err') +
      card('Средний чек', fmt(avgCheck), '') +
      card('Продано товаров', itemsCount, '') +
      card('Kaspi QR', fmt(kaspi), 'kaspi') +
      card('Наличные', fmt(cash), 'cash') +
      card('Банк', fmt(transfer), 'bank') +
    ...
    var topProduct = Object.keys(productSales).sort(function(a,b) { return productSales[b] - productSales[a]; })[0] || null;
    var topCashier = Object.keys(cashierSales).sort(function(a,b) { return cashierSales[b] - cashierSales[a]; })[0] || null;
    var peakHour = Object.keys(hourlySales).sort(function(a,b) { return hourlySales[b] - hourlySales[a]; })[0] || null;
    ...
    analysisHTML += '<div class="card" style="flex:1"><div class="card-label">📊 Способы оплаты</div><div style="display:flex;gap:4px;margin-top:6px;height:8px;border-radius:4px;overflow:hidden;background:var(--border)">' +
      (cashPct > 0 ? '<div style="flex:' + cashPct + ';background:#22c55e;min-width:4px" title="Наличные ' + cashPct + '%"></div>' : '') +
      (kaspiPct > 0 ? '<div style="flex:' + kaspiPct + ';background:#e11d48;min-width:4px" title="Kaspi QR ' + kaspiPct + '%"></div>' : '') +
      (transferPct > 0 ? '<div style="flex:' + transferPct + ';background:#3b82f6;min-width:4px" title="Банк ' + transferPct + '%"></div>' : '') +
      '</div><div style="display:flex;gap:12px;margin-top:4px;font-size:11px;color:var(--text-muted)">' +
      (cashPct > 0 ? '<span>💵 ' + cashPct + '%</span>' : '') +
      (kaspiPct > 0 ? '<span>📱 ' + kaspiPct + '%</span>' : '') +
      (transferPct > 0 ? '<span>🏦 ' + transferPct + '%</span>' : '') +
      '</div></div>';

    document.getElementById('stats-analysis').innerHTML = analysisHTML;

    const statSaleCols = ['Чек', 'Товары', 'Сумма', 'Оплата', 'Статус', 'Кассир', 'Время', ''];
    document.getElementById('stats-sales-table').innerHTML = receipts.length
      ? '<div class="table-wrap">' + tableHTML(
          statSaleCols,
          receipts.map(function (r) {
            const qty = r.items.reduce(function (sum, it) { return sum + (Number(it.quantity) || 0); }, 0);
            const row = [
              '<span class="code-tag">№ ' + r.id.slice(-6) + '</span>',
              qty + ' шт.',
              fmt(r.total),
  ...
  window.renderProductAnalysis = function () {
    ...
    var sales = getSales().filter(function (s) { return isSaleActive(s) && new Date(s.date) >= periodStart; });
    var sort = (document.getElementById('stats-product-sort') || {}).value || 'top';
    var prodMap = {};
    sales.forEach(function (s) {
      var key = s.productId || s.productName || '';
      if (!key) return;
      if (!prodMap[key]) prodMap[key] = { name: s.productName || '—', code: s.productCode || '', qty: 0, sum: 0, id: s.productId };
      prodMap[key].qty += Number(s.quantity) || 0;
      prodMap[key].sum += Number(s.total) || 0;
    });
    var prodArr = Object.keys(prodMap).map(function (k) { return prodMap[k]; });
  ...
  function renderStatCharts(sales, expenses) {
    Object.keys(_statCharts).forEach(function (k) { if (_statCharts[k]) { _statCharts[k].destroy(); delete _statCharts[k]; } });
    if (typeof Chart === 'undefined') return;

    // 1. Sales & Profit over last 30 days
    var dayMap = {};
    var now = new Date();
    for (var i = 29; i >= 0; i--) {
      var d = new Date(now);
      ...
      dayMap[key] = { revenue: 0, cost: 0 };
    }
    sales.forEach(function (s) {
      var sk = (s.date || '').slice(0, 10);
      if (dayMap[sk]) { dayMap[sk].revenue += s.total; dayMap[sk].cost += (Number(s.purchasePrice) || 0) * (Number(s.quantity) || 0); }
    });
    var labels = Object.keys(dayMap);
    var revData = labels.map(function (k) { return dayMap[k].revenue; });
    var costData = labels.map(function (k) { return dayMap[k].cost; });
    ...
    if (ctx1) {
      _statCharts.sales = new Chart(ctx1, {
        ...
        data: {
          ...
          datasets: [
            { label: 'Выручка', data: revData, backgroundColor: 'rgba(34,197,94,0.7)', borderColor: 'rgb(34,197,94)', borderWidth: 1 },
            { label: 'Себестоимость', data: costData, backgroundColor: 'rgba(239,68,68,0.7)', borderColor: 'rgb(239,68,68)', borderWidth: 1 }
          ]
        },
        options: {
    ...
    sales.forEach(function (s) {
      if (s.payment === 'cash') pCash += s.total;
      else if (s.payment === 'kaspi') pKaspi += s.total;
      else if (s.payment === 'transfer') pTransfer += s.total;
      else if (s.payment === 'debt') pDebt += s.total;
      else if (s.payment === 'mixed') {
    ...
    if (ctx2) {
      _statCharts.payments = new Chart(ctx2, {
        type: 'doughnut',
        data: {
          labels: ['Наличные', 'Kaspi QR', 'Банк', 'В долг'],
          datasets: [{
            data: [pCash, pKaspi, pTransfer, pDebt],
            backgroundColor: ['#22c55e', '#e11d48', '#3b82f6', '#f59e0b']
          }]
        },
        options: {
          responsive: true, maintainAspectRatio: false,
          plugins: {
            ...
            tooltip: { callbacks: { label: function (ctx) { return ctx.label + ': ' + ctx.parsed.toLocaleString('ru-RU') + ' ₸'; } } }
          }
        }
      });
    }
    ...
    var colors = ['#ef4444','#f97316','#eab308','#22c55e','#06b6d4','#3b82f6','#8b5cf6','#ec4899','#78716c'];
  ...
  function renderShiftStats() {
    ...
    var shiftData = shifts.map(function (sh) {
      var shiftSales = sales.filter(function (s) { return s.shiftId === sh.id; });
      var rev = shiftSales.reduce(function (s, x) { return s + x.total; }, 0);
      var cost = shiftSales.reduce(function (s, x) { return s + (Number(x.purchasePrice) || 0) * (Number(x.quantity) || 0); }, 0);
    ...
    var totalRev = shiftData.reduce(function (s, x) { return s + x.revenue; }, 0);
    var totalProfit = shiftData.reduce(function (s, x) { return s + x.profit; }, 0);
    var totalShifts = shiftData.length;
    var openShifts = shiftData.filter(function (s) { return s.status === 'open'; }).length;

    document.getElementById('shifts-stats-cards').innerHTML =
      card('Всего смен', totalShifts, '') +
      card('Открытых смен', openShifts, 'ok') +
      card('Выручка за все смены', fmt(totalRev), 'ok') +
      card('Прибыль за все смены', fmt(totalProfit), totalProfit >= 0 ? 'ok' : 'err');

    shiftData.sort(function (a, b) { return (b.openedAt || '').localeCompare(a.openedAt || ''); });
    ...
    document.getElementById('shifts-stats-table').innerHTML = rows.length
      ? '<div class="table-wrap">' + tableHTML(
          ['Кассир', 'Открыта', 'Закрыта', 'Продаж', 'Выручка', 'Себестоимость', 'Прибыль'],
          rows
        ) + '</div>'
      : '<div class="empty">Смен нет</div>';
  ...
  function renderCashierStats() {
    ...
    shifts.forEach(function (sh) {
      ...
      if (!cashierMap[name]) {
        var u = users.find(function (x) { return (x.email || x.username || '') === (sh.cashierUsername || ''); });
        ...
        cashierMap[name] = { name: name, username: sh.cashierUsername || '', shifts: 0, sales: 0, revenue: 0, cogs: 0, percent: pct, isCashier: isCashierRole };
      }
    ...
    cashierRows.sort(function (a, b) { return b.revenue - a.revenue; });

    var totalRev = cashierRows.reduce(function (s, x) { return s + x.revenue; }, 0);
    var totalSales = cashierRows.reduce(function (s, x) { return s + x.sales; }, 0);

    document.getElementById('cashiers-stats-cards').innerHTML =
      card('Всего кассиров', cashierRows.length, '') +
      card('Общая выручка', fmt(totalRev), 'ok') +
      card('Всего продаж', totalSales, '');
    ...
    document.getElementById('cashiers-stats-table').innerHTML = rows.length
      ? '<div class="table-wrap">' + tableHTML(
          ['Кассир', 'Смен', 'Продаж', 'Выручка', 'Себестоимость', 'Прибыль', 'Доля от выручки', 'Процент', 'Заработок'],
          rows
        ) + '</div>'
  ...
  // ─── NEW STATISTICS (unsold, analytics, heatmap, records) ───
  var _statsSubTab = 'unsold';

  window.switchStatsSubTab = function(tab) {
    _statsSubTab = tab;
    document.querySelectorAll('[data-stab2]').forEach(function(b) { b.classList.toggle('active', b.dataset.stab2 === tab); });
    document.querySelectorAll('.stab-sub').forEach(function(s) { s.style.display = 'none'; });
    var el = document.getElementById('stab2-' + tab);
    if (el) el.style.display = 'block';

    if (tab === 'unsold') renderUnsoldProducts(30);
  ...
  function renderUnsoldProducts(days) {
    ...
    var sales = getSales().filter(function(s) { return isSaleActive(s) && new Date(s.date) >= start; });
    var soldIds = {};
    sales.forEach(function(s) { if (s.productId) soldIds[s.productId] = true; });

    var products = getProducts();
    var unsold = products.filter(function(p) { return !soldIds[p.id]; });
  ...
  window.switchUnsoldPeriod = function(days) {
    document.querySelectorAll('[data-unsold]').forEach(function(b) { b.classList.toggle('active', parseInt(b.dataset.unsold) === days); });
  ...
  function renderAnalyticsCards(sales) {
    ...
    var totalRevenue = sales.reduce(function(s, x) { return s + (Number(x.total) || 0); }, 0);
    var avgCheck = totalSales > 0 ? Math.round(totalRevenue / totalSales) : 0;

    container.innerHTML =
      '<div class="card"><div class="card-label">📊 Всего продаж</div><div class="card-value" style="font-size:18px">' + totalSales + '</div></div>' +
      '<div class="card"><div class="card-label">💰 Общая выручка</div><div class="card-value" style="font-size:18px">' + fmt(totalRevenue) + '</div></div>' +
      '<div class="card"><div class="card-label">🧾 Средний чек</div><div class="card-value" style="font-size:18px">' + fmt(avgCheck) + '</div></div>' +
      '<div class="card"><div class="card-label">⏰ Пиковый час</div><div class="card-value" style="font-size:18px">' + peak.hour + ':00</div><div style="font-size:11px;color:var(--text-muted)">' + peak.count + ' продаж, ' + fmt(Math.round(peak.revenue)) + '</div></div>';
  }
  ...
  function renderPeakHour(sales) {
    ...
    sales.forEach(function(s) {
      var hour = s.date ? new Date(s.date).getHours() : 0;
      if (!hourlyChecks[hour]) hourlyChecks[hour] = 0;
    ...
    Object.keys(hourlyChecks).forEach(function(h) {
      if (hourlyChecks[h] > peakCount) { peakCount = hourlyChecks[h]; peakHour = parseInt(h); peakRev = hourlyRevenue[h]; }
    });
    return { hour: peakHour, count: peakCount, revenue: peakRev };
  }
  ...
  function renderSalesHeatmap(sales) {
    ...
    for (var d = 0; d < 7; d++) { heat[d] = {}; for (var h = 0; h < 24; h++) heat[d][h] = 0; }

    sales.forEach(function(s) {
      var dt = new Date(s.date);
      var day = dt.getDay();
      var hour = dt.getHours();
      if (heat[day] && heat[day][hour] !== undefined) heat[day][hour] += Number(s.total) || 0;
    });

    var maxVal = 0;
    for (var d2 = 0; d2 < 7; d2++) for (var h2 = 0; h2 < 24; h2++) maxVal = Math.max(maxVal, heat[d2][h2]);
    maxVal = maxVal || 1;

    var html = '<table style="border-collapse:collapse;font-size:11px"><thead><tr><th style="padding:4px;width:30px"></th>';
    for (var h3 = 0; h3 < 24; h3++) html += '<th style="padding:4px;min-width:28px;text-align:center;font-weight:400;color:var(--text-muted)">' + h3 + '</th>';
    html += '</tr></thead><tbody>';
    ...
    for (var d3 = 0; d3 < 7; d3++) {
      ...
      for (var h4 = 0; h4 < 24; h4++) {
        ...
        html += '<td style="padding:2px;text-align:center;background:rgb(' + r + ',' + g + ',' + b + ');border-radius:3px;font-size:10px;color:' + (intensity > 0.5 ? '#fff' : '#666') + '" title="' + days[d3] + ' ' + h4 + ':00 — ' + fmt(Math.round(val)) + ' ₸">' +
          (val > 0 ? (val > 100000 ? '🔥' : val > 10000 ? '•' : '·') : '') +
          '</td>';
      }
      html += '</tr>';
    }
  ...
  function renderRecords(sales, receipts, period) {
    ...
    Object.keys(dailyTotals).forEach(function(d) {
      if (dailyTotals[d] > bestDayTotal) { bestDayTotal = dailyTotals[d]; bestDay = d; }
    });
    ...
    Object.keys(weeklyTotals).forEach(function(w) {
      if (weeklyTotals[w] > bestWeekTotal) { bestWeekTotal = weeklyTotals[w]; bestWeek = w; }
    });
    ...
    Object.keys(monthlyTotals).forEach(function(m) {
      if (monthlyTotals[m] > bestMonthTotal) { bestMonthTotal = monthlyTotals[m]; bestMonth = m; }
    });

    cardsContainer.innerHTML =
      '<div class="card"><div class="card-label">🏆 Рекорд дня</div><div class="card-value" style="font-size:18px">' + (bestDay ? fmt(bestDayTotal) : '—') + ' ₸</div><div style="font-size:11px;color:var(--text-muted)">' + (bestDay || '—') + '</div></div>' +
      '<div class="card"><div class="card-label">🏆 Рекорд недели</div><div class="card-value" style="font-size:18px">' + (bestWeek ? fmt(bestWeekTotal) : '—') + ' ₸</div><div style="font-size:11px;color:var(--text-muted)">Неделя от ' + (bestWeek || '—') + '</div></div>' +
      '<div class="card"><div class="card-label">🏆 Рекорд месяца</div><div class="card-value" style="font-size:18px">' + (bestMonth ? fmt(bestMonthTotal) : '—') + ' ₸</div><div style="font-size:11px;color:var(--text-muted)">' + (bestMonth || '—') + '</div></div>';
  }
  ...
  function renderMostExpensiveReceipt(receipts) {
    ...
    var qty = items.reduce(function(s, it) { return s + (Number(it.quantity) || 0); }, 0);

    container.innerHTML = '<div class="card" style="border-left:3px solid var(--warn)">' +
      '<div style="font-weight:700;font-size:18px;color:var(--text)">' + fmt(best.total) + ' ₸</div>' +
      '<div style="display:flex;gap:16px;margin-top:6px;flex-wrap:wrap;font-size:13px;color:var(--text-secondary)">' +
      '<span>👤 ' + esc(cashierName) + '</span>' +
      '<span>📦 ' + qty + ' шт.</span>' +
      '<span>🕐 ' + fmtDate(best.date) + '</span>' +
      '<span>💳 ' + badgePay(best.payment, best.items[0]) + '</span>' +
      '</div></div>';
  }
  ...
  // Детальный экспорт продаж (по чекам + строки)
  window.exportSalesDetailedExcel = function () {
    ...
    receipts.forEach(function (r) {
      ...
      if (r.payment === 'mixed') {
        var parts = [];
        if (Number(r.cashAmount) > 0) parts.push('наличные: ' + Number(r.cashAmount));
        if (Number(r.kaspiAmount) > 0) parts.push('Kaspi QR: ' + Number(r.kaspiAmount));
        if (Number(r.transferAmount) > 0) parts.push('Банк: ' + Number(r.transferAmount));
        if (parts.length) pay = parts.join('; ');
      }
    ...
    var headers = ['ID чека', 'Номер чека', '№ строки', 'Код товара', 'Товар', 'Кол-во', 'Цена', 'Сумма строки', 'Итого по чеку', 'Оплата', 'Кассир', 'Дата и время', 'Должник', 'Телефон должника', 'Дата возврата долга'];
    var widths = [24, 12, 10, 14, 42, 10, 14, 16, 16, 24, 20, 20, 22, 20, 18];
    createExcelWorkbook(headers, rows, widths, 'Продажи').then(function (buf) {
      ...
      toast('Excel файл скачан', 'ok');
    }).catch(function (e) { toast('Ошибка: ' + (e.message || e), 'err'); });
  };
  ...
  function renderCashiersTable() {
    var all = getAllUsers().filter(function(u) { return u.role === 'cashier'; });
    document.getElementById('cashiers-table').innerHTML = all.length
      ? tableHTML(['Имя', 'Логин', 'Тип', 'Статус', 'Смена', 'Действия'], all.map(function (u) {
          ...
          return [
            u.name, u.username, typeLabel, status, shiftInfo,
            '<div class="actions">' +
  ...
  window.openCashierModal = function (id) {
    ...
    if (id) {
      ...
      if (!u) u = getLocalUsers().find(function (x) { return x.id === id; });
      if (u) {
  ...
  function _localUsersKey() { return 'sanaq_local_users_' + (currentStoreId || ''); }
  function getLocalUsers() {
    try {
      var raw = localStorage.getItem(_localUsersKey());
      if (raw) return JSON.parse(raw);
      // fallback to unnamespaced key if currentStoreId changed
      if (currentStoreId) {
  ...
  function setLocalUsers(arr) {
    ...
    // also keep the unnamespaced key for backward compatibility
  ...
  window.saveCashier = async function () {
    ...
    if (!editId) {
      if (!username) { toast('Введите логин', 'err'); return; }
      if (!name) { toast('Введите имя кассира', 'err'); return; }
      if (!password) { toast('Введите пароль', 'err'); return; }
      var localUsers = getLocalUsers();
      if (localUsers.some(function(u) { return u.username === username; })) {
        toast('Логин уже занят', 'err'); return;
      }
      var newId = uid();
      localUsers.push({
        id: newId,
      ...
      // Save permissions and max discount for new user
      var newPerms = _pendingPerms || Object.assign({}, DEFAULT_PERMISSIONS);
      setUserPermissions(newId, { permissions: newPerms, maxDiscount: _pendingMaxDisc || 0 });
      _pendingPerms = null;
      _pendingMaxDisc = 0;
    ...
    var localUsers = getLocalUsers();
    var localIdx = localUsers.findIndex(function(x) { return x.id === editId; });
    if (localIdx !== -1) {
      localUsers[localIdx].name = name;
      localUsers[localIdx].active = active;
      if (password) localUsers[localIdx].password = password;
      setLocalUsers(localUsers);
      const shareVal = parseFloat(document.getElementById('cashier-percent').value) || 0;
  ...
  window.openMyShift = function () {
    try {
      ...
      if (getOpenShiftForCashier(currentUser.id) || getOpenShiftForCashier(cashierKey)) {
        toast('Ваша смена уже открыта', 'err'); return;
      }
      ...
      setShifts(shifts);
      // Verify shift is in cache
      var verify = getOpenShiftForCashier(currentUser.id);
      if (!verify) {
        console.error('[Shift] Shift not found in cache after setShifts!');
        // Retry once: re-push directly
        var retry = getShifts();
        retry.push({ id: newId, cashierId: currentUser.id, cashierUsername: currentUser.email || currentUser.username || '', cashierName: currentUser.name, openedAt: new Date().toISOString(), closedAt: null, status: 'open', openedBy: currentUser.name });
        setShifts(retry);
      }
  ...
  window.openShift = function () {
    if (!currentUser) { toast('Пользователь не авторизован', 'err'); return; }
    const cashierKey = currentUser.email || currentUser.username || currentUser.id;
    if (getOpenShiftForCashier(currentUser.id) || getOpenShiftForCashier(cashierKey)) {
    ...
    updateSaleShiftBanner();
    // Diagnostic: check shift survival after 2s and 10s
    ...
    setTimeout(function() {
      ...
      if (!stillHere || stillHere.id !== _shiftId) {
        ...
        console.error('[Shift] Cache shifts:', JSON.stringify(getShifts().map(function(s){return {id:s.id,status:s.status};})));
      } else {
    ...
    if (shift) {
      ...
      ctrl.innerHTML =
        '<div style="display:flex;flex-wrap:wrap;align-items:center;gap:16px;justify-content:space-between">' +
          ...
          '</div>' +
          '<button class="btn btn-success btn-lg" onclick="openMyShift()">▶ Открыть мою смену</button>' +
        '</div>';
    }
  ...
  window.submitWriteOff = async function () {
    if (!checkPermission('writeOffStock')) { toast('Нет прав на списание товара', 'err'); return; }
  ...
  window.startAuditSession = function () {
    if (!checkPermission('inventory')) { toast('Нет прав на инвентаризацию', 'err'); return; }
    var products = getProducts();
  ...
  function badgePay(pay, item) {
    if (pay === 'mixed' && item) {
      var parts = [];
      if (Number(item.cashAmount) > 0) parts.push('💵' + fmt(item.cashAmount));
      if (Number(item.kaspiAmount) > 0) parts.push('📱' + fmt(item.kaspiAmount));
      if (Number(item.transferAmount) > 0) parts.push('🏦' + fmt(item.transferAmount));
      return '<span class="badge badge-mixed">' + parts.join(' ') + '</span>';
    }
    const map = { cash: 'badge-cash', kaspi: 'badge-kaspi', transfer: 'badge-bank', debt: 'badge-warn', mixed: 'badge-mixed' };
  ...
  window.printReceipt = function () {
    ...
    if (!printWin) { toast('Разрешите всплывающее окно для печати чека', 'err'); return; }
  ...
  window.openReceipt = function (receiptId) {
    ...
    var storeName = store ? store.storeName : 'SANAQ';
    var bin = localStorage.getItem('ap_store_bin') || '';
    var dateStr = r.date ? new Date(r.date).toLocaleString('ru-RU', { day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit' }) : '—';

    var payDetail = PAY_LABELS[r.payment] || r.payment || '—';
    if (r.payment === 'mixed') {
      var pts = [];
      if (Number(r.cashAmount) > 0) pts.push('нал:' + fmtShort(r.cashAmount));
      if (Number(r.kaspiAmount) > 0) pts.push('kaspiqr:' + fmtShort(r.kaspiAmount));
      if (Number(r.transferAmount) > 0) pts.push('банк:' + fmtShort(r.transferAmount));
      if (pts.length) payDetail = pts.join(' ');
    }

    var h = escapeHtml;
    var custName = '';
    if (r.customerId) {
      var cust = getCustomers().find(function (c) { return c.id === r.customerId; });
      if (cust) custName = cust.name || '';
    }
    var header = '<div class="receipt-center receipt-bold">' + h(storeName) + '</div>' +
      (bin ? '<div class="receipt-center">БИН/ИИН ' + h(bin) + '</div>' : '') +
      '<div class="receipt-center">КАССОВЫЙ ЧЕК</div>' +
      '<div class="receipt-row"><span>Чек №</span><span>' + receiptId.slice(-6) + '</span></div>' +
      '<div class="receipt-row"><span>Дата</span><span>' + dateStr + '</span></div>' +
      '<div class="receipt-row"><span>Кассир</span><span>' + h(r.userName || '—') + '</span></div>' +
      (custName ? '<div class="receipt-row"><span>Клиент</span><span>' + h(custName) + '</span></div>' : '') +
    ...
    r.items.forEach(function (it) {
      ...
      itemsHtml += '<div class="receipt-item">' +
        '<div class="receipt-name">' + h(name) + '</div>' +
        '<div class="receipt-row"><span>' + qty + ' x ' + fmtShort(price) + '</span><span>' + fmtShort(total) + '</span></div>' +
        (it.productCode ? '<div style="font-size:11px">Код: ' + h(it.productCode) + '</div>' : '') +
        '</div>';
    });
    ...
    if (Number(r.discountAmount) > 0) totalsHtml += '<div class="receipt-row"><span>Скидка</span><span>-' + fmtShort(r.discountAmount) + '</span></div>';
    if (Number(r.bonusSpend) > 0) totalsHtml += '<div class="receipt-row"><span>Оплата бонусами</span><span>-' + fmtShort(r.bonusSpend) + '</span></div>';
    totalsHtml += '<div class="receipt-sep"></div>';
    if (r.payment === 'cash') totalsHtml += '<div class="receipt-row"><span>Наличные</span><span>' + fmtShort(netTotal) + '</span></div>';
    else if (r.payment === 'kaspi') totalsHtml += '<div class="receipt-row"><span>Kaspi QR</span><span>' + fmtShort(netTotal) + '</span></div>';
    else if (r.payment === 'transfer') totalsHtml += '<div class="receipt-row"><span>Банк</span><span>' + fmtShort(netTotal) + '</span></div>';
    else if (r.payment === 'mixed') {
      if (Number(r.cashAmount) > 0) totalsHtml += '<div class="receipt-row"><span>Наличные</span><span>' + fmtShort(r.cashAmount) + '</span></div>';
      if (Number(r.kaspiAmount) > 0) totalsHtml += '<div class="receipt-row"><span>Kaspi QR</span><span>' + fmtShort(r.kaspiAmount) + '</span></div>';
      if (Number(r.transferAmount) > 0) totalsHtml += '<div class="receipt-row"><span>Банк</span><span>' + fmtShort(r.transferAmount) + '</span></div>';
    } else if (r.payment === 'debt') totalsHtml += '<div class="receipt-row"><span>В долг</span><span>' + fmtShort(netTotal) + '</span></div>';
    if (Number(r.earnedBonus) > 0) totalsHtml += '<div class="receipt-row"><span>Бонусы начислено</span><span>+' + fmtShort(r.earnedBonus) + '</span></div>';
    totalsHtml += '<div class="receipt-sep"></div>';
    ...
    if (returnBtn) {
      var returnable = r.items.some(function (it) { return Number(it.quantity) > 0; });
  ...
  window.changeDebtorRating = function (debtorId, rating) {
  ...
  // ─── ФИКС НЕДОСТАЮЩИХ ФУНКЦИЙ ДОЛГОВ ───
  window.renderDebts = function () {
    renderDebtsList();
    if (currentDebtorId) renderDebtorDetail();
  };

  window.onDebtClientChange = function () {
  ...
  function populateDebtClientSelector(selectedId) {
    ...
    sel.innerHTML = html;
    if (selectedId && debtors.some(function(d){ return d.id === selectedId; })) {
  ...
  window.openNewDebtorModal = function () {
  ...
  window.openAddDebt = function () {
    if (!currentDebtorId) { toast('Клиент не выбран', 'err'); return; }
    var debtor = getDebtors().find(function (d) { return d.id === currentDebtorId; });
    if (!debtor) return;
  ...
  window.debtBarcodeLookup = function () {
    var barcode = document.getElementById('debt-barcode').value.trim();
    if (!barcode) return;
    var p = getProducts().find(function (x) { return (x.barcode || '') === barcode; });
    if (p) {
  ...
  window.saveDebt = function () {
    ...
    if (!selectedClientId) { toast('Выберите клиента из списка', 'err'); return; }
    ...
    if (!productName) { toast('Введите товар/описание', 'err'); return; }
    if (amount <= 0) { toast('Введите сумму долга', 'err'); return; }
    if (selectedClientId === '__new__') {
      // Create new debtor + debt
      var name = document.getElementById('debt-name').value.trim();
      var phone = document.getElementById('debt-phone').value.trim();
      if (!name) { toast('Введите имя нового клиента', 'err'); return; }
      var debtors = getDebtors();
      var debtorId = uid();
      debtors.push({
        id: debtorId, name: name, phone: phone || '',
        rating: rating, created_at: new Date().toISOString(), updated_at: new Date().toISOString()
      });
      setDebtors(debtors);
      var debts = getDebts();
      debts.push({
        id: uid(), debtorId: debtorId, debtorName: name,
        productCode: productCode, productName: productName,
        quantity: qty, amount: amount,
        cashierName: currentUser.name, dueDate: dueDate || null, status: debtStatus,
      ...
      // Add debt to existing debtor
      var debtors = getDebtors();
      var debtor = debtors.find(function (d) { return d.id === selectedClientId; });
      if (!debtor) { toast('Должник не найден', 'err'); return; }
      debtor.rating = rating;
      debtor.phone = document.getElementById('debt-phone').value.trim() || debtor.phone;
      setDebtors(debtors);
      var debts = getDebts();
      debts.push({
        id: uid(), debtorId: selectedClientId, debtorName: debtor.name,
        productCode: productCode, productName: productName,
        quantity: qty, amount: amount,
        cashierName: currentUser.name, dueDate: dueDate || null, status: debtStatus,
        note: note, date: new Date().toISOString()
  ...
  window.openPayDebt = function (totalDebt) {
    if (!currentDebtorId) { toast('Клиент не выбран', 'err'); return; }
    var debtor = getDebtors().find(function (d) { return d.id === currentDebtorId; });
    if (!debtor) return;
    if (totalDebt <= 0) { toast('Нет долга для погашения', 'err'); return; }

    // Создаём оверлей ввода суммы
    ...
    overlay.innerHTML = '<div style="background:var(--bg);border:1px solid var(--border);border-radius:12px;padding:24px 32px;box-shadow:0 8px 30px rgba(0,0,0,0.3);max-width:400px;width:90%">' +
      '<div style="font-size:16px;font-weight:600;margin-bottom:4px">Погашение долга</div>' +
      '<div style="font-size:13px;color:var(--muted);margin-bottom:16px">' + debtor.name + ' — текущий долг: ' + fmt(totalDebt) + '</div>' +
      '<div class="field"><label>Сумма оплаты</label><input type="number" id="debt-pay-amount" class="form-input" step="1" min="1" max="' + totalDebt + '" value="' + totalDebt + '" style="font-size:18px;font-weight:700"></div>' +
      '<div style="display:flex;gap:8px;margin-top:16px">' +
      '<button class="btn btn-primary" style="flex:1" onclick="submitDebtPayment()">💸 Оплатить</button>' +
      '<button class="btn btn-secondary" style="flex:1" onclick="document.getElementById(\'debt-pay-overlay\').remove()">Отмена</button>' +
      '</div></div>';
    document.body.appendChild(overlay);
    setTimeout(function () { document.getElementById('debt-pay-amount').focus(); }, 100);
  };
  window._pendingDebtPayment = { debtorId: null, debtorName: null };

  window.showPaymentMethodModal = function (callback) {
    ...
    overlay.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;z-index:99999;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center';
    overlay.innerHTML = '<div style="background:var(--bg);border:1px solid var(--border);border-radius:12px;padding:24px 32px;box-shadow:0 8px 30px rgba(0,0,0,0.3);max-width:400px;width:90%">' +
      '<div style="font-size:16px;font-weight:600;margin-bottom:16px">Выберите способ оплаты</div>' +
      '<div style="display:flex;flex-direction:column;gap:8px">' +
      '<button class="btn btn-success btn-lg" onclick="window.selectPaymentMethod(\'cash\')" style="padding:14px;font-size:16px">💵 Наличные</button>' +
      '<button class="btn btn-lg" onclick="window.selectPaymentMethod(\'kaspi\')" style="padding:14px;font-size:16px;background:#E53935;color:#fff">📱 Kaspi QR</button>' +
      '<button class="btn btn-primary btn-lg" onclick="window.selectPaymentMethod(\'transfer\')" style="padding:14px;font-size:16px">🏦 Банк</button>' +
      '</div>' +
      '<button class="btn btn-secondary" style="width:100%;margin-top:12px" onclick="document.getElementById(\'payment-method-modal\').remove()">Отмена</button>' +
      '</div>';
    document.body.appendChild(overlay);
  ...
  window.submitDebtPayment = function () {
    var amt = parseFloat(document.getElementById('debt-pay-amount').value) || 0;
    if (amt <= 0) { toast('Введите сумму', 'err'); return; }
  ...
  function completeDebtPayment(debtorId, amount, paymentMethod) {
    var debtor = getDebtors().find(function (d) { return d.id === debtorId; });
    if (!debtor) { toast('Должник не найден', 'err'); return; }
    // Создаём запись продажи для учёта денег
    var shift = getOpenShiftForCashier(currentUser.id) || getOpenShiftForCashier(currentUser.username);
    var sales = getSales();
    var receiptId = uid();
    var dateStr = new Date().toISOString();
    sales.push({
      id: uid(), receiptId: receiptId, shiftId: shift ? shift.id : null,
      productId: null, productCode: '', productName: 'Погашение долга: ' + debtor.name,
      quantity: 1, unitPrice: amount, purchasePrice: 0, total: amount,
      ...
      customerId: null, userName: currentUser.name, username: currentUser.username,
      date: dateStr, status: 'completed',
      debtorName: debtor.name || '', debtPhone: debtor.phone || '', debtReturnDate: ''
    });
    setSales(sales);

    // Создаём компенсирующую запись (отрицательная сумма = оплата)
    var debts = getDebts();
    debts.push({
      id: uid(), debtorId: debtorId, debtorName: debtor.name,
      productCode: '', productName: 'Оплата',
      quantity: 1, amount: -amount,
      cashierName: currentUser.name, dueDate: null, status: 'open',
      note: 'Оплата через ' + (paymentMethod === 'cash' ? 'наличные' : paymentMethod === 'kaspi' ? 'Kaspi QR' : 'Банк'),
      date: dateStr
    });

    // Закрываем старые открытые долги с наименьшей суммой (FIFO)
    var openDebts = debts.filter(function (d) { return d.debtorId === debtorId && d.status === 'open' && d.amount > 0; });
    openDebts.sort(function (a, b) { return (a.date || '').localeCompare(b.date || ''); });
  ...
  function migrateDebtData() {
    ...
    try {
      ...
      // 1. Объединение дубликатов должников по имени (без учёта регистра)
      var merged = {};
      var dupMap = {};
      debtors.forEach(function (d) {
      ...
      if (changed) {
        var keepIds = {};
        Object.keys(merged).forEach(function (k) { keepIds[merged[k].id] = true; });
        debtors = debtors.filter(function (d) { return keepIds[d.id]; });
        debts = debts.map(function (d) {
      ...
      // 2. Конвертация старых paid-долгов в ledger-формат (добавляем компенсирующую запись)
      var newEntries = [];
      debts.forEach(function (d) {
        if (d.status === 'paid' && d.amount > 0 && d.productName !== 'Оплата') {
          var hasOffset = debts.some(function (x) {
            return x.debtorId === d.debtorId && x.amount < 0 && Math.abs(x.amount) === d.amount &&
                   x.date && d.date && Math.abs(new Date(x.date) - new Date(d.date)) < 120000;
          });
          if (!hasOffset) {
            newEntries.push({
              id: uid(), debtorId: d.debtorId, debtorName: d.debtorName || '',
              productCode: '', productName: 'Оплата',
              quantity: 1, amount: -(d.amount),
              cashierName: d.cashierName || '', dueDate: null, status: 'open',
      ...
      if (newEntries.length) debts = debts.concat(newEntries);
  ...
  window.renderDebtsList = function () {
    ...
    debts.forEach(function(d) {
      if (!debtorsMap[d.debtorId]) return;
    ...
    var filtered = debtors.filter(function (d) {
      if (search) {
        ...
        if (name.indexOf(search) === -1 && phone.indexOf(searchDigits) === -1 && (d.phone || '').toLowerCase().indexOf(search) === -1) return false;
      }
      return true;
    });

    filtered.sort(function(a,b) { return (b.lastActivity || '').localeCompare(a.lastActivity || ''); });

    var active = filtered.filter(function(d) { return d.totalDebt > 0; });
    var closed = filtered.filter(function(d) { return d.totalDebt <= 0; });

    function renderCards(list) {
      return list.map(function(d) {
        var actCls = (d.id === currentDebtorId) ? 'active' : '';
        return '<div class="debtor-card ' + actCls + '" onclick="selectDebtor(\'' + d.id + '\')">' +
               '<div class="debtor-card-header"><span>' + d.name + '</span><span class="debtor-card-total">' + fmt(d.totalDebt) + '</span></div>' +
               '<div class="debtor-card-phone">' + (d.phone || 'Нет телефона') + '</div>' +
               '</div>';
      }).join('');
    }
  ...
  window.selectDebtor = function(id) {
    currentDebtorId = id;
    renderDebtsList();
    renderDebtorDetail();
  };

  window.renderDebtorDetail = function() {
    ...
    var allDebts = getDebts().filter(function(d) { return d.debtorId === currentDebtorId; });
    var debts = allDebts.filter(function(d) { return d.status !== 'cancelled'; });
    var totalDebt = debts.reduce(function(sum, d) { return sum + (d.amount || 0); }, 0);
    debts.sort(function(a,b) { return (a.date || '').localeCompare(b.date || ''); });
    ...
    var msgs = debts.map(function(d) {
      ...
      if (d.quantity > 1 && !isPay) text += ' x' + d.quantity;
      var amtTxt = (isPay ? '+' : '-') + fmt(Math.abs(d.amount));
      var canDelete = d.status === 'paid' || d.status === 'cancelled' || d.amount <= 0;
      var statusBadge = d.status === 'paid' ? '<span class="badge badge-ok" style="font-size:10px">Погашен</span>' :
                        d.status === 'open' ? '<span class="badge badge-warn" style="font-size:10px">Активен</span>' :
                        d.status === 'cancelled' ? '<span class="badge badge-danger" style="font-size:10px">Отменён</span>' : '';
      
      return '<div class="msg-bubble ' + cls + '" style="position:relative">' +
             '<div class="msg-date">' + fmtDate(d.date) + (d.cashierName ? ' • ' + d.cashierName : '') + ' ' + statusBadge + '</div>' +
             '<div>' + text + '</div>' +
             '<div class="msg-amount">' + amtTxt + '</div>' +
             (canDelete ? '<button class="btn btn-sm btn-danger" style="position:absolute;top:4px;right:4px;padding:2px 6px;font-size:10px;line-height:1" onclick="event.stopPropagation();deleteDebt(\'' + d.id + '\')" title="Удалить запись">✕</button>' : '') +
             '</div>';
    }).join('');
    
    if (!msgs) msgs = '<div class="empty" style="margin:auto">История пуста</div>';
    
    var actions = '<button class="btn btn-primary" style="flex:1" onclick="openAddDebt()">+ Добавить долг</button>' +
                 '<button class="btn btn-success" style="flex:1" onclick="openPayDebt(' + totalDebt + ')">💸 Погасить долг</button>';
    if (totalDebt <= 0) {
      actions += '<button class="btn btn-danger" style="flex:1" onclick="deleteDebtor(\'' + currentDebtorId + '\')">✕ Удалить клиента</button>';
    }
    
    var html = '<div class="messenger-header">' +
               '<div><div style="font-size:18px;font-weight:600">' + debtor.name + '</div><div style="font-size:13px;color:var(--muted)">' + (debtor.phone || '') + '</div></div>' +
               '<div style="text-align:right"><div style="font-size:13px;color:var(--muted)">Общий долг</div><div style="font-size:20px;font-weight:700;color:' + (totalDebt > 0 ? 'var(--err)' : 'var(--ok)') + '">' + fmt(totalDebt) + '</div></div>' +
               '</div>' +
               '<div class="messenger-history">' + msgs + '</div>' +
               '<div class="messenger-actions">' + actions + '</div>';
  ...
  window.deleteDebtor = function (id) {
    confirmAction('Удалить клиента?', 'Клиент и вся история его долгов будут удалены безвозвратно.', function () {
      if (window.ApDb && typeof window.ApDb.deleteDebtor === 'function') {
        ...
        var debtors = getDebtors().filter(function (d) { return d.id !== id; });
        var debts = getDebts().filter(function (d) { return d.debtorId !== id; });
        setDebtors(debtors);
        setDebts(debts);
      }
      currentDebtorId = null;
  ...
  window.renderDocuments = function () {
    ...
    if (!el) return;
    if (!filtered.length) { el.innerHTML = '<div class="empty">Документов не найдено</div>'; return; }
    el.innerHTML = tableHTML(
      ['Номер', 'Тип', 'Клиент/Получатель', 'Сумма', 'Статус', 'Дата', 'Действия'],
      filtered.map(function (d) {
        var dt = d.type || d.docType || '';
        var typeLabel = dt === 'invoice' ? 'Счет на оплату' : dt === 'z2' ? 'Накладная З-2' : dt === 'invoice_sf' ? '📄 Счёт-фактура' : dt === 'deferred' ? '📋 Отложенный' : dt || '—';
        var statusLabel = d.status === 'pending' ? '<span class="badge badge-warn">Ожидает</span>'
          : d.status === 'paid' ? '<span class="badge badge-ok">Оплачено</span>'
          : d.status === 'issued' ? '<span class="badge badge-info">Выписано</span>'
          : d.status === 'cancelled' ? '<span class="badge badge-danger">Отменено</span>'
          : d.status;
        ...
        var actions = '<div style="display:flex;gap:4px;flex-wrap:nowrap">' +
          '<button class="btn btn-secondary btn-sm" onclick="openDocumentView(\'' + d.id + '\')" title="Открыть">👁</button>' +
          '<button class="btn btn-warning btn-sm" onclick="openDocumentEditById(\'' + d.id + '\')" title="Редактировать">✏️</button>' +
          '<button class="btn btn-info btn-sm" onclick="duplicateDocument(\'' + d.id + '\')" title="Дублировать">📋</button>' +
          '<button class="btn btn-info btn-sm" onclick="downloadDocumentPdf(\'' + d.id + '\')" title="Скачать PDF">📄</button>' +
          '<button class="btn btn-success btn-sm" onclick="downloadDocumentExcelById(\'' + d.id + '\')" title="Скачать Excel">📊</button>' +
          '<button class="btn btn-secondary btn-sm" onclick="printDocumentById(\'' + d.id + '\')" title="Печать">🖨️</button>' +
          '<button class="btn btn-danger btn-sm" onclick="deleteDocument(\'' + d.id + '\')" title="Удалить">❌</button>' +
          '</div>';
        return [
          '<a href="#" onclick="openDocumentView(\'' + d.id + '\');return false">' + docNum + '</a>',
          typeLabel, client, fmt(d.total || 0), statusLabel, fmtDate(d.date || d.documentDate),
          actions
        ];
      })
    );
  ...
  window.openDocumentEditById = function (docId) {
  ...
  window.deleteDocument = function (docId) {
    if (!docId) { toast('ID документа не указан', 'err'); return; }
    confirmAction('Удалить документ?', 'Документ будет удалён безвозвратно.', function () {
      if (window.ApDb && typeof window.ApDb.deleteDocument === 'function') {
        ...
        if (idx >= 0) { docs.splice(idx, 1); setDocuments(docs); }
      }
  ...
  window.duplicateDocument = function (docId) {
    if (!docId) { toast('ID документа не указан', 'err'); return; }
    var docs = getDocuments();
    var src = docs.find(function (d) { return d.id === docId; });
    if (!src) { toast('Документ не найден', 'err'); return; }
  ...
  window.downloadDocumentPdf = function (docId) {
    ...
    setTimeout(function () { printDocument(); }, 300);
  };

  window.printDocumentById = function (docId) {
    window._currentDocId = docId;
    openDocumentView(docId);
    setTimeout(function () { printDocument(); }, 300);
  };

  window.downloadDocumentExcelById = function (docId) {
    window._currentDocId = docId;
  ...
  window.openCreateInvoiceModal = function () {
    ...
    addInvoiceItemRow();
    var deferred = getDeferred().filter(function (d) { return d.status === 'pending'; });
      ...
      if (!deferred.length) {
        ...
        container.innerHTML = deferred.map(function (d) {
          var detail = (d.items || []).map(function (it) {
            return '<div style="padding:2px 0 2px 24px;font-size:0.9em;color:var(--text-muted)">' +
              esc(it.productName) + ' × ' + it.quantity + ' = ' + fmt(it.total) + '</div>';
          }).join('');
          return '<div style="padding:6px 0;border-bottom:1px solid var(--border)">' +
            '<label style="display:flex;align-items:center;gap:8px;cursor:pointer">' +
            '<input type="checkbox" value="' + d.id + '" onchange="updateInvoiceTypeUI()"> ' +
            '<strong>' + esc(d.customerName || 'Без клиента') + '</strong> — ' +
            fmt(d.total) + ' (' + (d.items ? d.items.length : 0) + ' поз.)</label>' + detail + '</div>';
        }).join('');
      }
    }
  ...
  window.openCreateZ2Modal = function () {
    ...
    if (container) {
      if (!deferred.length) {
        ...
        container.innerHTML = deferred.map(function (d) {
          ...
          return '<div style="padding:6px 0;border-bottom:1px solid var(--border)">' +
            '<label style="display:flex;align-items:center;gap:8px;cursor:pointer">' +
            '<input type="checkbox" value="' + d.id + '" onchange="updateZ2TypeUI()"> ' +
            '<strong>' + esc(d.customerName || 'Без клиента') + '</strong> — ' +
            fmt(d.total) + ' (' + (d.items ? d.items.length : 0) + ' поз.)</label>' + detail + '</div>';
        }).join('');
      }
  ...
  window.updateInvoiceTypeUI = function () {
    ...
    if (deferredSel) deferredSel.style.display = type === 'deferred' ? '' : 'none';
  };

  window.updateZ2TypeUI = function () {
  ...
  function addDocItemRow(tbodyId) {
    ...
    if (!tbody) return;
    var tr = document.createElement('tr');
    tr.innerHTML = '<td style="padding:2px;border:1px solid var(--border)"><input class="doc-item-code" style="width:100%;border:none;background:transparent;padding:4px" placeholder="Код"></td>' +
      '<td style="padding:2px;border:1px solid var(--border)"><input class="doc-item-name" style="width:100%;border:none;background:transparent;padding:4px" placeholder="Наименование товара"></td>' +
      '<td style="padding:2px;border:1px solid var(--border);text-align:center"><input class="doc-item-qty" type="number" step="any" value="1" min="0" style="width:70px;border:none;background:transparent;padding:4px;text-align:center" oninput="recalcDocItemRow(this)"></td>' +
      '<td style="padding:2px;border:1px solid var(--border);text-align:center"><input class="doc-item-price" type="number" step="any" value="0" min="0" style="width:90px;border:none;background:transparent;padding:4px;text-align:center" oninput="recalcDocItemRow(this)"></td>' +
      '<td style="padding:2px;border:1px solid var(--border);text-align:center"><span class="doc-item-total" style="font-weight:600">0</span></td>' +
      '<td style="padding:2px;border:1px solid var(--border);text-align:center"><button class="btn btn-sm btn-danger" style="padding:2px 6px;font-size:12px" onclick="this.closest(\'tr\').remove()">✕</button></td>';
    tbody.appendChild(tr);
  }

  window.addInvoiceItemRow = function () { addDocItemRow('invoice-items-tbody'); };
  window.addZ2ItemRow = function () { addDocItemRow('z2-items-tbody'); };
  window.addSFItemRow = function () { addDocItemRow('sf-items-tbody'); };
  ...
  function recalcDocItemRow(el) {
    ...
    if (totalSpan) totalSpan.textContent = fmt(qty * price);
  }
  ...
  function getDocItemsFromTable(tbodyId) {
    ...
    tbody.querySelectorAll('tr').forEach(function (tr) {
      ...
      var price = parseFloat((tr.querySelector('.doc-item-price') || {}).value) || 0;
      if (name && qty > 0) {
        items.push({ productCode: code, productName: name, quantity: qty, unitPrice: price, total: qty * price });
      }
    });
    return items;
  }

  window.saveNewInvoice = function () {
    var type = document.getElementById('invoice-type').value;
    var items = [];
    var idsToRemove = [];
    if (type === 'deferred') {
      var checks = document.querySelectorAll('#deferred-checkboxes input:checked');
      if (!checks.length) { toast('Выберите отложенные товары', 'err'); return; }
      var deferred = getDeferred();
      var checkedIds = [];
      ...
      deferred.forEach(function (d) {
        if (checkedIds.indexOf(d.id) >= 0) {
          (d.items || []).forEach(function (it) {
            items.push({ productCode: it.productCode, productName: it.productName, quantity: it.quantity, unitPrice: it.unitPrice, total: it.total });
          });
      ...
      if (!items.length) { toast('Добавьте товары в таблицу', 'err'); return; }
    }
    ...
    var templateMeta = {};
    try { var tpl = JSON.parse(localStorage.getItem('ap_selected_template') || 'null'); if (tpl) { templateMeta = { storeName: tpl.storeName, bin: tpl.bin, iik: tpl.iik, bik: tpl.bik, bankName: tpl.bankName, kbe: tpl.kbe, paymentCode: tpl.paymentCode, beneficiary: tpl.beneficiary, address: tpl.address, phone: tpl.phone, directorName: tpl.directorName, directorPosition: tpl.directorPosition, extra: tpl.extra }; } } catch (e) {}
    var docId = uid();
    if (type === 'deferred' && idsToRemove.length === 1) {
      // Update existing document from deferred (change type from 'deferred' to 'invoice')
      docId = idsToRemove[0];
      docs = docs.map(function (d) {
        if (d.id !== docId) return d;
        return Object.assign({}, d, {
          type: 'invoice', docType: 'invoice',
          items: items, clientName: customerName, clientPhone: customerPhone,
          total: items.reduce(function (s, it) { return s + (it.total || it.unitPrice * it.quantity); }, 0),
          status: 'pending', date: new Date().toISOString(), documentDate: new Date().toISOString(),
          meta: { storeInfo: templateMeta }
        });
      });
    } else if (type === 'deferred' && idsToRemove.length > 1) {
      // Multiple deferred items merged into one invoice: keep original deferred docs as completed
      docId = uid();
      docs = docs.map(function (d) {
      ...
      docs.unshift({
        id: docId, type: 'invoice', docType: 'invoice', number: '', docNumber: '',
        items: items, clientName: customerName, clientPhone: customerPhone,
        total: items.reduce(function (s, it) { return s + (it.total || it.unitPrice * it.quantity); }, 0),
        status: 'pending', date: new Date().toISOString(), documentDate: new Date().toISOString(),
        meta: { storeInfo: templateMeta }
    ...
    items.forEach(function (it) {
      docItems.push({
        id: uid(), documentId: docId, storeId: null,
        productId: null, productCode: it.productCode || '', productName: it.productName || '',
        unit: 'шт', quantity: it.quantity, unitPrice: it.unitPrice, total: it.total,
  ...
  window.saveNewZ2 = function () {
    ...
    var docId = uid();
    if (type === 'deferred' && idsToRemove.length === 1) {
      // Update existing document from deferred (change type from 'deferred' to 'z2')
      docId = idsToRemove[0];
      docs = docs.map(function (d) {
        if (d.id !== docId) return d;
        return Object.assign({}, d, {
          type: 'z2', docType: 'z2',
          items: items, recipientName: recipientName,
          total: items.reduce(function (s, it) { return s + (it.total || it.unitPrice * it.quantity); }, 0),
          status: 'pending', date: new Date().toISOString(), documentDate: new Date().toISOString(),
          meta: { storeInfo: templateMeta }
        });
      ...
      docs.unshift({
        id: docId, type: 'z2', docType: 'z2', number: '', docNumber: '',
        items: items, recipientName: recipientName,
        total: items.reduce(function (s, it) { return s + (it.total || it.unitPrice * it.quantity); }, 0),
        status: 'pending', date: new Date().toISOString(), documentDate: new Date().toISOString(),
        meta: { storeInfo: templateMeta }
  ...
  // ─── СЧЁТ-ФАКТУРА ───
  window.openCreateSFFModal = function () {
  ...
  window.saveNewSFF = function () {
    ...
    if (!customerName) { toast('Введите покупателя', 'err'); return; }
    if (!items.length) { toast('Добавьте товары', 'err'); return; }
    var templateMeta = {};
    try { var tpl = JSON.parse(localStorage.getItem('ap_selected_template') || 'null'); if (tpl) { templateMeta = { storeName: tpl.storeName, bin: tpl.bin, iik: tpl.iik, bik: tpl.bik, bankName: tpl.bankName, kbe: tpl.kbe, paymentCode: tpl.paymentCode, beneficiary: tpl.beneficiary, address: tpl.address, phone: tpl.phone, directorName: tpl.directorName, directorPosition: tpl.directorPosition, extra: tpl.extra }; } } catch (e) {}
    var docId = uid();
    var docs = getDocuments();
    var docItems = getDocumentItems();
    var total = items.reduce(function(s, it) { return s + (it.total || it.unitPrice * it.quantity); }, 0);
    var doc = {
      id: docId, type: 'invoice_sf', docType: 'invoice_sf', number: '', docNumber: '',
      items: items, customerName: customerName,
      customerIIN: customerIIN, customerAddress: customerAddress,
      total: total, status: 'pending',
      date: new Date().toISOString(), documentDate: new Date().toISOString(),
      meta: { customerIIN: customerIIN, customerAddress: customerAddress, storeInfo: templateMeta }
    };
    docs.unshift(doc);
    setDocuments(docs);
    items.forEach(function(it) {
  ...
  // ─── ПКО ───
  window.openCreatePKOModal = function () {
  ...
  window.saveNewPKO = function () {
    ...
    if (!payer) { toast('Введите от кого получено', 'err'); return; }
    if (!basis) { toast('Введите основание', 'err'); return; }
    if (amount <= 0) { toast('Введите сумму', 'err'); return; }
    var docId = uid();
    var docs = getDocuments();
    var doc = {
      id: docId, type: 'pko', docType: 'pko', number: '', docNumber: '',
      customerName: payer, basis: basis,
      total: amount, status: 'issued',
      date: new Date().toISOString(), documentDate: new Date().toISOString(),
      meta: {}
    };
    docs.unshift(doc);
  ...
  // ─── Classic document HTML builder (all template fields) ───
  window.classicAmountWords = function (n) {
    ...
    function numWords(num) {
      if (num === 0) return '';
      var res = '';
      if (num >= 1000) { res += numWords(Math.floor(num / 1000)) + ' тысяча '; num %= 1000; }
      // crude — good enough for typical totals
      if (num >= 100) { res += hundreds[Math.floor(num / 100)] + ' '; num %= 100; }
      if (num >= 20) { res += tens[Math.floor(num / 10)] + ' '; num %= 10; }
      else if (num >= 10) { res += teens[num - 10] + ' '; return res; }
  ...
  window.buildClassicInvoiceHTML = function (doc) {
    ...
    var totalQty = 0;
    (doc.items || []).forEach(function (it) { totalQty += Number(it.quantity || 0); });
    ...
    h += '<div style="font-size:11px;line-height:1.4;padding:8px 12px;border:1px solid #000;background:#fafafa;margin-bottom:16px">' +
      'Внимание! Оплата данного счета означает согласие с условиями поставки товара. Уведомление об оплате обязательно, в противном случае не гарантируется наличие товара на складе. Товар отпускается по факту прихода денег на р/с Поставщика, самовывозом, при наличии доверенности и документов, удостоверяющих личность.' +
      '</div>';
    h += '<div style="text-align:center;margin-bottom:20px"><div style="font-weight:700;font-size:20px;letter-spacing:1px">Счет на оплату № ' + escapeHtml(doc.docNumber || '') + '</div>' +
      '<div style="margin-top:4px;font-size:13px">от ' + fmtDate(doc.documentDate) + '</div></div>';

    h += '<div style="margin-bottom:12px">';
    h += '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;border:1px solid #000;padding:8px;font-size:12px">' +
      ...
      '</div>';
    h += '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;border:1px solid #000;border-top:none;padding:8px;font-size:12px">' +
      '<div><strong>ИИН:</strong> ' + escapeHtml(bin) + '</div>' +
      '<div></div><div></div>' +
      '</div>';
    h += '<div style="display:grid;grid-template-columns:2fr 1fr 2fr;gap:8px;border:1px solid #000;border-top:none;padding:8px;font-size:12px">' +
      '<div><strong>Банк бенефициара:</strong></div>' +
      '<div><strong>БИК:</strong></div>' +
      '<div><strong>Код назначения платежа:</strong></div>' +
      '</div>';
    h += '<div style="display:grid;grid-template-columns:2fr 1fr 2fr;gap:8px;border:1px solid #000;border-top:none;padding:8px;font-size:12px">' +
      '<div>' + escapeHtml(bankName) + '</div>' +
      '<div>' + escapeHtml(bik) + '</div>' +
      '<div>' + (paymentCode ? escapeHtml(paymentCode) : '') + '</div>' +
      '</div>';
    h += '</div>';
    ...
    if (custIIN) buyerStr = 'БИН / ИИН ' + escapeHtml(custIIN) + ',' + buyerStr;
    if (custAddr) buyerStr += ' ' + escapeHtml(custAddr);
    ...
    h += '<thead><tr>' +
      '<th style="padding:8px;border:1px solid #000;text-align:center;width:36px">№</th>' +
      '<th style="padding:8px;border:1px solid #000;text-align:left">Наименование</th>' +
      '<th style="padding:8px;border:1px solid #000;text-align:center;width:60px">Кол-во</th>' +
      '<th style="padding:8px;border:1px solid #000;text-align:center;width:50px">Ед.</th>' +
      '<th style="padding:8px;border:1px solid #000;text-align:right;width:100px">Цена</th>' +
      '<th style="padding:8px;border:1px solid #000;text-align:right;width:120px">Сумма</th>' +
      '</tr></thead><tbody>';
    (doc.items || []).forEach(function (it, i) {
      h += '<tr>' +
        '<td style="padding:8px;border:1px solid #000;text-align:center">' + (i + 1) + '</td>' +
        '<td style="padding:8px;border:1px solid #000">' + escapeHtml(it.productName || it.productCode || '') + '</td>' +
        '<td style="padding:8px;border:1px solid #000;text-align:center">' + (it.quantity || 0) + '</td>' +
        '<td style="padding:8px;border:1px solid #000;text-align:center">' + escapeHtml(it.unit || 'шт') + '</td>' +
        '<td style="padding:8px;border:1px solid #000;text-align:right">' + fmt(it.unitPrice) + '</td>' +
        '<td style="padding:8px;border:1px solid #000;text-align:right">' + fmt(it.total) + '</td>' +
        '</tr>';
    });
    h += '</tbody></table>';
  ...
  window.buildClassicZ2HTML = function (doc) {
    var si = (doc.meta && doc.meta.storeInfo) || {};
    var storeName = si.storeName || 'Организация';
    var bin = si.bin || '—';
    var address = si.address || '';
    ...
    h += '<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">' +
      ...
      '<div style="font-size:16px;font-weight:700">Форма 3-2</div></div>';
    ...
    if (address) senderLines += '<br><strong>Адрес:</strong> ' + escapeHtml(address);
    if (phone) senderLines += '<br><strong>Тел:</strong> ' + escapeHtml(phone);
    if (directorName) senderLines += '<br><strong>' + escapeHtml(directorPosition) + ':</strong> ' + escapeHtml(directorName);
    if (extraInfo) senderLines += '<br><em>' + escapeHtml(extraInfo) + '</em>';
    ...
    if (custIIN) recHtml += '<br><strong>БИН/ИИН:</strong> ' + escapeHtml(custIIN);
    if (custAddr) recHtml += '<br>' + escapeHtml(custAddr);
    h += '<div>' + recHtml + '</div>';
    ...
    h += '<thead><tr>' +
      '<th style="padding:8px;border:1px solid #000;text-align:center;width:36px">№ п/п</th>' +
      '<th style="padding:8px;border:1px solid #000;text-align:center;width:100px">Номенклатурный номер</th>' +
      '<th style="padding:8px;border:1px solid #000;text-align:left">Наименование, характеристика</th>' +
      '<th style="padding:8px;border:1px solid #000;text-align:center;width:50px">Ед.изм.</th>' +
      '<th style="padding:8px;border:1px solid #000;text-align:right;width:70px">Количество</th>' +
    ...
    (doc.items || []).forEach(function (it, i) {
      h += '<tr>' +
        '<td style="padding:8px;border:1px solid #000;text-align:center">' + (i + 1) + '</td>' +
        '<td style="padding:8px;border:1px solid #000;text-align:center">' + escapeHtml(it.productCode || '') + '</td>' +
        '<td style="padding:8px;border:1px solid #000;text-align:left">' + escapeHtml(it.productName || '') + '</td>' +
        '<td style="padding:8px;border:1px solid #000;text-align:center">' + escapeHtml(it.unit || 'шт') + '</td>' +
        '<td style="padding:8px;border:1px solid #000;text-align:right">' + (it.quantity || 0) + '</td>' +
        '<td style="padding:8px;border:1px solid #000;text-align:right">' + fmt(it.unitPrice) + '</td>' +
        '<td style="padding:8px;border:1px solid #000;text-align:right">' + fmt(it.total) + '</td>' +
        '</tr>';
    });
    ...
    h += '<div style="display:flex;justify-content:space-between;margin-bottom:16px;font-size:13px">' +
      '<div><strong>Итого наименований:</strong> ' + (doc.items || []).length + ', <strong>на сумму:</strong> ' + fmt(doc.total) + '</div>' +
    ...
    // Left side: 3 signature blocks stacked
    h += '<div style="flex:1;min-width:300px">';
    // 1. Отпуск разрешил
    h += '<div style="border:1px solid #000;padding:10px;margin-bottom:8px">' +
      '<div style="margin-bottom:6px"><strong>Отпуск разрешил:</strong></div>' +
      '<div style="display:flex;justify-content:space-between;margin-bottom:4px">' +
      '<span style="font-size:11px">' + escapeHtml(directorPosition) + '</span>' +
      '<span style="font-size:11px">_______________</span>' +
      '<span style="font-size:11px">_______________</span>' +
      '</div>' +
      '<div style="font-size:10px;text-align:right">должность&nbsp;&nbsp;&nbsp;подпись&nbsp;&nbsp;&nbsp;расшифровка подписи</div>' +
      '</div>';
    // 2. Главный бухгалтер
    h += '<div style="border:1px solid #000;padding:10px;margin-bottom:8px">' +
      '<div style="margin-bottom:6px"><strong>Главный бухгалтер:</strong></div>' +
      '<div style="display:flex;justify-content:flex-end;gap:20px;margin-bottom:4px">' +
      '<span style="font-size:11px">_______________</span>' +
      '<span style="font-size:11px">_______________</span>' +
      '</div>' +
      '<div style="font-size:10px;text-align:right">подпись&nbsp;&nbsp;&nbsp;расшифровка подписи</div>' +
      '</div>';
    // 3. Отпустил
    h += '<div style="border:1px solid #000;padding:10px">' +
      '<div style="margin-bottom:6px"><strong>Отпустил:</strong></div>' +
      '<div style="display:flex;justify-content:flex-end;gap:20px;margin-bottom:4px">' +
      '<span style="font-size:11px">_______________</span>' +
      '<span style="font-size:11px">_______________</span>' +
      '</div>' +
      ...
      '</div>';
    h += '</div>';
    // Right side: 1 signature block
    h += '<div style="flex:1;min-width:300px">';
    h += '<div style="border:1px solid #000;padding:10px">' +
      '<div style="margin-bottom:6px"><strong>Запасы получил:</strong></div>' +
      '<div style="display:flex;justify-content:flex-end;gap:20px;margin-bottom:4px">' +
      '<span style="font-size:11px">_______________</span>' +
      '<span style="font-size:11px">_______________</span>' +
      '</div>' +
      '<div style="font-size:10px;text-align:right;margin-bottom:8px">подпись&nbsp;&nbsp;&nbsp;расшифровка подписи</div>' +
      '<div style="font-size:11px;border-top:1px solid #ccc;padding-top:6px">' +
      'по доверенности № ________________ от ___ _________ 20__ года<br>выданной __________________________________________________' +
      '</div>' +
      '</div>';
    h += '</div>';
    h += '</div>';
  ...
  window.buildClassicSFHTML = function (doc) {
    var store = window.ApAuth && window.ApAuth.getCurrentStore();
    var si = (doc.meta && doc.meta.storeInfo) || {};
    ...
    function ndsRow(v) { return v * ndsRate / (100 + ndsRate); }
    function ndsOnly(v) { return v - ndsRow(v); }
    ...
    h += '<div style="text-align:center;font-size:12px;color:#333;margin-bottom:16px">Счёт-фактура № ' + (doc.docNumber || doc.number || doc.id.slice(0, 6)) +
      ' от "' + fmtDate(doc.documentDate).replace(/(\d+)\.(\d+)\.(\d+).*/, '$1 $2 $3') + '"</div>';
    ...
    h += '</table>';
    if (iik || bik || bankName) {
      h += '<div style="font-size:11px;margin-top:4px;padding:4px;background:#f5f5f5">' +
        '<strong>Банковские реквизиты:</strong> ' +
        (iik ? 'ИИК ' + escapeHtml(iik) + '; ' : '') +
        (bik ? 'БИК ' + escapeHtml(bik) + '; ' : '') +
        (bankName ? escapeHtml(bankName) : '') +
        '</div>';
    }
    h += '</div>';
    ...
    h += '<thead><tr>' +
      '<th style="padding:5px 4px;border:1.5px solid #000;text-align:center;width:30px">№</th>' +
      '<th style="padding:5px 4px;border:1.5px solid #000;text-align:left">Наименование товара</th>' +
      '<th style="padding:5px 4px;border:1.5px solid #000;text-align:center;width:40px">Ед.</th>' +
      '<th style="padding:5px 4px;border:1.5px solid #000;text-align:center;width:60px">Кол-во</th>' +
      '<th style="padding:5px 4px;border:1.5px solid #000;text-align:right;width:90px">Цена, ₸</th>' +
      '<th style="padding:5px 4px;border:1.5px solid #000;text-align:right;width:100px">Стоимость, ₸</th>' +
      '<th style="padding:5px 4px;border:1.5px solid #000;text-align:center;width:45px">НДС%</th>' +
      '<th style="padding:5px 4px;border:1.5px solid #000;text-align:right;width:100px">Сумма НДС, ₸</th>' +
      '<th style="padding:5px 4px;border:1.5px solid #000;text-align:right;width:110px">Итого с НДС, ₸</th>' +
      '</tr></thead><tbody>';
    if (!items.length) {
      ...
      items.forEach(function (it, i) {
        ...
        totalNds += ndsAmt;
        h += '<tr>' +
          '<td style="padding:4px;border:1.5px solid #000;text-align:center">' + (i + 1) + '</td>' +
          '<td style="padding:4px;border:1.5px solid #000;text-align:left">' + escapeHtml(it.productName || '') + '</td>' +
          '<td style="padding:4px;border:1.5px solid #000;text-align:center">' + (it.unit || 'шт') + '</td>' +
          '<td style="padding:4px;border:1.5px solid #000;text-align:center">' + (it.quantity || 0) + '</td>' +
          '<td style="padding:4px;border:1.5px solid #000;text-align:right">' + fmt(priceNoNds) + '</td>' +
          '<td style="padding:4px;border:1.5px solid #000;text-align:right">' + fmt(ndsOnly(cost)) + '</td>' +
          '<td style="padding:4px;border:1.5px solid #000;text-align:center">' + ndsRate + '%</td>' +
          '<td style="padding:4px;border:1.5px solid #000;text-align:right">' + fmt(ndsAmt) + '</td>' +
          '<td style="padding:4px;border:1.5px solid #000;text-align:right;font-weight:600">' + fmt(incNds) + '</td>' +
          '</tr>';
      });
    }
    h += '</tbody></table>';
    ...
    h += '<div style="display:flex;justify-content:space-between;max-width:600px;margin-bottom:4px">' +
      '<span><strong>Руководитель / ИП:</strong> ______________________</span>' +
      '<span><strong>Главный бухгалтер:</strong> ______________________</span>' +
      '</div>';
  ...
  window.buildClassicPKOHTML = function (doc) {
    var h = '<div style="font-family:\'Times New Roman\',Times,serif;color:#000;max-width:700px;margin:0 auto;padding:24px 20px;font-size:13px;line-height:1.4">';
    h += '<div style="text-align:center;font-size:16px;font-weight:700;margin-bottom:8px">ПРИХОДНЫЙ КАССОВЫЙ ОРДЕР</div>';
    h += '<div style="display:flex;justify-content:space-between;margin-bottom:16px">' +
      '<span><strong>№:</strong> ' + (doc.docNumber || doc.number || doc.id.slice(0, 6)) + '</span>' +
      '<span><strong>Дата:</strong> ' + fmtDate(doc.documentDate) + '</span>' +
      '</div>';
    h += '<div style="border:1px solid #000;padding:16px;margin-bottom:16px">';
  ...
  window.openDocumentView = function (docId) {
    ...
    if (dt === 'invoice') {
      if (content) content.innerHTML = window.buildClassicInvoiceHTML(adapted);
      if (title) title.textContent = 'Счет на оплату №' + (doc.number || doc.docNumber || doc.id.slice(0, 6));
    } else if (dt === 'z2') {
      if (content) content.innerHTML = window.buildClassicZ2HTML(adapted);
      if (title) title.textContent = 'Накладная З-2 №' + (doc.number || doc.docNumber || doc.id.slice(0, 6));
    } else if (dt === 'invoice_sf') {
      if (content) content.innerHTML = window.buildClassicSFHTML(adapted);
      if (title) title.textContent = 'Счёт-фактура №' + (doc.number || doc.docNumber || doc.id.slice(0, 6));
    } else if (dt === 'pko') {
      if (content) content.innerHTML = window.buildClassicPKOHTML(adapted);
      if (title) title.textContent = 'ПКО №' + (doc.number || doc.docNumber || doc.id.slice(0, 6));
    } else {
      if (title) title.textContent = (dt === 'invoice' ? 'Счет на оплату' : 'Накладная З-2') + ' №' + (doc.number || doc.docNumber || doc.id.slice(0, 6));
      if (content) {
        var clientInfo = dt === 'invoice'
          ? '<p><strong>Клиент:</strong> ' + esc(doc.clientName || doc.customerName || '—') + (doc.clientPhone || doc.customerPhone ? ' (' + esc(doc.clientPhone || doc.customerPhone) + ')' : '') + '</p>'
          : '<p><strong>Получатель:</strong> ' + esc(doc.recipientName || '—') + '</p>';
        ...
        content.innerHTML =
          '<div style="margin-bottom:16px">' + clientInfo +
          '<p><strong>Дата:</strong> ' + fmtDate(doc.date || doc.documentDate) + '</p>' +
          '<p><strong>Статус:</strong> <span class="badge badge-' + (doc.status === 'paid' || doc.status === 'issued' ? 'ok' : doc.status === 'cancelled' ? 'danger' : 'warn') + '">' + statusLabel + '</span></p>' +
          '<p><strong>Итого:</strong> ' + fmt(doc.total || 0) + ' ₸</p></div>' +
          '<div class="panel"><div class="panel-title">Товары</div>' + itemsHtml + '</div>';
      }
    }
    ...
    if (statusBtn) {
      if (dt === 'invoice' || dt === 'invoice_sf') {
        ...
        statusBtn.onclick = function () { changeDocumentStatus(docId, 'paid'); };
      } else if (dt === 'pko') {
        statusBtn.style.display = 'none';
      } else {
        statusBtn.style.display = doc.status === 'pending' ? '' : 'none';
        statusBtn.textContent = '📦 Отметить выписанным';
        statusBtn.onclick = function () { changeDocumentStatus(docId, 'issued'); };
      }
    }
    openModal('modal-view-document');
  };

  window.changeDocumentStatus = function (docId, status) {
    var docs = getDocuments();
    var idx = docs.findIndex(function (d) { return d.id === docId; });
    if (idx < 0) { toast('Документ не найден', 'err'); return; }
    docs[idx].status = status;
    setDocuments(docs);
    toast('Статус обновлен', 'ok');
    renderDocuments();
    if (window._currentDocId === docId) openDocumentView(docId);
  };
  ...
  window.downloadDocumentExcel = function () {
    var docId = window._currentDocId;
    if (!docId) { toast('Нет открытого документа', 'err'); return; }
    var docs = getDocuments();
    var doc = docs.find(function (d) { return d.id === docId; });
    if (!doc) { toast('Документ не найден', 'err'); return; }
    try {
      ...
      var docDate = (doc.date || doc.documentDate) ? new Date(doc.date || doc.documentDate).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' }) : '—';
      var totalRaw = doc.total || 0;
      var items = doc.items || [];
      var totalQty = items.reduce(function (s, it) { return s + Number(it.quantity || 0); }, 0);

      // ── Excel export matching user's photo spec ──
      var wb = XLSX.utils.book_new();
      var data = [], merges = [];
      function M(r1, c1, r2, c2) { merges.push({ s: { r: r1, c: c1 }, e: { r: r2, c: c2 } }); }
      function cellAt(arr, r, c, val) { if (!arr[r]) arr[r] = []; arr[r][c] = val; }
      function fmtCell(v, opts) { return { v: v, s: opts || {} }; }
      function fnt(sz, b, i, n) { return { font: { sz: sz || 10, bold: b || false, italic: i || false, name: 'Arial' }, alignment: { horizontal: n && n.h || 'left', vertical: n && n.v || 'center', wrapText: n && n.w || false } }; }

      function tin(s) { return { style: s || 'thin' }; }
      ...
      function buildSheetFromData(data, merges) {
        ...
        var refEnd = XLSX.utils.encode_cell({ r: Math.max(maxR, 0), c: Math.max(maxC, 0) });
        ws['!ref'] = 'A1:' + refEnd;
        if (merges && merges.length) ws['!merges'] = merges;
        return ws;
      }
      function bdr(t,r,b,l) { return { top: t||{}, bottom: b||{}, left: l||{}, right: r||{} }; }
      if (isInvoice) {
        // ── INVOICE: 39 cols (A-AM) ──
        var C = { B:1, D:3, E:4, F:5, G:6, S:18, T:19, U:20, V:21, W:22, X:23, Y:24, Z:25, AA:26, AB:27, AC:28, AD:29, AE:30, AF:31, AG:32, AH:33, AI:34, AJ:35, AK:36, AL:37, AM:38 };
        // Row 0-5: Warning text G1:AM6 in bordered box
        cellAt(data, 0, C.G, { v: 'Внимание! Оплата данного счета означает согласие с условиями поставки товара. Уведомление об оплате обязательно, в противном случае не гарантируется наличие товара на складе. Товар отпускается по факту прихода денег на р/с Поставщика, самовывозом, при наличии доверенности и документов удостоверяющих личность.', s: { font: fnt(8,false,false,{h:'center',v:'center',w:true}).font, alignment: fnt(8,false,false,{h:'center',v:'center',w:true}).alignment, border: { top: med(), bottom: med(), left: med(), right: med() } } });
        M(0, C.G, 5, C.AM);
        // Row 8: "Образец платежного поручения"
        cellAt(data, 7, C.B, fmtCell('Образец платежного поручения', fnt(10, true, false, {h:'left',v:'center'})));
        // Row 9-13: Bank details with borders (thin, matching spec)
        function bb(top, right, bottom, left) {
          var b = {};
          if (top) b.top = tin(); if (right) b.right = tin();
          if (bottom) b.bottom = tin(); if (left) b.left = tin();
          return b;
        }
        // Row 9 headers
        cellAt(data, 8, C.B, { v: 'Бенефициар:', s: { font: fnt(9,true).font, alignment: fnt(9,true,false,{h:'left',v:'top'}).alignment, border: bb(1,0,0,1) } }); M(8, C.B, 8, C.U);
        cellAt(data, 8, C.V, { v: 'ИИК', s: { font: fnt(9,true).font, alignment: fnt(9,true,false,{h:'center',v:'top'}).alignment, border: bb(1,0,0,1) } }); M(8, C.V, 8, C.AE);
        cellAt(data, 8, C.AF, { v: 'Кбе', s: { font: fnt(9,true).font, alignment: fnt(9,true,false,{h:'center',v:'top'}).alignment, border: bb(1,1,0,1) } }); M(8, C.AF, 8, C.AM);
        // Row 10 values
        cellAt(data, 9, C.B, { v: beneficiary, s: { font: fnt(9,true).font, alignment: fnt(9,true,false,{h:'left',v:'top',w:true}).alignment, border: bb(0,0,0,1) } }); M(9, C.B, 9, C.U);
        cellAt(data, 9, C.V, { v: iik, s: { font: fnt(9,true).font, alignment: fnt(9,true,false,{h:'center',v:'center',w:true}).alignment, border: bb(0,0,0,1) } }); M(9, C.V, 9, C.AC);
        cellAt(data, 9, C.AF, { v: kbe, s: { font: fnt(9,true).font, alignment: fnt(9,true,false,{h:'center',v:'center',w:true}).alignment, border: bb(0,1,0,1) } }); M(9, C.AF, 10, C.AM);
        // Row 11 ИИН
        cellAt(data, 10, C.B, { v: 'ИИН: ' + bin, s: { font: fnt(9,false).font, alignment: fnt(9,false,false,{h:'left',v:'top'}).alignment, border: bb(0,0,0,1) } }); M(10, C.B, 10, C.U);
        // Row 12 headers
        cellAt(data, 11, C.B, { v: 'Банк бенефициара:', s: { font: fnt(9,false).font, alignment: fnt(9,false,false,{h:'left',v:'top'}).alignment, border: bb(1,0,0,1) } }); M(11, C.B, 11, C.U);
        cellAt(data, 11, C.V, { v: 'БИК', s: { font: fnt(9,true).font, alignment: fnt(9,true,false,{h:'center',v:'top'}).alignment, border: bb(1,0,0,1) } }); M(11, C.V, 11, C.AC);
        cellAt(data, 11, C.AD, { v: 'Код назначения платежа', s: { font: fnt(9,true).font, alignment: fnt(9,true,false,{h:'center',v:'top'}).alignment, border: bb(1,1,0,1) } }); M(11, C.AD, 11, C.AM);
        // Row 13 values
        cellAt(data, 12, C.B, { v: bankName, s: { font: fnt(9,false).font, alignment: fnt(9,false,false,{h:'left',v:'top',w:true}).alignment, border: bb(0,0,1,1) } }); M(12, C.B, 12, C.U);
        cellAt(data, 12, C.V, { v: bik, s: { font: fnt(9,true).font, alignment: fnt(9,true,false,{h:'center',v:'top',w:true}).alignment, border: bb(0,0,1,1) } }); M(12, C.V, 12, C.AC);
        // Row 16: Title B16:AM17
        ...
        cellAt(data, 15, C.B, fmtCell('Счет на оплату № ' + docNumber + ' от ' + d.getDate() + ' ' + ms[d.getMonth()] + ' ' + d.getFullYear() + ' г.', fnt(14,true,false,{h:'left',v:'center'})));
        M(15, C.B, 16, C.AM);
        // Row 18: empty separator
        cellAt(data, 17, C.B, fmtCell('', fnt(9)));
        M(17, C.B, 17, C.AM);
        // Row 20: Поставщик
        cellAt(data, 19, C.B, fmtCell('Поставщик:', fnt(9,false,false,{h:'left',v:'top'}))); M(19, C.B, 19, C.E);
        cellAt(data, 19, C.F, fmtCell('БИН / ИИН ' + bin + ', ' + storeName, fnt(9,true,false,{h:'left',v:'top',w:true}))); M(19, C.F, 19, C.AM);
        // Row 22: Покупатель
        cellAt(data, 21, C.B, fmtCell('Покупатель:', fnt(9,false,false,{h:'left',v:'top'}))); M(21, C.B, 21, C.E);
        var bStr = customerName;
        if (custIIN) bStr = 'БИН / ИИН ' + custIIN + ', ' + bStr;
        if (custAddr) bStr += ' ' + custAddr;
        cellAt(data, 21, C.F, fmtCell(bStr, fnt(9,true,false,{h:'left',v:'top',w:true}))); M(21, C.F, 21, C.AM);
        // Row 24: Договор
        cellAt(data, 23, C.B, fmtCell('Договор:', fnt(10,false,false,{h:'left',v:'top'}))); M(23, C.B, 23, C.E);
        cellAt(data, 23, C.F, fmtCell(contractText, fnt(10,true,false,{h:'left',v:'top',w:true}))); M(23, C.F, 23, C.AM);
        // Row 26: Table header with borders medium outer / thin inner
        function thdr(rr, txt, c1, c2, leftB, rightB) {
          var b = { top: med(), bottom: tin('thin') };
          if (leftB) b.left = med(); else b.left = tin();
          if (rightB) b.right = med(); else b.right = tin();
          cellAt(data, rr, c1, { v: txt, s: { font: fnt(9,true).font, alignment: fnt(9,true,false,{h:'center',v:'center'}).alignment, border: b } });
          if (c2 > c1) M(rr, c1, rr, c2);
        }
        ...
        items.forEach(function (it, idx) {
          ...
          function tcell(rrr, c1, c2, txt, al, leftM, rightM) {
            var b = { top: tin(), bottom: tin() };
            if (leftM) b.left = med(); else b.left = tin();
            if (rightM) b.right = med(); else b.right = tin();
            cellAt(data, rrr, c1, { v: txt, s: { font: fnt(8,false).font, alignment: { horizontal: al||'left', vertical: 'top', wrapText: true }, border: b } });
            if (c2 > c1) M(rrr, c1, rrr, c2);
          }
        ...
        function totCell(rrr, c1, c2, txt, leftM, rightM) {
          var b = { top: med(), bottom: med() };
          if (leftM) b.left = med(); else b.left = tin();
          if (rightM) b.right = med(); else b.right = tin();
          cellAt(data, rrr, c1, { v: txt, s: { font: fnt(9,true).font, alignment: { horizontal: 'right', vertical: 'top' }, border: b } });
          if (c2 > c1) M(rrr, c1, rrr, c2);
        }
        ...
        // Row 37: total info
        rr += 3;
        cellAt(data, rr, C.B, fmtCell('Всего наименований ' + items.length + ', на сумму ' + Number(totalRaw).toLocaleString('ru-RU',{minimumFractionDigits:2}) + ' KZT', fnt(9,false,false,{h:'left',v:'top'})));
        M(rr, C.B, rr, C.AM);
        // Row 38: amount in words
        rr++;
        cellAt(data, rr, C.B, fmtCell('Всего к оплате: ' + window.classicAmountWords(totalRaw), fnt(9,true,false,{h:'left',v:'top',w:true})));
        M(rr, C.B, rr, C.AK);
        // Row 40-41: Signature
        rr += 2;
        cellAt(data, rr, C.B, fmtCell('Исполнитель', fnt(9,true,false,{h:'left',v:'center'})));
        cellAt(data, rr, C.G, fmtCell('', fnt(8)));
        M(rr, C.G, rr, C.V);
        cellAt(data, rr, C.W, fmtCell('/' + (directorName || '________') + '/', fnt(8,false,false,{h:'left',v:'center'})));
        M(rr, C.W, rr, C.AM);
        // Build sheet
        var ws = buildSheetFromData(data, merges);
        ws['!cols'] = [{wch:2},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3},{wch:3}];
        ws['!print'] = { paperSize: 9, orientation: 'portrait' };
        XLSX.utils.book_append_sheet(wb, ws, 'Лист_1');
      } else {
        // ── Z2 NAKLADNAYA: 49 cols (A-AW) ──
        var C = {};
        for (var ci = 0; ci <= 48; ci++) {
          if (ci < 26) C[String.fromCharCode(65 + ci)] = ci;
          else C['A' + String.fromCharCode(65 + ci - 26)] = ci;
        }
        // Block 1: Приложение 26 at AN1:AW4 (right upper corner)
        var appLines = ['Приложение 26', 'к приказу Министра финансов', 'Республики Казахстан', 'от 20 декабря 2012 года № 562'];
        appLines.forEach(function (l, i) {
          cellAt(data, i, C.AN, fmtCell(l, fnt(8,false,true,{h:'center',v:'center'})));
          M(i, C.AN, i, C.AW);
        });
        // Row 5: Форма З-2 label
        cellAt(data, 4, C.AQ, fmtCell('Форма З-2', fnt(9,false,false,{h:'center',v:'center'})));
        M(4, C.AQ, 4, C.AW);
        // Row 9: Organization header (IIN at AQ9:AW9 per spec)
        cellAt(data, 8, C.A, fmtCell('Организация (индивидуальный предприниматель)', fnt(8,false,false,{h:'left',v:'bottom',w:true})));
        M(8, C.A, 8, C.M);
        cellAt(data, 8, C.N, fmtCell(storeName, fnt(9,true,false,{h:'center',v:'top',w:true})));
        M(8, C.N, 8, C.AJ);
        cellAt(data, 8, C.AQ, fmtCell(bin, fnt(9,true,false,{h:'center',v:'center'})));
        // Row 12-13: Номер документа / Дата
        cellAt(data, 11, C.AP, fmtCell('Номер документа', fnt(8,false,false,{h:'center',v:'center',w:true})));
        cellAt(data, 11, C.AT, fmtCell('Дата составления', fnt(8,false,false,{h:'center',v:'center',w:true})));
        cellAt(data, 12, C.AP, fmtCell(docNumber, fnt(8,true,false,{h:'center',v:'center',w:true})));
        cellAt(data, 12, C.AT, fmtCell(docDate, fnt(8,true,false,{h:'center',v:'center',w:true})));
        // Row 15: Title full width
        cellAt(data, 14, C.A, fmtCell('НАКЛАДНАЯ НА ОТПУСК ЗАПАСОВ НА СТОРОНУ', fnt(10,true,false,{h:'center',v:'center'})));
        M(14, C.A, 14, C.AW);
        // Row 18-19: Sender/Receiver/Responsible
        ...
        function z2ValBdr(leftM, rightM) {
          var b = { top: tin('thin'), bottom: tin('thin') };
          if (leftM) b.left = med(); else b.left = tin();
          if (rightM) b.right = med(); else b.right = tin();
          return b;
        }
        var hdrs18 = [
          {t:'Организация (ИП) - отправитель', c1:C.A, c2:C.K, l:true, r:false},
          {t:'Организация (ИП) - получатель', c1:C.L, c2:C.V, l:false, r:false},
          {t:'Ответственный за поставку (Ф.И.О.)', c1:C.W, c2:C.AE, l:false, r:false},
          {t:'Транспортная организация', c1:C.AF, c2:C.AN, l:false, r:false},
          {t:'Товарно-транспортная накладная (номер, дата)', c1:C.AO, c2:C.AW, l:false, r:true}
        ];
        hdrs18.forEach(function (h) {
          cellAt(data, 17, h.c1, { v: h.t, s: { font: fnt(8,false).font, alignment: fnt(8,false,false,{h:'center',v:'center',w:true}).alignment, border: z2HdrBdr(h.l, h.r) } });
          M(17, h.c1, 17, h.c2);
        });
        var vals19 = [
          {t:storeName, c1:C.A, c2:C.K, l:true, r:false},
          {t:customerName, c1:C.L, c2:C.V, l:false, r:false},
          {t:directorName, c1:C.W, c2:C.AE, l:false, r:false},
          {t:'', c1:C.AF, c2:C.AN, l:false, r:false},
          {t:'', c1:C.AO, c2:C.AW, l:false, r:true}
        ];
        vals19.forEach(function (h) {
          cellAt(data, 18, h.c1, { v: h.t, s: { font: fnt(8,false).font, alignment: fnt(8,false,false,{h:'center',v:'center',w:true}).alignment, border: z2ValBdr(h.l, h.r) } });
          M(18, h.c1, 18, h.c2);
        });
        // Row 21-22: Two-level table headers
        function z2TblHdrBdr(leftM, rightM, isTop) {
          var b = { top: isTop ? med() : tin('thin'), bottom: tin('thin') };
          if (leftM) b.left = med(); else b.left = tin();
          if (rightM) b.right = med(); else b.right = tin();
          return b;
        }
        var tblH = [
          {t:'Номер по порядку', r1:20, r2:21, c1:C.A, c2:C.B, l:true, r:false},
          {t:'Наименование, характеристика', r1:20, r2:21, c1:C.C, c2:C.N, l:false, r:false},
          {t:'Номенкла-турный номер', r1:20, r2:21, c1:C.O, c2:C.S, l:false, r:false},
          {t:'Единица измерения', r1:20, r2:21, c1:C.T, c2:C.V, l:false, r:false},
          {t:'Количество', r1:20, r2:20, c1:C.W, c2:C.AE, l:false, r:false},
          {t:'подлежит отпуску', r1:21, r2:21, c1:C.W, c2:C.AA, l:false, r:false},
          {t:'отпущено', r1:21, r2:21, c1:C.AB, c2:C.AE, l:false, r:false},
          {t:'Цена за единицу, в KZT', r1:20, r2:21, c1:C.AF, c2:C.AK, l:false, r:false},
          {t:'Сумма с НДС, в KZT', r1:20, r2:21, c1:C.AL, c2:C.AQ, l:false, r:false},
          {t:'Сумма НДС, в KZT', r1:20, r2:21, c1:C.AR, c2:C.AW, l:false, r:true}
        ];
        tblH.forEach(function (h) {
          for (var rr = h.r1; rr <= h.r2; rr++) {
            cellAt(data, rr, h.c1, { v: h.t, s: { font: fnt(8,false).font, alignment: fnt(8,false,false,{h:'center',v:'center',w:true}).alignment, border: z2TblHdrBdr(h.l, h.r, rr === h.r1) } });
            M(h.r1, h.c1, h.r2, h.c2);
          }
        });
        // Row 23: Column numbers
        var colNums = [1,2,3,4,5,6,7,8,9];
        var colRanges = [[C.A,C.B,true],[C.C,C.N,false],[C.O,C.S,false],[C.T,C.V,false],[C.W,C.AA,false],[C.AB,C.AE,false],[C.AF,C.AK,false],[C.AL,C.AQ,false],[C.AR,C.AW,false,true]];
        colNums.forEach(function (n, i) {
          var b = { top: tin(), bottom: tin('thin') };
          b.left = colRanges[i][2] ? med() : tin();
          b.right = colRanges[i][3] ? med() : tin();
          cellAt(data, 22, colRanges[i][0], { v: String(n), s: { font: fnt(8,false).font, alignment: fnt(8,false,false,{h:'center',v:'center'}).alignment, border: b } });
        ...
        items.forEach(function (it, idx) {
          ...
          function z2dc(c1, c2, txt, al, leftM, rightM) {
            ...
            if (leftM) b.left = med(); else b.left = tin();
            if (rightM) b.right = med(); else b.right = tin();
            cellAt(data, dr, c1, { v: txt, s: { font: fnt(8,false).font, alignment: { horizontal: al||'left', vertical: 'center', wrapText: true }, border: b } });
            if (c2 > c1) M(dr, c1, dr, c2);
          }
        ...
        function z2tc(c1, c2, txt, al, leftM, rightM) {
          ...
          if (leftM) b.left = med(); else b.left = tin();
          if (rightM) b.right = med(); else b.right = tin();
          cellAt(data, dr, c1, { v: txt, s: { font: fnt(9,true).font, alignment: { horizontal: al||'left', vertical: 'center' }, border: b } });
          if (c2 > c1) M(dr, c1, dr, c2);
        }
        ...
        dr += 2;
        // Signature rows 60-69
        cellAt(data, dr, C.A, fmtCell('Всего отпущено количество запасов (прописью): ' + totalQty, fnt(8,false,false,{h:'left',v:'center'}))); M(dr, C.A, dr, C.AW); dr++;
        cellAt(data, dr, C.A, fmtCell('на сумму (прописью): ' + window.classicAmountWords(totalRaw), fnt(8,false,false,{h:'left',v:'center'}))); M(dr, C.A, dr, C.AW); dr++;
        dr++; // blank
        // Left signatures (3 blocks)
        var sigLeft = [
          {title:'Отпуск разрешил:', pos:directorPosition},
          {title:'Главный бухгалтер:', pos:''},
          {title:'Отпустил:', pos:''}
        ];
        sigLeft.forEach(function (s) {
          var posTxt = s.pos ? s.pos + ' ' : '';
          cellAt(data, dr, C.A, fmtCell(s.title, fnt(8,false,false,{h:'left',v:'center'}))); M(dr, C.A, dr, C.D);
          cellAt(data, dr, C.E, fmtCell('/_______________/_______________/', fnt(8,false,false,{h:'left',v:'center'}))); M(dr, C.E, dr, C.M);
          dr++;
        });
        dr++; // blank between left and right
        // Right signature: Запасы получил
        cellAt(data, dr, C.AE, fmtCell('Запасы получил:', fnt(8,false,false,{h:'left',v:'center'}))); M(dr, C.AE, dr, C.AJ); dr++;
        cellAt(data, dr, C.AE, fmtCell('/_______________/_______________/', fnt(8,false,false,{h:'left',v:'center'}))); M(dr, C.AE, dr, C.AJ); dr++;
        cellAt(data, dr, C.AE, fmtCell('по доверенности № ________________ от ___ _________ 20__ года', fnt(8,false,false,{h:'left',v:'center'}))); M(dr, C.AE, dr, C.AW); dr++;
        cellAt(data, dr, C.AE, fmtCell('выданной __________________________________________________', fnt(8,false,false,{h:'left',v:'center'}))); M(dr, C.AE, dr, C.AW); dr++;
        // М.П.
        dr++;
        cellAt(data, dr, C.A, fmtCell('М.П.', fnt(8,false,false,{h:'left',v:'center'})));
        // Build Z2 sheet
        var ws = buildSheetFromData(data, merges);
        var colW = [];
        for (var ci = 0; ci < 49; ci++) colW.push({wch:3});
        ws['!cols'] = colW;
        ws['!rows'] = [];
        // Row heights per spec (index → Excel row, 0-indexed)
        for (var ri = 0; ri <= 4; ri++) ws['!rows'][ri] = { hpt: 11 };
        ws['!rows'][8] = { hpt: 12 };
        ws['!rows'][17] = { hpt: 23 };
        ws['!rows'][18] = { hpt: 47 };
        ws['!rows'][20] = { hpt: 17 };
        ws['!rows'][21] = { hpt: 17 };
        // Data rows ~23pt, signatures ~15pt
        for (var ri = dr; ri < data.length; ri++) {
          if (!ws['!rows'][ri]) ws['!rows'][ri] = { hpt: 23 };
        }
        ws['!print'] = { paperSize: 9, orientation: 'landscape' };
        XLSX.utils.book_append_sheet(wb, ws, 'Лист_1');
      }
  ...
  window.saveStoreSettings = function () {
    ...
    try { localStorage.setItem('sanaq_return_limit_' + (currentStoreId || ''), returnLimit); } catch(e) {}
    if (ds) {
  ...
  function getUserData(userId) {
    try {
      ...
      if (!all) all = JSON.parse(localStorage.getItem('sanaq_permissions_' + (currentStoreId || '')) || '{}');
      var data = all[userId];
      if (!data) return { permissions: Object.assign({}, DEFAULT_PERMISSIONS), maxDiscount: 0 };
      if (data.permissions) return data;
      var oldMax = data._maxDiscount || 0;
      delete data._maxDiscount;
      return { permissions: data, maxDiscount: oldMax };
    } catch(e) { return { permissions: Object.assign({}, DEFAULT_PERMISSIONS), maxDiscount: 0 }; }
  }
  ...
  function setUserPermissions(userId, data) {
    try {
      ...
      if (window.ApDb && window.ApDb.setAppData) window.ApDb.setAppData('permissions', all);
      // If we're updating our own permissions, apply immediately
      if (currentUser && currentUser.id === userId) applyRoleUI();
    } catch(e) {}
  }
  ...
  var PERMISSION_GROUPS = {
    'Продажи': ['viewSales','viewReceiptHistory','searchReceipts','printReceipt','reprintReceipt','openCashDrawer','openShift','closeShift','cashIn','cashOut','viewCashOps','saleWithoutCustomer','saleOnCredit','refundMoney','useCustomerBonuses','accrueBonuses','canChangeQty','canChangePriceInSale','canAddUniversal','canManualItemDiscount','canManualCartDiscount','createSale','canReturn','canCancelReceipt','canDeferSale','canChangePayment','canMixedPayment'],
    'Товары': ['viewProducts','addProducts','editProducts','deleteProducts','changePrices','changeStock','importProducts','exportProducts','massChangePrices','massChangeCategories','massDelete','viewPurchasePrice','viewCostPrice','viewProductProfit'],
    'Категории': ['viewCategories','addCategory','editCategory','deleteCategory'],
    'Клиенты': ['viewCustomers','addCustomer','editCustomer','deleteCustomer','manageBonuses','viewPurchaseHistory','exportCustomers'],
    'Склад': ['viewWarehouse','receiveStock','writeOffStock','inventory','transferStock','viewPurchasePrices'],
    'Статистика': ['viewStatistics','viewProfit','viewExpenses','viewRevenue','viewProductAnalytics','viewCashierAnalytics','viewCustomerAnalytics'],
    'Долги': ['viewDebts','createDebt','editDebt','closeDebt','deleteDebt'],
    'Настройки': ['changeSettings','manageUsers','manageRoles','managePermissions','exportData','importData','backupData','restoreData','clearData']
  };
  var PERMISSION_LABELS = {
    viewSales: 'Просмотр продаж', viewReceiptHistory: 'Просмотр истории чеков', searchReceipts: 'Поиск чеков', printReceipt: 'Печать чека', reprintReceipt: 'Повторная печать чека',
    openCashDrawer: 'Открытие денежного ящика', openShift: 'Открытие смены', closeShift: 'Закрытие смены', cashIn: 'Внесение денег', cashOut: 'Изъятие денег', viewCashOps: 'Просмотр кассовых операций',
    saleWithoutCustomer: 'Продажа без клиента', saleOnCredit: 'Продажа в долг', refundMoney: 'Возврат денег', useCustomerBonuses: 'Использование бонусов', accrueBonuses: 'Начисление бонусов',
    canChangeQty: 'Изменение кол-ва', canChangePriceInSale: 'Изменение цены при продаже', canAddUniversal: 'Универсальный товар', canManualItemDiscount: 'Ручная скидка на товар', canManualCartDiscount: 'Ручная скидка на корзину',
    createSale: 'Создание продажи', canReturn: 'Возврат товара', canCancelReceipt: 'Отмена чека', canDeferSale: 'Отложенная продажа', canChangePayment: 'Изменить способ оплаты', canMixedPayment: 'Смешанная оплата',
    viewProducts: 'Просмотр товаров', addProducts: 'Добавление товаров', editProducts: 'Редактирование товаров', deleteProducts: 'Удаление товаров',
    changePrices: 'Изменение цен', changeStock: 'Изменение остатков', importProducts: 'Импорт товаров', exportProducts: 'Экспорт товаров',
    massChangePrices: 'Массовое изменение цен', massChangeCategories: 'Массовое изменение категорий', massDelete: 'Массовое удаление',
    viewPurchasePrice: 'Просмотр закупочной цены', viewCostPrice: 'Просмотр себестоимости', viewProductProfit: 'Просмотр прибыли по товару',
    viewCategories: 'Просмотр категорий', addCategory: 'Добавление категорий', editCategory: 'Редактирование категорий', deleteCategory: 'Удаление категорий',
    viewCustomers: 'Просмотр клиентов', addCustomer: 'Добавление клиентов', editCustomer: 'Редактирование клиентов', deleteCustomer: 'Удаление клиентов',
    manageBonuses: 'Управление бонусами', viewPurchaseHistory: 'История покупок', exportCustomers: 'Экспорт клиентов',
    viewWarehouse: 'Просмотр склада', receiveStock: 'Приход товара', writeOffStock: 'Списание товара', inventory: 'Инвентаризация', transferStock: 'Перемещение товаров', viewPurchasePrices: 'Просмотр закупочных цен',
    viewStatistics: 'Просмотр статистики', viewProfit: 'Просмотр прибыли', viewExpenses: 'Просмотр расходов', viewRevenue: 'Просмотр выручки',
    viewProductAnalytics: 'Аналитика по товарам', viewCashierAnalytics: 'Аналитика по кассирам', viewCustomerAnalytics: 'Аналитика по клиентам',
    viewDebts: 'Просмотр долгов', createDebt: 'Создание долга', editDebt: 'Редактирование долга', closeDebt: 'Закрытие долга', deleteDebt: 'Удаление долга',
    changeSettings: 'Изменение настроек', manageUsers: 'Управление пользователями', manageRoles: 'Управление ролями', managePermissions: 'Управление правами',
    exportData: 'Экспорт данных', importData: 'Импорт данных', backupData: 'Резервное копирование', restoreData: 'Восстановление базы', clearData: 'Очистка базы'
  };
  var _pendingPerms = null;
  var _pendingMaxDisc = 0;

  window.renderPermissionsEditor = function(userId) {
    ...
    // Toolbar
    var html = '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px;grid-column:1/-1">' +
      '<input type="text" id="perm-search" placeholder="🔍 Поиск права..." style="flex:1;min-width:120px;padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:12px;background:var(--bg2)" oninput="filterPermissions(this.value)">' +
      '<button class="btn btn-sm btn-secondary" onclick="selectAllPermissions()" title="Выбрать все права">✓ Все</button>' +
      '<button class="btn btn-sm btn-secondary" onclick="deselectAllPermissions()" title="Снять все права">✕ Ничего</button>' +
      '<button class="btn btn-sm btn-secondary" onclick="copyPermissionsFromUser()" title="Скопировать права другого пользователя">📋 Копировать</button>' +
      '<button class="btn btn-sm btn-secondary" onclick="savePermissionTemplate()" title="Сохранить текущие права как шаблон">💾 Шаблон</button>' +
      '<button class="btn btn-sm btn-secondary" onclick="applyPermissionTemplate()" title="Применить шаблон прав">📂 Применить</button>' +
      '</div>';
    // Permission groups
    Object.keys(PERMISSION_GROUPS).forEach(function(group) {
      html += '<div class="perm-group-card" data-group="' + group + '" style="background:var(--bg3);border-radius:10px;padding:10px 12px;border:1px solid var(--border)">' +
        '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">' +
        '<div style="font-weight:700;font-size:12px;color:var(--text-secondary);text-transform:uppercase;letter-spacing:0.3px">' + group + '</div>' +
        '<div style="display:flex;gap:4px">' +
        '<span onclick="selectAllInGroup(\'' + group + '\',true)" style="cursor:pointer;font-size:11px;color:var(--text-muted);padding:2px 6px;border-radius:4px;background:var(--bg2)">✓ все</span>' +
        '<span onclick="selectAllInGroup(\'' + group + '\',false)" style="cursor:pointer;font-size:11px;color:var(--text-muted);padding:2px 6px;border-radius:4px;background:var(--bg2)">✕ нет</span>' +
        '</div></div>';
      PERMISSION_GROUPS[group].forEach(function(key) {
        html += '<label class="perm-item" data-key="' + key + '" style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;padding:2px 0">' +
          '<input type="checkbox" data-perm="' + key + '" ' + (perms[key] ? 'checked' : '') + ' onchange="savePermission(this)"> ' +
    ...
    // Max discount
    html += '<div style="grid-column:1/-1;background:var(--bg3);border-radius:10px;padding:10px 12px;border:1px solid var(--border)">' +
      '<div style="font-weight:700;font-size:12px;color:var(--text-secondary);text-transform:uppercase;letter-spacing:0.3px;margin-bottom:6px">Параметры кассира</div>' +
      '<div style="display:flex;align-items:center;gap:8px">' +
      '<label style="font-size:13px">Макс. ручная скидка (%)</label>' +
      '<select id="cashier-max-discount" style="padding:4px 8px;border:1px solid var(--border);border-radius:6px;font-size:13px;background:var(--bg2)" onchange="saveCashierMaxDiscount(this.value)">' +
      [0,5,10,15,20,25].map(function(v) { return '<option value="' + v + '" ' + (maxDisc === v ? 'selected' : '') + '>' + v + '%</option>'; }).join('') +
      '</select></div></div>';
    container.innerHTML = html;
  };
  window.savePermission = function(cb) {
  ...
  window.saveCashierMaxDiscount = function(val) {
    var userId = document.getElementById('cashier-edit-id').value;
    if (!userId) {
      _pendingMaxDisc = parseInt(val) || 0;
      return;
    }
    var data = getUserData(userId);
  ...
  function _permUserId() { return document.getElementById('cashier-edit-id').value; }
  function _permData() {
    var uid = _permUserId();
    if (!uid) return _pendingPerms || Object.assign({}, DEFAULT_PERMISSIONS);
    return getUserData(uid).permissions;
  }
  function _setPerm(permName, val) {
    var uid = _permUserId();
    if (!uid) {
      if (!_pendingPerms) _pendingPerms = Object.assign({}, DEFAULT_PERMISSIONS);
  ...
  window.selectAllPermissions = function() {
    Object.keys(DEFAULT_PERMISSIONS).forEach(function(k) { _setPerm(k, true); });
    renderPermissionsEditor(_permUserId());
    toast('Все права выбраны', 'ok');
  };
  window.deselectAllPermissions = function() {
    Object.keys(DEFAULT_PERMISSIONS).forEach(function(k) { _setPerm(k, false); });
    renderPermissionsEditor(_permUserId());
    toast('Все права сняты', 'ok');
  };
  window.selectAllInGroup = function(group, on) {
    var keys = PERMISSION_GROUPS[group] || [];
    keys.forEach(function(k) { _setPerm(k, on); });
    renderPermissionsEditor(_permUserId());
  };
  window.filterPermissions = function(query) {
  ...
  window.copyPermissionsFromUser = function() {
    var users = getAllUsers().filter(function(u) { return u.id !== currentUser.id && u.role !== 'admin'; });
    if (!users.length) { toast('Нет других пользователей', 'err'); return; }
    var list = users.map(function(u) { return '<div onclick="applyCopiedPermissions(\'' + u.id + '\')" style="padding:6px 10px;cursor:pointer;border-bottom:1px solid var(--border)">' + esc(u.name) + ' (' + esc(u.role) + ')</div>'; }).join('');
    showCustomModal('Выберите пользователя для копирования прав', '<div style="max-height:300px;overflow-y:auto">' + list + '</div>');
  };
  window.applyCopiedPermissions = function(sourceUserId) {
    closeModal('modal-custom');
    var data = getUserData(sourceUserId);
    var targetId = _permUserId();
    if (!targetId) {
      _pendingPerms = Object.assign({}, data.permissions);
  ...
  window.savePermissionTemplate = function() {
    var uid = _permUserId();
    ...
    try {
      ...
      templates.push({ name: name, permissions: perms, createdAt: new Date().toISOString() });
      localStorage.setItem('sanaq_perm_templates_' + (currentStoreId || ''), JSON.stringify(templates));
      toast('Шаблон "' + name + '" сохранён', 'ok');
    } catch(e) { toast('Ошибка сохранения шаблона', 'err'); }
  };
  window.applyPermissionTemplate = function() {
    try {
      var templates = JSON.parse(localStorage.getItem('sanaq_perm_templates_' + (currentStoreId || '')) || '[]');
      if (!templates.length) { toast('Нет сохранённых шаблонов', 'err'); return; }
    ...
    } catch(e) { toast('Ошибка загрузки шаблонов', 'err'); }
  };
  window.applyTemplateIndex = function(idx) {
    closeModal('modal-custom');
    try {
      var templates = JSON.parse(localStorage.getItem('sanaq_perm_templates_' + (currentStoreId || '')) || '[]');
      ...
      var targetId = _permUserId();
      if (!targetId) {
        _pendingPerms = Object.assign({}, tpl.permissions);
        renderPermissionsEditor(null);
    ...
    } catch(e) { toast('Ошибка применения шаблона', 'err'); }
  };
  function showCustomModal(title, body) {
    ...
    if (!existing) {
      ...
      div.addEventListener('click', function(e) { if (e.target === div) closeModal('modal-custom'); });
    }
  ...
  function getPromotions() {
    ...
    try { return JSON.parse(localStorage.getItem('sanaq_promotions_' + (currentStoreId || '')) || '[]'); } catch(e) { return []; }
  }
  function setPromotions(arr) {
    try { localStorage.setItem('sanaq_promotions_' + (currentStoreId || ''), JSON.stringify(arr)); } catch(e) {}
    if (window.ApDb && window.ApDb.setAppData) window.ApDb.setAppData('promotions', arr);
  }
  ...
  function getProductDiscount(productId) {
    ...
    promos.forEach(function(p) {
      if (p.productId === productId) {
        if (!best || p.discountValue > best.discountValue) best = p;
      }
    });
    return best;
  }
  window.renderPromotionsPage = function() {
    ...
    list.forEach(function(p) {
      ...
      var products = getProducts();
      var product = products.find(function(x) { return x.id === p.productId; });
      ...
      html += '<div class="card" style="' + (isActive ? 'border-left:3px solid var(--success)' : 'border-left:3px solid var(--text-muted);opacity:0.7') + '">' +
        ...
        '<span style="flex:1;font-weight:600">' + esc(pName) + '</span>' +
        '<span class="badge ' + (isActive ? 'badge-ok' : 'badge') + '">' + (isActive ? 'Активна' : 'Завершена') + '</span>' +
        '<span style="font-weight:700;color:var(--err)">-' + (p.discountType === 'percent' ? p.discountValue + '%' : fmt(p.discountValue) + ' ₸') + '</span>' +
        '<span style="font-size:12px;color:var(--text-secondary)">' + p.startDate + ' → ' + p.endDate + '</span>' +
        '<div style="display:flex;gap:4px">' +
        '<button class="icon-btn" onclick="editPromotion(\'' + p.id + '\')">✏️</button>' +
        '<button class="icon-btn del" onclick="deletePromotion(\'' + p.id + '\')">🗑</button>' +
        '</div></div></div>';
    });
    html += '</div>';
    container.innerHTML = html;
  ...
  window.savePromotion = function() {
    ...
    if (!productId) { toast('Выберите товар', 'err'); return; }
    if (discountValue <= 0) { toast('Введите размер скидки', 'err'); return; }
    if (!startDate || !endDate) { toast('Укажите даты', 'err'); return; }
    var data = { productId: productId, discountType: discountType, discountValue: discountValue, startDate: startDate, endDate: endDate, startTime: startTime, endTime: endTime };
    var list = getPromotions();
    if (id) {
      list = list.map(function(p) { return p.id === id ? Object.assign({}, p, data) : p; });
  ...
  window.editPromotion = function(id) { openPromotionEditor(id); };
  window.deletePromotion = function(id) {
    confirmAction('Удалить акцию', 'Удалить акцию?', function() {
      var list = getPromotions().filter(function(p) { return p.id !== id; });
      setPromotions(list);
      renderPromotionsPage();
      toast('Акция удалена', 'ok');
    });
  };
  window.togglePromoDiscountType = function() {};
  ...
  setInterval(function() {
    ...
    list = list.map(function(p) {
      var end = new Date(p.endDate + 'T' + (p.endTime || '23:59'));
      if (p.active !== false && now > end) { p.active = false; changed = true; }
      return p;
    });
    if (changed) setPromotions(list);
  }, 60000);
  // ─── INIT ───
  async function initApp() {
    ...
    // Seed local users only if Supabase has no members (local-only mode)
    if (getUsers().length === 0) {
      var existingLU = getLocalUsers();
      if (!existingLU.length) {
        setLocalUsers([
          { id: 'user-admin', username: 'admin', password: 'admin123', name: 'Администратор', role: 'admin', active: true },
          { id: uid(), username: 'cashier', password: 'cashier123', name: 'Кассир 1', role: 'cashier', active: true }
        ]);
      }
    }
    ...
    // ⏰ Автоматическая резервная копия каждые 2 часа
    setInterval(function () {
  ...
  function autoBackup() {
    try {
      ...
      if (typeof toast === 'function') toast('💾 Авто-резервная копия создана', 'ok');
    } catch (e) {
  ...
  function restoreAutoBackup() {
    try {
      ...
      if (age > 86400000) { localStorage.removeItem('ap_auto_backup_' + store); return; } // >1 day
      if (getProducts().length === 0 && data.products && data.products.length > 0) {
        ...
        if (data.deferred) setDeferred(data.deferred);
        toast('💾 Данные восстановлены из авто-копии', 'ok');
      }
    } catch (e) {}
  }

  // Show last auto-backup time
  ...
  function renderNotifications() {
    ...
    var products = getProducts();
    var lowItems = products.filter(function(p) { return p.quantity <= (p.minStock || 5); });
    if (lowItems.length > 0) {
      notifs.push({ icon: '⚠️', title: 'Низкий остаток: ' + lowItems.length + ' товаров', desc: lowItems.slice(0,3).map(function(p) { return p.name + ' (' + p.quantity + ' шт)'; }).join(', ') + (lowItems.length > 3 ? ' и ещё ' + (lowItems.length - 3) : ''), time: 'сейчас' });
    }

    var shifts = getShifts();
    var openShifts = shifts.filter(function(s) { return s.status === 'open'; });
    if (openShifts.length > 0) {
      notifs.push({ icon: '🕐', title: 'Открытые смены: ' + openShifts.length, desc: openShifts.map(function(s) { return s.cashierName + ' (' + fmtDate(s.openedAt) + ')'; }).join(', '), time: 'сейчас' });
    }

    var deferred = getDeferred ? getDeferred() : [];
    if (deferred.length > 0) {
      notifs.push({ icon: '📦', title: 'Отложенные товары: ' + deferred.length, desc: 'Есть отложенные товары, ожидающие обработки', time: 'сейчас' });
    }

    var debts = getDebts ? getDebts() : [];
    var openDebts = debts.filter(function(d) { return d.status === 'open'; });
    if (openDebts.length > 0) {
      notifs.push({ icon: '💰', title: 'Активные долги: ' + openDebts.length, desc: 'Общая сумма: ' + fmt(openDebts.reduce(function(s,d) { return s + (Number(d.amount)||0); }, 0)) + ' ₸', time: 'сейчас' });
    }
    ...
    if (ts) {
      ...
      if (elapsed > 24) {
        notifs.push({ icon: '💾', title: 'Резервная копия устарела', desc: 'Последняя копия была сделана ' + Math.floor(elapsed/24) + ' дней назад', time: Math.floor(elapsed/24) + 'д назад' });
      }
    }
  ...
  function updateNotifBadge() {
    var products = getProducts();
    var lowCount = products.filter(function(p) { return p.quantity <= (p.minStock || 5); }).length;
    var shifts = getShifts();
    var openCount = shifts.filter(function(s) { return s.status === 'open'; }).length;
  ...
  function getUISettings() {
    ...
    if (v && typeof v === 'object') { _uiSettings = v; return _uiSettings; }
    try {
  ...
  function saveUISettings() {
    ...
    if (window.ApDb && window.ApDb.setAppData) window.ApDb.setAppData('ui_settings', _uiSettings);
  }
  ...
  function applyUISettings() {
    ...
    var colsCss = cols;
    if (window.innerWidth < 768) colsCss = Math.min(cols, 3);
    if (window.innerWidth < 480) colsCss = Math.min(cols, 2);
    ...
    styleEl.textContent =
      'html { font-size: ' + (14 * scale) + 'px !important; }' +
      '.card { padding: ' + (20 * cardSize) + 'px !important; }' +
      '.card-value { font-size: ' + (1.8 * cardSize) + 'em !important; }' +
      '.panel { padding: ' + (16 * cardSize) + 'px !important; }' +
      '.modal { padding: ' + (4 * cardSize) + 'px !important; }' +
      '.modal-body { padding: ' + (16 * cardSize) + 'px !important; }' +
      '#pos-products { --grid-cols: ' + colsCss + '; }' +
      '.btn, button:not(.icon-btn):not(.nav-btn):not(.pos-btm-btn):not(.tab):not(.modal-close) { ' +
      '  padding: ' + (8 * btnSize) + 'px ' + (14 * btnSize) + 'px !important; ' +
      '  font-size: ' + (13 * btnSize) + 'px !important; ' +
      '  border-radius: ' + (6 * btnSize) + 'px !important; ' +
      '}';
  ...
  function applyUIVisibility() {
    ...
    // Show floating restore button when sidebar is hidden
    var restoreBtn = document.getElementById('sidebar-restore-btn');
    if (restoreBtn) {
  ...
  window.restoreSidebar = function() {
    _uiSettings = getUISettings();
    if (!_uiSettings.visibility) _uiSettings.visibility = {};
  ...
  function applyUIPosMode(mode) {
    ...
    if (mode === 'compact') html.classList.add('ui-compact');
    else if (mode === 'large') html.classList.add('ui-large');
    else html.classList.add('ui-standard');
  }

  window.setUIPosMode = function(mode) {
    _uiSettings = getUISettings();
  ...
  window.openUISettings = function() {
    var s = getUISettings();
    ...
    if (visContainer) {
      ...
      visContainer.innerHTML = Object.keys(labels).map(function(key) {
        return '<label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer">' +
          '<input type="checkbox" ' + (vis[key] !== false ? 'checked' : '') + ' onchange="toggleUIVisibility(\'' + key + '\', this.checked)"> ' +
          labels[key] +
          '</label>';
      }).join('');
    }
  ...
  window.toggleUIVisibility = function(key, visible) {
    _uiSettings = getUISettings();
    if (!_uiSettings.visibility) _uiSettings.visibility = {};
  ...
  window.setUIProfile = function(profile) {
    var profiles = {
      phone: { scale: 0.85, cardSize: 0.85, cols: 2 },
      tablet: { scale: 0.9, cardSize: 0.9, cols: 3 },
      desktop: { scale: 1, cardSize: 1, cols: 4 },
      pos: { scale: 1.15, cardSize: 1.1, cols: 5 }
    };
    ...
    applyUISettings();
    openUISettings();
    toast('Профиль «' + profile + '» применён', 'ok');
  };
  ...
  window.setUICardSize = function(size) {
    _uiSettings = getUISettings();
  ...
  window.setUICols = function(cols) {
    _uiSettings = getUISettings();
  ...
  window.setUIButtonSize = function(size) {
    _uiSettings = getUISettings();
  ...
  window.saveUIProfile = function() {
    var name = document.getElementById('ui-profile-name').value.trim();
    if (!name) { toast('Введите название профиля', 'err'); return; }
    var s = getUISettings();
  ...
  function getUIProfiles() {
    ...
    try { return JSON.parse(localStorage.getItem('sanaq_ui_profiles_' + (currentStoreId || '')) || '{}'); } catch(e) { return {}; }
  }
  function setUIProfiles(profiles) {
    try { localStorage.setItem('sanaq_ui_profiles_' + (currentStoreId || ''), JSON.stringify(profiles)); } catch(e) {}
    if (window.ApDb && window.ApDb.setAppData) window.ApDb.setAppData('ui_profiles', profiles);
  }
  function renderSavedUIProfiles() {
    ...
    container.innerHTML = names.map(function(name) {
      return '<div class="ui-saved-profile">' +
        '<span class="name">' + esc(name) + '</span>' +
        '<div class="actions">' +
        '<button class="btn btn-sm btn-secondary" onclick="loadUIProfile(\'' + name.replace(/'/g, "\\'") + '\')">📂</button>' +
        '<button class="btn btn-sm btn-danger" onclick="deleteUIProfile(\'' + name.replace(/'/g, "\\'") + '\')">✕</button>' +
        '</div></div>';
    }).join('');
  }

  window.loadUIProfile = function(name) {
    var profiles = getUIProfiles();
    var p = profiles[name];
    if (!p) return;
    _uiSettings = getUISettings();
    Object.keys(p).forEach(function(k) { _uiSettings[k] = p[k]; });
  ...
  window.deleteUIProfile = function(name) {
    confirmAction('Удалить профиль', 'Удалить профиль «' + name + '»?', function() {
      var profiles = getUIProfiles();
  ...
  var DEFAULT_POS_LAYOUT = ['favorites', 'categories', 'shift-history', 'search'];

  function getPosLayout() {
    var s = getUISettings();
    return s.posLayout || ['favorites', 'categories', 'shift-history', 'search'];
  }
  ...
  function renderPosLayoutEditor() {
    ...
    var labels = { favorites: '⭐ Быстрые товары', categories: '🗂️ Категории', 'shift-history': '📋 История продаж', search: '🔍 Поиск' };
    container.innerHTML = layout.map(function(key, i) {
      return '<div draggable="true" class="pos-layout-item" data-key="' + key + '" data-index="' + i + '" ondragstart="onPosLayoutDragStart(event)" ondragover="onPosLayoutDragOver(event)" ondrop="onPosLayoutDrop(event)" ondragend="onPosLayoutDragEnd(event)" style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:var(--bg3);border-radius:8px;cursor:grab;border:1px solid var(--border)">' +
        '<span style="cursor:grab;color:var(--text-muted)">⠿</span>' +
        '<span style="flex:1;font-size:13px">' + (labels[key] || key) + '</span>' +
        '</div>';
    }).join('');
  }

  window.resetPosLayout = function() {
    _uiSettings = getUISettings();
    _uiSettings.posLayout = ['favorites', 'categories', 'shift-history', 'search'];
    saveUISettings();
    renderPosLayoutEditor();
    applyPosLayout();
  ...
  // Drag & drop for POS layout editor
  ...
  function onPosLayoutDrop(e) {
    ...
    var target = e.target.closest('.pos-layout-item');
    if (!target || target.dataset.key === draggedKey) return;
  ...
  window.addEventListener('resize', function() {
    ...
    if (grid) {
      var cols = s.cols || 4;
      if (window.innerWidth < 768) cols = Math.min(cols, 3);
      if (window.innerWidth < 480) cols = Math.min(cols, 2);
  ...
  window.toggleBulkSelectAll = function(cb) {
    document.querySelectorAll('.bulk-item').forEach(function(el) {
      ...
      if (cb.checked) _bulkSelected.add(el.dataset.id);
      else _bulkSelected.delete(el.dataset.id);
    });
    updateBulkBar();
  };
  ...
  document.addEventListener('change', function(e) {
    if (e.target.classList.contains('bulk-item')) {
      if (e.target.checked) _bulkSelected.add(e.target.dataset.id);
      else _bulkSelected.delete(e.target.dataset.id);
      updateBulkBar();
  ...
  window.clearBulkSelection = function() {
    _bulkSelected.clear();
    document.querySelectorAll('.bulk-item').forEach(function(el) { el.checked = false; });
    var selAll = document.getElementById('bulk-select-all');
    if (selAll) selAll.checked = false;
    updateBulkBar();
  };

  window.bulkChangePrice = function() {
    if (_bulkSelected.size === 0) { toast('Нет выбранных товаров', 'err'); return; }
  ...
  window.applyBulkPrice = function() {
    ...
    if (isNaN(value) || value < 0) { toast('Введите корректное значение', 'err'); return; }
    var list = getProducts();
    var changed = 0;
    list = list.map(function(p) {
      if (_bulkSelected.has(p.id)) {
        ...
        switch(type) {
          case 'set': p.price = value; break;
          case 'percent': p.price = oldPrice * (1 + value / 100); break;
          case 'increase': p.price = oldPrice + value; break;
          case 'decrease': p.price = Math.max(0, oldPrice - value); break;
        }
  ...
  window.bulkChangeCategory = function() {
    ...
    sel.innerHTML = '<option value="">Без категории</option>' + getCategories().map(function(c) { return '<option value="' + c.id + '">' + esc(c.name) + '</option>'; }).join('');
  ...
  window.applyBulkCategory = function() {
    ...
    var changed = 0;
    list = list.map(function(p) {
      if (_bulkSelected.has(p.id)) { p.category = cat; changed++; }
      return p;
    });
    setProducts(list);
  ...
  window.bulkDelete = function() {
    ...
    confirmAction('Массовое удаление', 'Удалить ' + _bulkSelected.size + ' товаров?', function() {
      ...
      var deleted = 0;
      list = list.filter(function(p) {
        if (_bulkSelected.has(p.id)) { deleted++; return false; }
        return true;
      });
      setProducts(list);
      clearBulkSelection();
  ...
  // ─── BULK DISCOUNT ───
  window.bulkApplyDiscount = function() {
    if (_bulkSelected.size === 0) { toast('Нет выбранных товаров', 'err'); return; }
  ...
  window.applyBulkDiscount = function() {
    var value = parseFloat(document.getElementById('bulk-discount-value').value);
    if (isNaN(value) || value <= 0 || value > 100) { toast('Введите скидку от 1 до 100%', 'err'); return; }
    ...
    getProducts().forEach(function(p) {
      if (_bulkSelected.has(p.id)) {
        var existing = promos.findIndex(function(pr) { return pr.productId === p.id && pr.active !== false; });
        if (existing >= 0) {
  ...
  window.bulkRemoveDiscount = function() {
    if (_bulkSelected.size === 0) { toast('Нет выбранных товаров', 'err'); return; }
    confirmAction('Удаление скидок', 'Удалить скидки для ' + _bulkSelected.size + ' товаров?', function() {
      var promos = getPromotions();
      var selectedIds = new Set();
      _bulkSelected.forEach(function(id) { selectedIds.add(id); });
      var before = promos.length;
      promos = promos.filter(function(pr) { return !selectedIds.has(pr.productId); });
      setPromotions(promos);
      clearBulkSelection();
      renderProducts();
...
function getUserPin(userId) {
  ...
  try {
    ...
    if (!pins) pins = JSON.parse(localStorage.getItem('sanaq_user_pins') || '{}');
...
function saveUserPin(userId, pin) {
  try {
    ...
    if (window.ApDb && window.ApDb.setAppData) window.ApDb.setAppData('user_pins', pins);
  } catch(e) {}
}
...
function getAuditLog() {
    ...
    if (v !== null && v !== undefined) return Array.isArray(v) ? v : [];
    try { return JSON.parse(localStorage.getItem('sanaq_audit_log') || '[]'); } catch(e) { return []; }
  }

  function setAuditLog(arr) {
    try { localStorage.setItem('sanaq_audit_log', JSON.stringify(arr)); } catch(e) {}
    if (window.ApDb && window.ApDb.setAppData) window.ApDb.setAppData('audit_log', arr);
  }
  ...
  window.savePinCode = function() {
    ...
    if (pin.length < 4 || pin.length > 6) { toast('PIN должен быть 4–6 цифр', 'err'); return; }
    if (pin !== confirm) { toast('PIN-коды не совпадают', 'err'); return; }
  ...
  window.openCashierSwitch = function() {
    var users = getAllUsers();
    var container = document.getElementById('cashier-switch-list');
    container.innerHTML = '<div style="margin-bottom:12px;font-size:13px;color:var(--text-secondary)">Выберите пользователя для входа</div>' +
      users.filter(function(u) { return u.active !== false; }).map(function(u) {
        ...
        return '<div class="cashier-switch-item" onclick="' + (isCurrent ? '' : 'switchToUser(\'' + u.id + '\')') + '" style="' + (isCurrent ? 'opacity:0.6' : '') + '">' +
          '<div class="avatar">' + esc(initial) + '</div>' +
          '<div class="info"><div class="name">' + esc(u.name || '—') + (isCurrent ? ' <span style="font-size:11px;color:var(--text-muted)">(текущий)</span>' : '') + '</div><div class="role">' + esc(u.role || '—') + '</div></div>' +
          hasPin +
          '</div>';
      }).join('');
  ...
  window.switchToUser = function(userId) {
    ...
    if (!target) { toast('Пользователь не найден', 'err'); return; }
    ...
    if (userPin) {
      ...
      setTimeout(function() { document.getElementById('pin-login-code').focus(); }, 100);
    } else {
      doSwitchUser(target);
    }
  };

  window.checkPinLogin = function() {
    var entered = document.getElementById('pin-login-code').value;
    var userId = _pendingSwitchUserId;
    if (!userId) return;
    var userPin = getUserPin(userId);
    if (!userPin) { closeModal('modal-pin-login'); doSwitchUser(_findUserAnywhere(userId)); return; }
  ...
  function _findUserAnywhere(userId) {
    var u = getUsers().find(function(u) { return u.id === userId; });
    if (u) return u;
    return getLocalUsers().find(function(u) { return u.id === userId; });
  }
  ...
  // ─── SCANNER TOGGLE FUNCTIONS ───
  window.toggleScannerAutoClose = function(cb) {
    _scannerAutoClose = cb.checked;
  };
  ...
  window.toggleScannerTorch = function() {
    ...
    if (capabilities && capabilities.torch) {
      ...
      track.applyConstraints({ advanced: [{ torch: !currentlyOn }] }).catch(function(e) { console.warn('[Scanner] Torch error:', e); });
    }
  };
  ...
  window.openQtyPopup = function(productId) {
    var item = saleCart.find(function(c) { return c.id === productId; });
    if (!item) return;
    _qtyPopupProductId = productId;
  ...
  window.confirmQtyPopup = function() {
    var qty = parseInt(document.getElementById('qty-popup-input').value) || 1;
    var item = saleCart.find(function(c) { return c.id === _qtyPopupProductId; });
    if (!item) { closeModal('modal-qty-popup'); return; }
    var maxQty = item.maxQty || 9999;
    if (qty > maxQty) { toast('На складе всего ' + maxQty + ' шт.', 'err'); return; }
    if (qty < 1) qty = 1;
    item.qty = qty;
    renderSaleCart();
  ...
  // Local login handler (для кассиров созданных без email)
  document.getElementById('login-form').onsubmit = function (e) {
    ...
    try {
      ...
      if (!u) { toast('Неверный логин или пароль', 'err'); return; }
      if (u.memberId) { toast('Этот аккаунт входит через Supabase (email/пароль)', 'err'); return; }
      if (u.password !== pass) { toast('Неверный логин или пароль', 'err'); return; }
      currentUser = { id: u.id, username: u.username || '', name: u.name, role: u.role || 'cashier' };
      toast('Добро пожаловать, ' + u.name + '!', 'ok');
      showApp();
    } catch (err) {
      try { toast('Ошибка входа: ' + (err && err.message ? err.message : err), 'err'); } catch (e2) {}
      throw err;
    }
  };
