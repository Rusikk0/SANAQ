...
import { _pendingPerms, currentStoreId, _bulkSelected, scannerStream, confirmCallback, _scannerAutoClose } from './store.js';
...
import { _importState } from './statistics.js';
import { currentUser } from './users.js';
import { saleCart } from './cart.js';
import { PAY_LABELS, currentPayment } from './sales.js';
import { _discountItemId, _qtyPopupProductId } from './products.js';
...
function readExcelFile(file, section) {
    ...
    _importState = { section: section, rawData: null, colMap: null, rows: null, fileName: file.name };
...
function _setPerm(permName, val) {
    ...
    if (!uid) {
        if (!_pendingPerms)
            _pendingPerms = Object.assign({}, DEFAULT_PERMISSIONS);
