...
const getProducts = () => (window.DataService ? window.DataService.get('products') : []);
...
const getDocumentItems = () => (window.ApDb ? window.ApDb.getDocumentItems() : []);
...
function findProductByBarcode(term, requireStock) {
    const t = normalizeBarcode(term);
    if (!t) return null;
    const found = getProducts().find(p => p.barcode && normalizeBarcode(p.barcode) === t;
    });
...
function findProductByCode(term, requireStock) {
    ...
    if (!t) return null;
    const found = getProducts().find(p => (p.code || '').toUpperCase() === t;
    });
    if (!found) return null;
    if (requireStock && found.quantity <= 0) return null;
    return found;
...
function findProductByScan(term) {
    const bc = findProductByBarcode(term, true);
    if (bc) return bc;
    const code = findProductByCode(term, true);
    if (code) return code;
    const bcNoStock = findProductByBarcode(term, false);
    if (bcNoStock && bcNoStock.quantity <= 0) {
        toast('Товар \xAB' + bcNoStock.name + '\xBB нет на складе', 'warn');
        return null;
    }
    const codeNoStock = findProductByCode(term, false);
...
function handleBarcodeScan(raw) {
    ...
    if (!code)
        return;
    const modalOpen = document.getElementById('modal-product').classList.contains('show');
    ...
    const product = findProductByScan(code) || findProductByBarcode(code, false) || findProductByCode(code, false);
    if (!product) {
...
function syncProductFilterUI() {
    ...
    sel.innerHTML = '<option value="">Все категории</option>' + cats.map(c => '<option value="' + c.id + '">' + c.name + '</option>';
    }).join('');
    ...
    if (supSel) {
        ...
        const suppliers = prods.filter(p => p.supplier;
        }).map(p => p.supplier;
        });
        suppliers = suppliers.filter(v, i, a => a.indexOf(v) === i;
        }).sort(a, b => a.localeCompare(b, 'ru');
        });
        supSel.innerHTML = '<option value="">Все поставщики</option>' + suppliers.map(s => '<option value="' + esc(s) + '">' + esc(s) + '</option>';
        }).join('');
...
function onBarcodeDetected(code) {
    if (!scannerActive || !scannerTarget) return;
    const now = Date.now();
    ...
    if (scannerTarget === 'sale-search') {
        const exact = findProductByScan(code);
        if (exact) {
            addToCart(exact.id);
            toast('\u2705 ' + exact.name, 'ok');
            ...
            const matches = smartMatchProducts(code);
            if (matches.length === 1) {
...
function renderUnsoldProducts(days) {
    ...
    const sales = getSales().filter(s => isSaleActive(s) && new Date(s.date) >= start;
    });
    const soldIds = {};
    sales.forEach(s => {
        if (s.productId)
    ...
    const unsold = products.filter(p => !soldIds[p.id];
    });
    const container = document.getElementById('unsold-products-list');
    if (!unsold.length) {
        container.innerHTML = '<div class="empty">Все товары продавались за последние ' + days + ' дней</div>';
        return;
    }
    const html = '<div style="font-size:13px;color:var(--text-muted);margin-bottom:8px">Товаров без продаж: ' + unsold.length + '</div>';
...
function recalcDocItemRow(el) {
    ...
    const totalSpan = tr.querySelector('.doc-item-total');
    if (totalSpan)
...
function getDocItemsFromTable(tbodyId) {
    ...
    tbody.querySelectorAll('tr').forEach(tr => {
        const code = (tr.querySelector('.doc-item-code') || {}).value || '';
        const name = (tr.querySelector('.doc-item-name') || {}).value || '';
        const qty = parseFloat((tr.querySelector('.doc-item-qty') || {}).value) || 0;
        const price = parseFloat((tr.querySelector('.doc-item-price') || {}).value) || 0;
        if (name && qty > 0) {
            items.push({
...
function smartMatchProducts(term) {
    const t = term.trim();
    if (!t) return [];
    const variations = generateSearchVariations(t);
    ...
    products.forEach(p => {
        ...
        const searchText = name + ' ' + code + ' ' + barcode + ' ' + sku;
        const bestScore = 0;
        const matchType = '';
...
function renderBarcodePreview() {
    const el = document.getElementById('product-barcode-preview');
    if (!el)
        return;
    const text = normalizeBarcode(document.getElementById('product-barcode').value);
...
function generateProductBarcode() {
    ...
    do {
        bc = generateEAN13(n++);
    } while (list.some(p => p.barcode === bc;
    }));
...
function printProductBarcode() {
    const text = normalizeBarcode(document.getElementById('product-barcode').value);
    const name = document.getElementById('product-name').value.trim() || 'Товар';
    const priceStr = barcodePriceLabel();
    if (!text) {
        toast('Сначала укажите штрих-код', 'err');
        return;
    }
    const lw = parseInt(document.getElementById('label-width').value) || 43;
    const lh = parseInt(document.getElementById('label-height').value) || 25;
    const isEan13 = /^\d{13}$/.test(text);
    const svgHtml, numHtml;
    if (isEan13) {
        const targetW = lw * 8 * 0.92;
        const modW = Math.max(1, Math.round(targetW / 115));
        const barH = Math.max(20, Math.round(lh * 8 * 0.4));
        ...
        const targetPx = lw * 8 * 0.85;
        const scale = Math.max(1, Math.floor(targetPx / (totalUnits * 2)));
        const targetHeightPx = lh * 8 * 0.35;
        const heightScale = Math.max(1, Math.round(targetHeightPx / 40));
...
function renderProducts() {
    ...
    if (search) {
        const smartResults = smartMatchProducts(search);
        const smartIds = {};
        smartResults.forEach(p => {
            smartIds[p.id] = true;
        });
        list = list.filter(p => smartIds[p.id];
        });
    }
    if (cat)
    ...
    if (lowOnly)
        list = list.filter(p => p.quantity <= (p.minStock || 5);
        });
    const skuSearch = (document.getElementById('filter-sku').value || '').toLowerCase().trim();
    if (skuSearch)
        list = list.filter(p => (p.sku || '').toLowerCase().includes(skuSearch);
        });
    const supFilter = (document.getElementById('filter-supplier').value || '').trim();
    if (supFilter)
        list = list.filter(p => (p.supplier || '') === supFilter;
        });
    ...
    const rows = list.map(function (p) {
        ...
        const catName = '\u2014';
        if (p.category) {
            const catObj = getCategories().find(c => c.id === p.category;
            });
        ...
        const starBtn = p.favorite ? '<span class="star-fav" onclick="toggleFavoriteFromTable(\'' + p.id + '\', event)" title="Убрать из избранного">\u2605</span>' : '<span class="star-no-fav" onclick="toggleFavoriteFromTable(\'' + p.id + '\', event)" title="Добавить в избранное">\u2606</span>';
        const row = [
        ...
        const discount = getProductDiscount(p.id);
        ...
        row.push(qty);
        const canEdit = checkPermission('editProducts');
        const canDelete = checkPermission('deleteProducts');
...
function exportProductsToExcel() {
    exportSectionToExcel('products', getProducts().slice().sort(a, b => String(a.name || '').localeCompare(String(b.name || ''), 'ru', { sensitivity: 'base' });
    }), 'SANAQ_Товары_' + todayStr() + '.xlsx');
}

function exportProductsToCSV() {
    const cfg = EXCEL_SECTIONS.products;
    if (!cfg)
        return;
    const data = getProducts().slice().sort(a, b => String(a.name || '').localeCompare(String(b.name || ''), 'ru', { sensitivity: 'base' });
    });
    const rows = data.map(item => cfg.headers.map(h, i => getFieldValue(item, cfg.fields[i]);
        });
    });
    const csvContent = '\uFEFF' + cfg.headers.join(';') + '\n' + rows.map(r => r.join(';');
    }).join('\n');
...
function openProductModal(id) {
    ...
    catSel.innerHTML = '<option value="">Без категории</option>' + getCategories().map(c => '<option value="' + c.id + '">' + c.name + '</option>';
    }).join('');
    if (id) {
...
function saveProduct() {
    ...
    let list = getProducts();
    const duplicate = list.find(p => p.code === code && p.id !== editId;
    });
    if (duplicate) {
    ...
    if (barcode) {
        const dupBc = list.find(p => p.barcode === barcode && p.id !== editId;
        });
        if (dupBc) {
    ...
    if (sku) {
        const dupSku = list.find(p => p.sku === sku && p.id !== editId;
        });
        if (dupSku) {
    ...
    if (editId) {
        list = list.map(p => p.id === editId ? Object.assign({}, p, data) : p;
        });
...
function addUniversalProduct() {
    const modal = document.getElementById('modal-universal-product');
    if (!modal) {
    ...
    const qtyEl = document.getElementById('uni-qty');
    if (nameEl) {
        nameEl.value = '';
        nameEl.focus();
    }
...
function confirmUniversalProduct() {
    ...
    const price = parseFloat(document.getElementById('uni-price').value) || 0;
    const qty = parseInt(document.getElementById('uni-qty').value) || 1;
    if (!name) {
        toast('Введите название товара', 'err');
        return;
    }
...
function renderPosCatProducts(catId) {
    try {
        ...
        const products = (getProducts() || []).filter(p => p.category === catId && p.quantity > 0;
        });
        if (!products.length) {
            list.style.cssText = '';
        ...
        }).map(p => '<div class="pos-quick-item" onclick="closeModal(\'modal-pos-categories\');addToCart(\'' + p.id + '\')">' + '<div class="qp-price">' + fmt(p.price) + '</div>' + '<div style="font-size:14px;color:#6b7280;margin-top:4px">' + esc(p.name) + '</div></div>';
        }).join('') + '</div>';
    } catch (e) {
...
function renderProductAnalysis() {
    ...
    const sort = (document.getElementById('stats-product-sort') || {}).value || 'top';
    const prodMap = {};
    sales.forEach(s => {
    ...
    const prodArr = Object.keys(prodMap).map(k => prodMap[k];
    });
    prodArr.sort(a, b => sort === 'top' ? b.qty - a.qty : a.qty - b.qty;
    });
    const container = document.getElementById('stats-product-analysis');
    if (!prodArr.length) {
        container.innerHTML = '<div class="empty">Нет продаж за период</div>';
        return;
    }
    const top = prodArr.slice(0, 5);
    const worst = prodArr.slice(-5).reverse();
    const html = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px">';
...
function debtBarcodeLookup() {
    const barcode = document.getElementById('debt-barcode').value.trim();
    if (!barcode)
        return;
    const p = getProducts().find(x => (x.barcode || '') === barcode;
    });
    if (p) {
