...
(function (global) {
  ...
  var SYNC_INTERVAL_MS = 30 * 1000; // 30 seconds
  ...
  function _immediateUpsert(table, items, toRow) {
    ...
    c.from(table).upsert(rows, { onConflict: 'id' })
      ...
      .catch(function (err) {
        console.warn('[ApDb] direct upsert failed for', table, err.message || err);
        pushToQueue({ type: 'upsert_' + table, payload: items.slice() });
        if (global.toast) global.toast('Ошибка синхронизации: данные сохранены локально', 'err');
      });
  }
  ...
  function _immediateUpsertSales(arr) {
    ...
    _replayUpsertSales(arr).then(function () {
      ...
      console.warn('[ApDb] direct sale upsert failed:', err.message || err);
      pushToQueue({ type: 'upsert_sales', payload: arr.slice() });
      if (global.toast) global.toast('Ошибка синхронизации продажи: данные сохранены локально', 'err');
    });
  }
