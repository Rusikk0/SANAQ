...
(function (global) {
  ...
  function _immediateUpsert(table, items, toRow) {
    ...
    c.from(table).upsert(rows, { onConflict: 'id' })
      ...
      .catch(function (err) {
        console.warn('[ApDb] direct upsert failed for', table, err && (err.message || err.error || err.description || err));
      ...
      for (var i = 0; i < offlineQueue.length; i++) {
        ...
        try {
          ...
          if (item.retries <= 3 || item.retries % 50 === 0) {
            console.warn('[ApDb] Queue item stuck after ' + item.retries + ' retries: ' + item.op.type + ' | ' + errMsg);
            ...
            else if (errMsg.indexOf('42P01') >= 0 || errMsg.indexOf('relation') >= 0) userMsg = 'Ошибка SQL: таблица не найдена (' + item.op.type + ')';
            else if (errMsg.indexOf('23505') >= 0) userMsg = 'Конфликт дубликата (unique violation): ' + item.op.type;
            else if (errMsg.indexOf('23503') >= 0) userMsg = 'Ошибка внешнего ключа (foreign key): ' + item.op.type;
