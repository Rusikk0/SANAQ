...
(function (global) {
  ...
  function getAppData(key) {
    ...
    if (key === 'permissions') {
      try { var id = sid(); if (id) { var r = localStorage.getItem('sanaq_permissions_' + id); if (r) return JSON.parse(r); } } catch(e) {}
    }
    if (key === 'promotions') {
      try { var id = sid(); if (id) { var r = localStorage.getItem('sanaq_promotions_' + id); if (r) return JSON.parse(r); } } catch(e) {}
    }
    return null;
  }
  function setAppData(key, value) {
    ...
    var legacyKey = legacyAppDataMap[key];
    if (legacyKey) {
      try { localStorage.setItem(legacyKey, typeof value === 'string' ? value : String(value)); } catch(e) {}
    }
    if (key === 'permissions') {
      try { var id = sid(); if (id) localStorage.setItem('sanaq_permissions_' + id, JSON.stringify(value)); } catch(e) {}
    }
    if (key === 'promotions') {
      try { var id = sid(); if (id) localStorage.setItem('sanaq_promotions_' + id, JSON.stringify(value)); } catch(e) {}
    }
  }
  ...
  function runMigrations() {
    try {
      var migKey = 'ap_migrated_v2_' + sid();
      if (!sid() || localStorage.getItem(migKey)) return;
      ...
      migrateLegacyJson('sanaq_permissions_' + sid(), 'permissions');
      migrateLegacyJson('sanaq_promotions_' + sid(), 'promotions');
