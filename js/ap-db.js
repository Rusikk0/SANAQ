...
(function (global) {
  ...
  function categoryFromRow(r) {
    return { id: r.id, name: r.name, subcategories: r.subcategories || [] };
  }
  ...
  async function loadAll() {
    ...
    localProducts.forEach(function (lp) {
      ...
      ['favorite','subcategory','sku','supplier','markup'].forEach(function (f) {
    ...
    // Переносим подкатегории из локального кэша, если в БД их ещё нет
    var localCats = cache.categories || [];
    cache.categories = (q[1].data || []).map(categoryFromRow);
    var catSubMap = {};
    localCats.forEach(function (lc) { if (lc.subcategories && lc.subcategories.length) catSubMap[lc.id] = lc.subcategories; });
    cache.categories.forEach(function (c) { if (catSubMap[c.id] && (!c.subcategories || !c.subcategories.length)) c.subcategories = catSubMap[c.id]; });
  ...
  async function syncCategories(arr) {
    ...
    if (arr.length) {
      var rows = arr.map(function (cat) {
        return { id: cat.id.length > 20 ? cat.id : undefined, store_id: id, name: cat.name, subcategories: cat.subcategories || [] };
      });
      ...
      if (!reload.error) {
        cache.categories = (reload.data || []).map(categoryFromRow);
        // Если колонки subcategories в БД ещё нет — восстанавливаем из arr
        var subMap = {};
        arr.forEach(function (ac) { if (ac.subcategories && ac.subcategories.length) subMap[ac.id] = ac.subcategories; });
        cache.categories.forEach(function (c) { if (subMap[c.id] && (!c.subcategories || !c.subcategories.length)) c.subcategories = subMap[c.id]; });
      }
    }
  }
