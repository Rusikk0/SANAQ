...
(function (global) {
  ...
  async function loadAll() {
    ...
    // Переносим локальное поле favorite на товары с сервера
    var localFavMap = {};
    localProducts.forEach(function (lp) { if (lp.favorite) localFavMap[lp.id] = true; });
    cache.products.forEach(function (p) { if (localFavMap[p.id]) p.favorite = true; });
