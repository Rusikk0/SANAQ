...
(function (global) {
  ...
  async function processOfflineQueue() {
    ...
    var c = sb();
    var id = sid();
    if (!c || !id) { syncing = false; isProcessingQueue = false; return; }
  ...
  async function syncCategories(arr) {
    ...
    if (arr.length) {
      var rows = arr.map(function (cat) {
        return { id: cat.id.length > 20 ? cat.id : undefined, store_id: id, name: cat.name, updated_at: new Date().toISOString() };
      });
